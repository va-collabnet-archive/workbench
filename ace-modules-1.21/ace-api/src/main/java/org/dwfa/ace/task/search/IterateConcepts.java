...
public class IterateConcepts extends AbstractTask {
    ...
    public Condition evaluate(I_EncodeBusinessProcess process, I_Work worker) throws TaskFailedException {
        try {
            if (conIt == null) {
                conIt = LocalVersionedTerminology.get().getConceptIterator();
                process.setProperty(uuidListListPropName, new ArrayList<List<UUID>>());
            } else {
                if (process.readProperty(this.componentPropName) != null) {
                    ArrayList<List<UUID>> componentList = (ArrayList<List<UUID>>) process.readProperty(uuidListListPropName);
