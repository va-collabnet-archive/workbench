...
public final class ColumnTypeCleanerImpl implements ColumnTypeCleaner {
    ...
    private String removeClosingTable(final String type) {
        return type.endsWith(TRAILING_CLOSE_TABLE) ? type.substring(0, type.length() - TRAILING_CLOSE_TABLE.length())
                                                  : type;
    }

    private String removeTrailingComma(final String column) {
        return column.endsWith(TRAILING_COLUMN_SEPARATOR) ? column.substring(0, column.length()
            - TRAILING_COLUMN_SEPARATOR.length()) : column;
    }
}
