...
public class Concept implements I_Transact, I_GetConceptData, ConceptChronicleBI, Comparable<Concept> {
    ...
    private static Concept mergeWithEConcept(EConcept eConcept, Concept c, boolean updateLucene) throws IOException {
    ...
    public Collection<Description.Version> getDescriptionVersions(NidSetBI allowedStatus, NidSetBI allowedTypes,
        ...
        for (Description d : descriptions) {
            versions.addAll(d.getVersions(allowedStatus, allowedTypes, viewPositions, precedence, contradictionMgr));
        }
        return versions;
    }
    ...
    public Collection<Relationship.Version> getSrcRelVersions(NidSetBI allowedStatus, NidSetBI allowedTypes,
        ...
        for (Relationship r : rels) {
            versions.addAll(r.getVersions(allowedStatus, allowedTypes, viewPositions, precedence, contradictionMgr));
        }
        return versions;
    }
    ...
    public Collection<Image.Version> getMediaVersions(NidSetBI allowedStatus, NidSetBI allowedTypes,
        ...
        for (Image m : media) {
            versions.addAll(m.getVersions(allowedStatus, allowedTypes, viewPositions, precedence, contradictionMgr));
        }
        return versions;
    }
    ...
    @Override
    public List<I_ConceptAttributeTuple> getConceptAttributeTuples(NidSetBI allowedStatus, PositionSetBI positionSet,
    ...
    public Collection<ConceptAttributes.Version> getConceptAttrVersions(NidSetBI allowedStatus,
        ...
        versions.addAll(getConceptAttributes().getVersions(allowedStatus, viewPositions, precedence, contradictionMgr));
        return versions;
    }
    ...
    public I_DescriptionTuple getDescTuple(I_IntList typePrefOrder, I_IntList langPrefOrder, NidSetBI allowedStatus,
        ...
        for (int prefNid : typePrefOrder.getListArray()) {
            typeSet.add(prefNid);
        }
        switch (sortPref) {
            case LANG_B4_TYPE:
    ...
    public Description.Version getDescTuple(I_IntList descTypePreferenceList, I_ConfigAceFrame config)
            throws IOException {
        return (Version) getDescTuple(descTypePreferenceList, config.getLanguagePreferenceList(), config.getAllowedStatus(), config.getViewPositionSetReadOnly(), config.getLanguageSortPref(), config.getPrecedence(), config.getConflictResolutionStrategy());
    }

    @Override
    public List<I_DescriptionTuple> getDescriptionTuples(NidSetBI allowedStatus, NidSetBI allowedTypes,
    ...
    @Override
    public List<I_DescriptionTuple> getDescriptionTuples() throws IOException, TerminologyException {

        I_ConfigAceFrame config = Terms.get().getActiveAceFrameConfig();

        return getDescriptionTuples(config.getAllowedStatus(), config.getDescTypes(), config.getViewPositionSetReadOnly(), config.getPrecedence(), config.getConflictResolutionStrategy());
    }
    ...
    public List<I_ImageTuple> getImageTuples() throws IOException, TerminologyException {
        ...
        return getImageTuples(config.getAllowedStatus(), null, config.getViewPositionSetReadOnly(), config.getPrecedence(), config.getConflictResolutionStrategy());
    }
    ...
    public List<? extends I_RelTuple> getSourceRelTuples(NidSetBI allowedTypes, boolean addUncommitted,
        ...
        throw new UnsupportedOperationException("Use a method that does not require getting the 'active' config");
    }
        ...
        if (srcRelTypes.size() > 0) {
            for (Relationship r : getSourceRels()) {
                ...
                r.addTuples(aceConfig.getAllowedStatus(), srcRelTypes, aceConfig.getViewPositionSetReadOnly(),
                        currentVersions, aceConfig.getPrecedence(), aceConfig.getConflictResolutionStrategy());
    ...
    public boolean promote(I_TestComponent test, I_Position viewPosition, PathSetReadOnly pomotionPaths,
            NidSetBI allowedStatus, Precedence precedence) throws IOException, TerminologyException {
    ...
    @Override
    public List<I_ImageTuple> getImageTuples(NidSetBI allowedStatus, NidSetBI allowedTypes, PositionSetBI positions,
            Precedence precedencePolicy, ContradictionManagerBI contradictionManager) throws IOException {
    ...
    @Override
    public List<I_RelTuple> getDestRelTuples(NidSetBI allowedStatus, NidSetBI allowedTypes, PositionSetBI positions,
            Precedence precedencePolicy, ContradictionManagerBI contradictionManager) throws IOException {
        List<I_RelTuple> returnRels = new ArrayList<I_RelTuple>();
    ...
    public I_RepresentIdSet getPossibleKindOfConcepts(NidSetBI isATypes, I_ShowActivity activity) throws IOException {
    ...
    @Override
    public Collection<? extends RelGroupChronicleBI> getRelGroups() throws IOException {
    ...
    public int compareTo(Concept concept) {
        {
            try {
                return this.getInitialText().compareTo(concept.getInitialText());
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
