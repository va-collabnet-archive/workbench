...
public abstract class RefsetMember<R extends RefsetRevision<R, C>, C extends RefsetMember<R, C>>
    ...
    public void readFromBdb(TupleInput input) {
        ...
        if (referencedComponentNid == 0) {
            try {
                referencedComponentNid = Bdb.uuidToNid(ArchitectonicAuxiliary.Concept.ERRONEOUS.getPrimoridalUid());
                AceLog.getAppLog().warning("Invalid RC nid == 0, for refset member: " + nid +
                        " in concept: " + Bdb.getUuidsToNidMap().getUuidsForNid( Bdb.getConceptNid(nid)));
            } catch (IOException ex) {
                Logger.getLogger(RefsetMember.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void writeToBdb(TupleOutput output, int maxReadOnlyStatusAtPositionNid) {
        assert referencedComponentNid != 0 : "rc == 0";
        if (referencedComponentNid == 0) {
            throw new RuntimeException("rc == 0");
        }
