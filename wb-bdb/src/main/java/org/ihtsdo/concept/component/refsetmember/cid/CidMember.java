...
public class CidMember extends RefsetMember<CidRevision, CidMember> implements I_ExtendByRefPartCid {
	...
	public CidMember(ERefsetCidMember refsetMember, Concept enclosingConcept) throws IOException {
		super(refsetMember, enclosingConcept);
		c1Nid = Bdb.uuidToNid(refsetMember.getC1Uuid());
		if (refsetMember.getRevisionList() != null) {
			revisions = new ArrayList<CidRevision>(refsetMember.getRevisionList().size());
			for (ERefsetCidRevision eVersion: refsetMember.getRevisionList()) {
	...
	protected final void readMemberParts(TupleInput input,
		...
		for (int i = 0; i < additionalVersionCount; i++) {
			CidRevision r = new CidRevision(input, this);
