...
public abstract class ConceptComponent<R extends Revision<R, C>, C extends ConceptComponent<R, C>>
		implements I_AmTermComponent, I_AmPart, I_AmTuple, I_Identify,
		I_IdPart, I_IdVersion, I_HandleFutureStatusAtPositionSetup {

	public static void addNidToBuffer(StringBuffer buf, int nidToConvert) {
		try {
			if (nidToConvert != 0 && Terms.get().hasConcept(nidToConvert)) {
				buf.append("\"");
				buf.append(Terms.get().getConcept(nidToConvert)
						.getInitialText());
				buf.append("\" [");
	...
	public String toString() {
		...
		if (primordialSapNid >= 0) {
			...
			buf
					.append(" !!! Invalid sapNid. Cannot compute path, time, status. !!! ");
		}
		buf.append("};");
		return buf.toString();
	}
	...
	public HashMap<Integer, ConceptComponent<?, ?>.Version> getSapMap() {
		...
		HashMap<Integer, ConceptComponent<?, ?>.Version> sapMap = new HashMap<Integer, ConceptComponent<?, ?>.Version>(
				size);
	...
	protected ConceptComponent(Concept enclosingConcept, TupleInput input)
		...
		assert this.primordialUNid != Integer.MIN_VALUE : "Processing nid: "
				+ enclosingConcept.getNid();
		assert nid != Integer.MAX_VALUE : "Processing nid: "
				+ enclosingConcept.getNid();
		assert nid != Integer.MIN_VALUE : "Processing nid: "
				+ enclosingConcept.getNid();
	}
	...
	protected ConceptComponent(EComponent<?> eComponent,
		...
		assert this.nid != Integer.MAX_VALUE : "Processing nid: "
				+ enclosingConcept.getNid();
		this.enclosingConceptNid = enclosingConcept.getNid();
		Bdb.getNidCNidMap().setCidForNid(this.enclosingConceptNid, this.nid);
		this.primordialSapNid = Bdb.getSapNid(eComponent);
		this.primordialUNid = Bdb.getUuidsToNidMap().getUNid(
				eComponent.getPrimordialComponentUuid());
	...
	public void merge(C component) {
		...
		HashMap<Integer, ConceptComponent<?, ?>.Version> newSapMap = component
				.getSapMap();
		newSapMap.keySet().removeAll(currentSapNids);
		for (ConceptComponent<?, ?>.Version v : newSapMap.values()) {
			assert !currentSapNids.contains(v.getSapNid()) : "currentSapNids: "
					+ currentSapNids + " v: " + v + " newSapMap: " + newSapMap;
			addRevision((R) v.makeAnalog(v.getStatusId(), v.getPathId(), v
					.getTime()));
		}
	...
	public void convertId(List<EIdentifier> list) {
		...
		additionalIdentifierParts = new ArrayList<IdentifierVersion>(list
				.size());
		for (EIdentifier idv : list) {
			Object denotation = idv.getDenotation();
			switch (IDENTIFIER_PART_TYPES.getType(denotation.getClass())) {
			case LONG:
				additionalIdentifierParts.add(new IdentifierVersionLong(
						(EIdentifierLong) idv));
				break;
			case STRING:
				additionalIdentifierParts.add(new IdentifierVersionString(
						(EIdentifierString) idv));
				break;
			case UUID:
				Bdb.getUuidsToNidMap().put((UUID) denotation, nid);
				additionalIdentifierParts.add(new IdentifierVersionUuid(
						(EIdentifierUuid) idv));
				break;
			default:
				throw new UnsupportedOperationException();
	...
	private void readIdentifierFromBdb(TupleInput input) {
		...
		for (int i = 0; i < listSize; i++) {
			...
			case LONG:
				...
				break;
			case STRING:
				IdentifierVersionString idvs = new IdentifierVersionString(
						input);
	...
	private final void writeIdentifierToBdb(TupleOutput output,
		...
		assert primordialSapNid != Integer.MAX_VALUE : "Processing nid: "
				+ enclosingConceptNid;
		assert primordialUNid != Integer.MIN_VALUE : "Processing nid: "
				+ enclosingConceptNid;
	...
	public final int getAuthorityNid() {
		try {
			return ArchitectonicAuxiliary.Concept.UNSPECIFIED_UUID.localize()
					.getNid();
		} catch (IOException e) {
			throw new RuntimeException(e);
		} catch (TerminologyException e) {
	...
	public final List<UUID> getUUIDs() {
		...
		if (additionalIdentifierParts != null) {
			for (IdentifierVersion idv : additionalIdentifierParts) {
				if (IdentifierVersionUuid.class
						.isAssignableFrom(idv.getClass())) {
	...
	public UniversalAceIdentification getUniversalId() throws IOException,
		...
		if (additionalIdentifierParts == null) {
			...
			universal = new UniversalAceIdentification(
					additionalIdentifierParts.size() + 1);
		}
	...
	public final List<Version> getMutableParts(
	...
	public abstract void writeToBdb(TupleOutput output,
			int maxReadOnlyStatusAtPositionNid);
	...
	public final void setPathId(int pathId) {
		...
		if (pathId != getPathId()) {
			this.primordialSapNid = Bdb.getSapNid(getStatusId(), pathId,
					Long.MAX_VALUE);
		}
	}
	...
	public final void setTime(long time) {
		...
		if (time != getTime()) {
			this.primordialSapNid = Bdb.getSapNid(getStatusId(), getPathId(),
					time);
		}
	}
	...
	public final void setStatusId(int statusId) {
		...
		if (statusId != this.getStatusId()) {
			this.primordialSapNid = Bdb.getSapNid(statusId, getPathId(),
					Long.MAX_VALUE);
		}
	}
	...
	public boolean conceptComponentFieldsEqual(ConceptComponent<R, C> another) {
		...
		if (this.additionalIdentifierParts != null
				&& another.additionalIdentifierParts == null) {
			return false;
		}
		if (this.additionalIdentifierParts == null
				&& another.additionalIdentifierParts != null) {
		...
		if (this.additionalIdentifierParts != null) {
			if (this.additionalIdentifierParts
					.equals(another.additionalIdentifierParts) == false) {
				return false;
			}
		}
	...
	public String validate(ConceptComponent<?, ?> another) throws IOException {
		...
		if (this.additionalIdentifierParts != null) {
			if (this.additionalIdentifierParts
					.equals(another.additionalIdentifierParts) == false) {
				buf
						.append("\tConceptComponent.additionalIdentifierParts not equal: \n"
								+ "\t\tthis.additionalIdentifierParts = "
								+ this.additionalIdentifierParts
								+ "\n"
								+ "\t\tanother.additionalIdentifierParts = "
								+ another.additionalIdentifierParts + "\n");
			}
		}
			...
			if (this.revisions.equals(another.revisions) == false) {
				for (int i = 0; i < this.revisions.size(); i++) {
					...
					if (another.revisions.size() > i) {
						Revision<R, C> thisRevision = (Revision<R, C>) this.revisions
								.get(i);
						Revision<R, C> anotherRevision = (Revision<R, C>) another.revisions
								.get(i);
						validationResults = thisRevision
								.validate(anotherRevision);
						if (validationResults.length() != 0) {
							buf.append("\tRevision[" + i + "] not equal: \n");
							buf.append(validationResults);
						}
					} else {
						buf.append("\tConceptComponent.revision[" + i
								+ "] not equal: \n");
						buf
								.append("\t\tThere is no corresponding Revision in another to compare it to.\n");
					}
				}
			}
		}
