...
public class ParallelConceptIterator implements Callable<Boolean>, I_FetchConceptFromCursor {
    ...
    private enum FETCH { ONE, TWO, THREE};
	...
	public ParallelConceptIterator(int first, int last, int count,
	        I_ProcessUnfetchedConceptData processor, 
			Database readOnly,
            Database mutable) {
		super();
		this.first = first;
    ...
    private Concept fetchThree() throws IOException {
        ...
        return Concept.get(currentCNid,
				...
				if (roKey == mutableKey) {
					...
					if (roKey < last) {
						...
						mutableKey = advanceCursor(mutableCursor,
								mutableFoundKey, mutableFoundDataPartial);
					} else {
