...
public class EConceptChangeSetComputer implements I_ComputeEConceptForChangeSet {
	...
	public String toString() {
		return "EConceptChangeSetComputer: minSapNid: " + minSapNid + 
			" maxSapNid: " + maxSapNid + " policy: " + policy;
	}
	
	public EConceptChangeSetComputer(ChangeSetPolicy policy, I_IntSet commitSapNids) {
		super();
		this.policy = policy;
		switch (policy) {
		case COMPREHENSIVE:
	...
	/* (non-Javadoc)
	 * @see org.ihtsdo.cs.I_ComputeEConceptForChangeSet#getEConcept(org.ihtsdo.concept.Concept)
	 */
	public EConcept getEConcept(Concept c) 
		throws IOException {
		EConcept ec = new EConcept();
		ec.setPrimordialUuid(c.getPrimUuid());
		ec.setConceptAttributes(processConceptAttributes(c));
		ec.setDescriptions(processDescriptions(c));
		ec.setRelationships(processRelationships(c));
		ec.setDestRelUuidTypeUuids(processNidNidList(c.getData().getDestRelNidTypeNidList()));
		ec.setImages(processImages(c));
		ec.setRefsetMembers(processRefsetMembers(c));
		ec.setRefsetUuidMemberUuidForConcept(processNidNidList(
				c.getData().getRefsetNidMemberNidForConceptList()));
		ec.setRefsetUuidMemberUuidForDescriptions(processNidNidList(
				c.getData().getRefsetNidMemberNidForDescriptionsList()));
		ec.setRefsetUuidMemberUuidForImages(processNidNidList(
				c.getData().getRefsetNidMemberNidForImagesList()));
		ec.setRefsetUuidMemberUuidForRefsetMembers(processNidNidList(
				c.getData().getRefsetNidMemberNidForRefsetMembersList()));
		ec.setRefsetUuidMemberUuidForRels(processNidNidList(
				c.getData().getRefsetNidMemberNidForRelsList()));
		return ec;
	}
	
	private List<ERefsetMember<?>> processRefsetMembers(Concept c) throws IOException {
		...
		for (RefsetMember<?, ?> member: c.getRefsetMembers()) {
				for (RefsetMember<?,?>.Version v: member.getTuples()) {
					...
					if (v.getSapNid() >= minSapNid && v.getSapNid() <= maxSapNid &&
	...
	private List<EImage> processImages(Concept c)  throws IOException {
	...
	private List<ERelationship> processRelationships(Concept c) throws IOException {
	...
	private List<EDescription> processDescriptions(Concept c) throws IOException {
	...
	private EConceptAttributes processConceptAttributes(Concept c) throws IOException {
	...
	private void setupRevision(EComponent ec,
			ConceptComponent.Version v, ERevision ev)
			throws IOException {
