...
public class WorkbenchRunner {
    ...
    public WorkbenchRunner(String[] args, LifeCycle lc) {
        try {
            ...
            /*
             * from
             * http://lists.apple.com/archives/java-dev/2004/oct/msg00591.html
             * 
             * The problem is that a ProgressMonitor would cause some kind of
             * background thread
             * which would throw up junk events which had to be processed, a
             * memory and CPU drain.
             * Specifically, a ProgressMonitor contains a JProgressBar and a
             * JPBar on Mac OS X uses
             * AquaProgressBarUI as its look-and-feel. APBarUI is buggy in that
             * even though the
             * progress bar may no longer be visible, the thread used to animate
             * the progress
             * bar never stops doing its thing. Each displayed JPBar has its own
             * thread, all
             * of which get stuck in memory, which can slow things down to a
             * crawl and cause,
             * as with my application, OutOfMemoryExceptions.
             * 
             * The solution is to not use AquaProgressBarUI. I have put the
             * following lines in
             * my code so that my application will use the BasicProgressBarUI
             * instead.
            ...
            if (ComponentFrameBean.MAC_OS_X) {
                ...
                javax.swing.UIManager.put("javax.swing.plaf.basic.BasicProgressBarUI", Class
                    .forName("javax.swing.plaf.basic.BasicProgressBarUI"));
            ...
            if (jiniConfig != null) {
                wbConfigFile =
                        (File) jiniConfig.getEntry(this.getClass().getName(), "wbConfigFile", File.class, new File(
                            "config/wb.config"));
                initializeFromSubversion =
                        (Boolean) jiniConfig.getEntry(this.getClass().getName(), "initFromSubversion", Boolean.class,
                            Boolean.FALSE);
                svnUpdateOnStart =
                        (String[]) jiniConfig.getEntry(this.getClass().getName(), "svnUpdateOnStart", String[].class, null);
                ACE.refsetOnly =
                        (Boolean) jiniConfig.getEntry(this.getClass().getName(), "refsetOnly", Boolean.class, Boolean.FALSE);
            } else {
            ...
            if (wbConfigFile != null && wbConfigFile.exists() && wbPropertiesFile.exists()) {
                ...
                if (userProfile == null) {
                    // user did not select a profile, they hit cancel
                ...
                ObjectInputStream ois = new ObjectInputStream(new BufferedInputStream(new FileInputStream(userProfile)));
            ...
            ChangeSetWriterHandler.addWriter(AceConfig.config.getUsername() + ".eccs", new EConceptChangeSetWriter(new File(
                AceConfig.config.getChangeSetRoot(), AceConfig.config.getChangeSetWriterFileName()), new File(
                AceConfig.config.getChangeSetRoot(), "." + AceConfig.config.getChangeSetWriterFileName()),
            ...
            ChangeSetWriterHandler.addWriter(AceConfig.config.getUsername() + ".commitLog.xls", new CommitLog(new File(
                AceConfig.config.getChangeSetRoot(), "commitLog.xls"), new File(AceConfig.config.getChangeSetRoot(), "."
                + "commitLog.xls")));
            ...
            if (successCount == 0) {
                JOptionPane.showMessageDialog(LogWithAlerts.getActiveFrame(null), "No frames where opened. Now exiting.",
    ...
    private static class GetProfileWorker extends SwingWorker<Boolean> {
        ...
        public GetProfileWorker(CountDownLatch latch) {
            ...
            loginDialog.setLocation((d.width / 2) - (loginDialog.getWidth() / 2), (d.height / 2)
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                try {
                    ...
                    firstStartup = false;
                    if (ace.getViewPositionSet() == null || ace.getViewPositionSet().size() == 0) {
                        Set<PositionBI> viewPositions = new HashSet<PositionBI>();

                        viewPositions.add(new Position(Integer.MAX_VALUE, Bdb.getPathManager().get(
    ...
    private void setupLookAndFeel() throws ConfigurationException, ClassNotFoundException, InstantiationException,
        ...
        if (jiniConfig != null) {
            String lookAndFeelClassName =
                    (String) jiniConfig.getEntry(this.getClass().getName(), "lookAndFeelClassName", String.class, UIManager
