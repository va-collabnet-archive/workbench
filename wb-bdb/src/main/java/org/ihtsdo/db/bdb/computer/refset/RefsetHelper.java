...
public class RefsetHelper extends RefsetUtilities implements I_HelpRefsets {
	...
	public I_IntSet getIsATypesForRefset(int refsetNid) throws Exception {
		...
		Set<? extends I_GetConceptData> requiredIsAType = memberRefset
				.getSourceRelTargets(getAllowedStatuses(), isATypes, null,
						false, true);
	...
	public I_ExtendByRefPartCid getFirstCurrentRefsetExtension(
		...
		for (I_ExtendByRef extension : Terms.get()
			...
			if (latestPart != null
					&& latestPart.getStatusId() == ReferenceConcepts.CURRENT
							.getNid()) {
				return latestPart;
			}
		}
	...
	public <T extends I_ExtendByRefPart> List<T> getAllCurrentRefsetExtensions(
		...
		for (I_ExtendByRef extension : Terms.get()
			...
			if (extension.getRefsetId() == refsetId) {
				...
				// current
				if (latestPart.getStatusId() == ReferenceConcepts.CURRENT
						.getNid()) {
	 ...
	 */
	public boolean hasRefsetExtension(int refsetId, int componentNid,
			final RefsetPropertyMap extProps) throws Exception {
    	access();
    	List<? extends I_ExtendByRef> extensions = Terms.get()
		.getAllExtensionsForComponent(componentNid, true);
		for (I_ExtendByRef extension : extensions) {

			if (extension == null) {
				AceLog.getAppLog().alertAndLogException(
					new Exception("Null extension in list: " + 
							extensions + " from component: " + 
	...
	public I_ExtendByRef getRefsetExtension(int refsetId,
		...
		for (I_ExtendByRef extension : Terms.get()
			...
			if (extension.getRefsetId() == refsetId &&
					extProps.validate((I_ExtendByRefPart) extension)) {
				return extension;
			}
		}
		return null;
	...
	public boolean hasCurrentRefsetExtension(int refsetId, int conceptId,
		...
		if (!extProps.hasProperty(RefsetPropertyMap.REFSET_PROPERTY.STATUS)) {
			extProps.with(RefsetPropertyMap.REFSET_PROPERTY.STATUS,
	...
	public <T extends I_ExtendByRefPart> I_ExtendByRef getOrCreateRefsetExtension(
			...
			final RefsetPropertyMap propMap) throws Exception {

    	access();
		I_ExtendByRef extension = getRefsetExtension(refsetId,
				componentId, propMap);
		...
		// editing)
		return makeMemberAndSetup(refsetId, componentId, type, propMap, UUID
				.randomUUID());
	}

	protected I_ExtendByRef makeMemberAndSetup(int refsetId,
			int referencedComponentNid, REFSET_TYPES type,
			final RefsetPropertyMap propMap, UUID memberUuid)
			throws IOException {

    	access();
		return BdbTermFactory.createMember(memberUuid, referencedComponentNid,
				type, Concept.get(refsetId), getConfig(), propMap);
	}

	public boolean newRefsetExtension(int refsetId, int componentId,
			REFSET_TYPES type, RefsetPropertyMap propMap,
			I_ConfigAceFrame config) throws Exception {
    	access();
		if (hasCurrentRefsetExtension(refsetId, componentId, propMap)) {
			return false;
		}
		getOrCreateRefsetExtension(refsetId, componentId, type, propMap);
		return true;
	}
	...
	public boolean retireRefsetExtension(int refsetId, int conceptId,
		...
		for (I_ExtendByRef extension : Terms.get()
			...
			if (extension.getRefsetId() == refsetId) {
				...
				if (!extProps
						.hasProperty(RefsetPropertyMap.REFSET_PROPERTY.STATUS)) {
					extProps.with(RefsetPropertyMap.REFSET_PROPERTY.STATUS,
							ReferenceConcepts.CURRENT.getNid());
				}
				...
				if (extProps.validate(latestPart)) {
					...
					I_ExtendByRefPartCid clone = (I_ExtendByRefPartCid) latestPart
							.makeAnalog(ReferenceConcepts.RETIRED.getNid(),
									latestPart.getPathId(), Long.MAX_VALUE);
	...
	public boolean hasPurpose(int refsetId, int purposeId) throws Exception {
		...
		if (refsetPurposeCache.containsKey(purposeId)) {
			...
			allowedTypes.add(RefsetAuxiliary.Concept.REFSET_PURPOSE_REL
					.localize().getNid());

			Set<? extends I_GetConceptData> destRelOrigins = purpose
					.getDestRelOrigins(allowedTypes, false, true);
	...
	public Set<? extends I_GetConceptData> getMarkedParentRefsetForRefset(
    	...
    	access();
		return getSourceRelTarget(refsetIdentityConcept, config,
				RefsetAuxiliary.Concept.MARKED_PARENT_REFSET.localize()
						.getNid());
	}
	...
	private Set<? extends I_GetConceptData> getSourceRelTarget(
			I_GetConceptData refsetIdentityConcept, I_ConfigAceFrame config,
			int refsetIdentityNid) throws IOException {
    	access();
		I_IntSet allowedTypes = Terms.get().newIntSet();
		allowedTypes.add(refsetIdentityNid);
		Set<? extends I_GetConceptData> matchingConcepts = refsetIdentityConcept
				.getSourceRelTargets(config.getAllowedStatus(), allowedTypes,
						config.getViewPositionSetReadOnly(), false);
	...
	private Set<? extends I_GetConceptData> getDestRelOrigins(
			I_GetConceptData refsetIdentityConcept, I_ConfigAceFrame config,
			int refsetIdentityNid) throws IOException {
    	access();
		I_IntSet allowedTypes = Terms.get().newIntSet();
		allowedTypes.add(refsetIdentityNid);
		Set<? extends I_GetConceptData> matchingConcepts = refsetIdentityConcept
				.getDestRelOrigins(config.getAllowedStatus(), allowedTypes,
						config.getViewPositionSetReadOnly(), false);
		return matchingConcepts;
	}
