...
public class VersionComputer<V extends ConceptComponent<?, ?>.Version> {
	...
	public void addSpecifiedVersions(I_IntSet allowedStatus,
			I_Position viewPosition, List<V> specifiedVersions, List<V> versions) {
			...
			if (mapper.onRoute(version)) {
				if (partsForPosition.size() == 0) {
					...
					while (latestIterator.hasNext()) {
						...
						switch (mapper
								.fastRelativePosition(version, partToTest)) {
						case AFTER:
							if (added) {
	...
	public Collection<V> getSpecifiedVersions(I_IntSet allowedStatus,
			I_Position viewPosition, List<? extends V> versions) {
			...
			for (V part : versions) {
				...
				if (mapper.onRoute(part)) {
					if (partsForPosition.size() == 0) {
						...
						List<V> partsToCompare = new ArrayList<V>(
								partsForPosition);
						for (V prevPartToTest : partsToCompare) {
							...
							case EQUAL:
								...
								AceLog.getAppLog()
										.warning(RELATIVE_POSITION.EQUAL
	...
	private void addSpecifiedVersionsNullPositions(I_IntSet allowedStatus,
			...
			boolean addUncommitted, List<? extends V> versions) {
		if (versions == null) {
			return;
		}
