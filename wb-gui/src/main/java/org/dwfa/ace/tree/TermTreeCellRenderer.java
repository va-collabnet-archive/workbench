...
import org.dwfa.cement.*;
import org.dwfa.tapi.TerminologyException;
import org.dwfa.vodb.bind.*;
...
public class TermTreeCellRenderer extends DefaultTreeCellRenderer
   ...
   public TermTreeCellRenderer(I_ConfigAceFrame aceConfig, TermTreeHelper helper)
         ...
         if (node.getUserObject() != null) {
            if (I_GetConceptDataForTree.class.isAssignableFrom(node.getUserObject().getClass())) {
               ...
               List<? extends I_ConceptAttributeTuple> attributes =
                  cb.getConceptAttributeTuples(aceConfig.getAllowedStatus(),
               ...
               if (tdt != null) {
                  ...
                  if (showViewerImagesInTaxonomy) {
                     for (I_ImageTuple imageTuple :
                             cb.getImageTuples(aceConfig.getAllowedStatus(), viewerImageTypes,
                                               aceConfig.getViewPositionSetReadOnly(),
                                               aceConfig.getPrecedence(),
                                               aceConfig.getConflictResolutionStrategy())) {
                  ...
                  if (showPathInfoInTaxonomy) {
                     showPathInfoInTaxonomy(cb, htmlSuffixes);
                  }
                  ...
                  if (showRefsetInfoInTaxonomy) {
                     showRefsetInfoInTaxonomy(cb, descriptionsWithExtensions, htmlPrefixes, htmlSuffixes);
                  }
               ...
               List<? extends I_RelTuple> versions = cb.getSourceRelTuples(aceConfig.getAllowedStatus(),
               ...
               if (sourceRelTupleSize > 1) {
                  ...
                  this.setBorder(BorderFactory.createCompoundBorder(this.getBorder(),
                          BorderFactory.createMatteBorder(0, iconRect.x, 0, 0,
               ...
               for (I_OverrideTaxonomyRenderer override : aceConfig.getTaxonomyRendererOverrideList()) {
                  override.overrideTreeCellRendererComponent(this, tree, cb, sel, expanded, leaf, row,
                          hasFocus, aceConfig);
               }
            } else {
               this.setIcon(null);
   ...
   private void showRefsetInfoInTaxonomy(I_GetConceptDataForTree cb,
           List<I_DescriptionVersioned> descriptionsWithExtensions, List<String> htmlPrefixes,
      ...
      for (I_DescriptionVersioned desc : descriptions) {
         List<? extends I_ExtendByRef> descExts = Terms.get().getAllExtensionsForComponent(desc.getDescId());
            ...
            if ((ebr != null) && (ebr.getRefsetId() == i)) {
               ...
               case BOOLEAN :
                  ebr.addTuples(aceConfig.getAllowedStatus(), aceConfig.getViewPositionSetReadOnly(),
                                returnTuples, aceConfig.getPrecedence(),
                                aceConfig.getConflictResolutionStrategy());
               ...
               case CID :
                  ...
                  for (I_ExtendByRefVersion t : returnTuples) {
                     I_GetConceptData ebrCb =
                        Terms.get().getConcept(((I_ExtendByRefPartCid) t.getMutablePart()).getC1id());

                     for (I_ImageTuple imageTuple :
                             ebrCb.getImageTuples(aceConfig.getAllowedStatus(), viewerImageTypes,
                                                  aceConfig.getViewPositionSetReadOnly(),
                                                  aceConfig.getPrecedence(),
                                                  aceConfig.getConflictResolutionStrategy())) {
                        htmlPrefixes.add("<img src='ace:" + imageTuple.getNid() + "$"
   ...
   private void showPathInfoInTaxonomy(I_GetConceptDataForTree cb, List<String> htmlSuffixes)
           throws TerminologyException, IOException {
