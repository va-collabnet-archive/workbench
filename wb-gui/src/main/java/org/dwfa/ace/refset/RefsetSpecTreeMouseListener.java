...
public class RefsetSpecTreeMouseListener extends MouseAdapter {
	...
	private void makeAndShowPopup(MouseEvent e) throws TerminologyException {
		try {
			...
			AceLog.getAppLog().info(
					"makeAndShowPopup getLocalEditState = "
							+ specEditor.getLocalEditState());
			JPopupMenu popup = null;

			if (e.isPopupTrigger()
					&& !specEditor.getLocalEditState().equals(
							EditState.READONLY)) {
				try {
					JTree tree = (JTree) e.getSource();
					int rowForLocation = tree.getRowForLocation(e.getX(),
							e.getY());
					int[] selectedRow = tree.getSelectionRows();
					if (rowForLocation < 0 || selectedRow == null
							|| selectedRow[0] != rowForLocation) {
						tree.clearSelection();
						// no row selected so show grouping clauses and
						// concept-contains clauses
						if (specEditor.getLocalEditState().equals(
								EditState.EDIT)) {
						...
						if (selPath != null) {
							if (rowForLocation != -1) {
								...
								if (specEditor.getLocalEditState().equals(
									...
									Collection<? extends I_ExtendByRef> extensions = Terms
											.get()
											.getRefsetExtensionMembers(
													aceConfig
															.getRefsetSpecInSpecEditor()
															.getConceptNid());

									HashMap<Integer, I_ExtendByRef> memberIdBasedExtensionMap = new HashMap<Integer, I_ExtendByRef>();
									memberIdBasedExtensionMap = populateMemberIdBasedExtensionMap(extensions);

									switch (EConcept.REFSET_TYPES
											.nidToType(specPart.getTypeNid())) {
									case CID_CID:
										...
										if (refsetSpecHelper
												.isDescriptionComputeType()) {
											// show AND, OR, !AND, !OR
											// show desc clauses
											excludeDesc = false;
										} else if (refsetSpecHelper
												.isRelationshipComputeType()) {
											// show AND, OR, !AND, !OR
											// show rel clauses
											excludeRel = false;
										} else {
											if (clauseIsChildOfConceptContainsDesc(
													specPart,
													memberIdBasedExtensionMap)) {
												// show AND, OR, !AND, !OR
												// show desc clauses
												excludeDesc = false;
											} else if (clauseIsChildOfConceptContainsRel(
													specPart,
													memberIdBasedExtensionMap)) {
												// show AND, OR, !AND, !OR
												// show rel clauses
										...
										popup = makePopup(e, specPart,
												excludeConcept, excludeDesc,
												excludeRel, excludeContains);
										break;
									case CID_CID_CID:
										popup = makePopup(
												e,
								...
								// no row selected so show grouping clauses and
								// concept-contains clauses
								popup = makePopup(e, null, true, true, true,
										false);
							}
						}
					}
	...
	private boolean clauseIsChildOfConceptContainsRel(I_ExtendByRef specPart,
		...
		int conceptContainsRelNid = RefsetAuxiliary.Concept.CONCEPT_CONTAINS_REL_GROUPING
				.localize().getNid();
		I_ExtendByRefPartCidCid cidCidPart = (I_ExtendByRefPartCidCid) specPart;
		if (cidCidPart.getC2id() == conceptContainsRelNid) {
			return true;
		} else {
			I_ExtendByRef parentSpecPart = componentIdBasedExtensionMap
					.get(specPart.getComponentNid());
			if (parentSpecPart == null) {
				return false;
			} else {
				return clauseIsChildOfConceptContainsRel(parentSpecPart,
						componentIdBasedExtensionMap);
			}
		}
	}
	...
	private boolean clauseIsChildOfConceptContainsDesc(I_ExtendByRef specPart,
		...
		int conceptContainsDescNid = RefsetAuxiliary.Concept.CONCEPT_CONTAINS_DESC_GROUPING
				.localize().getNid();
		I_ExtendByRefPartCidCid cidCidPart = (I_ExtendByRefPartCidCid) specPart;
		if (cidCidPart.getC2id() == conceptContainsDescNid) {
			...
			if (parentSpecPart == null) {
				...
				return clauseIsChildOfConceptContainsDesc(parentSpecPart,
						componentIdBasedExtensionMap);
			}
		}
	}

	private JPopupMenu makeCommentOnlyPopup(I_ExtendByRef specPart)
			throws IOException, TerminologyException {
		...
		if (specPart != null) {
			...
			if (uncommitted) {
				...
				List<I_ExtendByRefVersion> tuples = (List<I_ExtendByRefVersion>) specPart
						.getTuples(aceConfig.getAllowedStatus(),
	...
	private JPopupMenu makePopup(MouseEvent e, I_ExtendByRef specPart,
			boolean excludesConcept, boolean excludesDesc, boolean excludesRel,
			boolean excludesContains) throws FileNotFoundException,
			IOException, ClassNotFoundException, TerminologyException {
		JPopupMenu popup = new JPopupMenu();
		JMenuItem noActionItem = new JMenuItem("");
		popup.add(noActionItem);

		// adding grouping clauses (OR, AND, !OR, !AND) - these are always
		// displayed
		File groupingFile = new File(AceFrame.pluginRoot,
				"refsetspec/branch-popup/grouping");
		...
		if (!excludesContains) {
			File containsFile = new File(AceFrame.pluginRoot,
					"refsetspec/branch-popup/contains");
			JMenu newSubMenuContains = new JMenu(containsFile.getName());
			popup.add(newSubMenuContains);
			ProcessPopupUtil.addSubmenMenuItems(newSubMenuContains,
					containsFile, this.aceConfig.getWorker());
		}

		// sub-menu for concept based clauses e.g. concept is, concept is child
		// of
		if (!excludesConcept) {
			File conceptFile = new File(AceFrame.pluginRoot,
					"refsetspec/branch-popup/concept");
			...
			ProcessPopupUtil.addSubmenMenuItems(newSubMenuConcept, conceptFile,
					this.aceConfig.getWorker());
			// sub-menu for diff
			conceptFile = new File(AceFrame.pluginRoot,
					"refsetspec/branch-popup/diff");
		...
		if (!excludesDesc) {
			File descFile = new File(AceFrame.pluginRoot,
					"refsetspec/branch-popup/desc");
			JMenu newSubMenuDesc = new JMenu(descFile.getName());
			popup.add(newSubMenuDesc);
			ProcessPopupUtil.addSubmenMenuItems(newSubMenuDesc, descFile,
					this.aceConfig.getWorker());
		}

		// sub-menu for rel based clauses e.g. rel is
		if (!excludesRel) {
			File relFile = new File(AceFrame.pluginRoot,
					"refsetspec/branch-popup/rel");
			JMenu newSubMenuRel = new JMenu(relFile.getName());
			popup.add(newSubMenuRel);
			ProcessPopupUtil.addSubmenMenuItems(newSubMenuRel, relFile,
					this.aceConfig.getWorker());
		}
	...
	private JPopupMenu makePopup(MouseEvent e, File directory,
			I_ExtendByRef specPart) throws FileNotFoundException, IOException,
			ClassNotFoundException, TerminologyException {
		JPopupMenu popup = new JPopupMenu();
		JMenuItem noActionItem = new JMenuItem("");
		popup.add(noActionItem);
		ProcessPopupUtil.addSubmenMenuItems(popup, directory,
				this.aceConfig.getWorker());
		if (specPart != null) {
			popup.addSeparator();
						...
						for (I_GetConceptData commentRefsetIdentityConcept : commentRefsets) {
							...
							I_ExtendByRef newExtension = refsetHelper
									.getOrCreateRefsetExtension(
											...
											REFSET_TYPES.STR, refsetMap, UUID
													.randomUUID());
	...
	public static void addExtensionsToMap(
		...
		for (I_ExtendByRef ext : list) {
			if (ext.getRefsetId() == refsetNid) {
				extensionMap.put(ext.getMemberId(), new DefaultMutableTreeNode(
						ext));
	...
	private class RetireSpecAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			try {
				...
				I_ExtendByRef clauseBeingRetired = (I_ExtendByRef) thinExtByRefTuple
						.getCore();
				Collection<? extends I_ExtendByRef> extensions = Terms.get()
						.getAllExtensionsForComponent(
								aceConfig.getRefsetSpecInSpecEditor()
										.getConceptNid(), true);
				...
				fetchedComponents.add(aceConfig.getRefsetSpecInSpecEditor()
						.getConceptNid());

				addExtensionsToMap(extensions, extensionMap, fetchedComponents,
						aceConfig.getRefsetSpecInSpecEditor().getConceptNid());

				DefaultMutableTreeNode selectedRoot = new DefaultMutableTreeNode(
						clauseBeingRetired);
				DefaultMutableTreeNode specRoot = new DefaultMutableTreeNode(
						clauseBeingRetired);
				extensionMap.put(aceConfig.getRefsetSpecInSpecEditor()
						.getConceptNid(), specRoot);

				for (DefaultMutableTreeNode extNode : extensionMap.values()) {
					I_ExtendByRef ext = (I_ExtendByRef) extNode.getUserObject();
					if (ext.getComponentNid() == clauseBeingRetired
							.getMemberId()) {
						selectedRoot.add(extNode);
					} else if (ext.getComponentNid() == aceConfig
							.getRefsetSpecInSpecEditor().getConceptNid()) {
						// do nothing
					} else {
				...
				if (childCount > 0) {
					// display dialog that alerts the user that there are
					// children clauses which need to be retired
					...
					int n = JOptionPane
							.showOptionDialog(
									...
									"Multiple clause retirement",
									JOptionPane.YES_NO_OPTION,
									JOptionPane.QUESTION_MESSAGE, null,
									options, options[0]);
					if (n == JOptionPane.YES_OPTION) {
						try {
							// retire all children as well as the
							// selected/current clause
		...
		private void retireAllDescendants(DefaultMutableTreeNode selectedRoot)
				throws Exception {
			int childCount = selectedRoot.getChildCount();
			for (int i = 0; i < childCount; i++) {

				DefaultMutableTreeNode childNode = (DefaultMutableTreeNode) selectedRoot
						.getChildAt(i);
				I_ExtendByRef childClause = (I_ExtendByRef) childNode
						.getUserObject();
				List<? extends I_ExtendByRefVersion> childExtensions = childClause
						.getTuples(aceConfig.getAllowedStatus(),
								aceConfig.getViewPositionSetReadOnly(),
								aceConfig.getPrecedence(),
								aceConfig.getConflictResolutionStrategy());
		...
		private void retireClause(I_ExtendByRefVersion currentExtVersion)
				throws Exception {
			...
			I_ExtendByRefPart newPart = (I_ExtendByRefPart) currentPart
					.makeAnalog(ArchitectonicAuxiliary.Concept.RETIRED
							.localize().getNid(), currentPart.getPathNid(),
							Long.MAX_VALUE);
			...
			I_GetConceptData refsetIdentityConcept = aceConfig
					.getRefsetInSpecEditor();
			RefsetSpec refsetSpec = new RefsetSpec(refsetIdentityConcept, true,
					aceConfig);
			...
			if (specConcept != null && editTimeConcept != null) {
				helper.newLongRefsetExtension(editTimeConcept.getConceptNid(),
						specConcept.getConceptNid(), System.currentTimeMillis());
			}
	...
	private class CancelChangeAction implements ActionListener {
				...
				if (childCount > 0) {
					...
					int n = JOptionPane
							.showOptionDialog(
									OpenFrames.getActiveFrame(),
									"The selected clause has child-clauses. How would you like to proceed?",
									"Multiple clause cancel",
									JOptionPane.YES_NO_OPTION,
									JOptionPane.QUESTION_MESSAGE, null,
									options, options[0]);
					if (n == JOptionPane.YES_OPTION) {
						try {
							// cancel all children as well as the
							// selected/current clause
		...
		private void forgetAllDescendants(DefaultMutableTreeNode selectedRoot)
				throws Exception {
			int childCount = selectedRoot.getChildCount();
			for (int i = 0; i < childCount; i++) {
		...
		private void forgetClause(I_ExtendByRef specPart) throws Exception {
			...
			try {
				if (specPart.isUncommitted()) {
					...
					I_GetConceptData refsetIdentityConcept = aceConfig
							.getRefsetInSpecEditor();
					RefsetSpec refsetSpec = new RefsetSpec(
							refsetIdentityConcept, true, aceConfig);
					refsetSpec.setLastEditTime(System.currentTimeMillis());
				}
				Terms.get().addUncommitted(
						Terms.get().getConcept(specPart.getRefsetId()));
			} catch (IOException e) {
				AceLog.getAppLog().alertAndLogException(e);
			} catch (TerminologyException e) {
				AceLog.getAppLog().alertAndLogException(e);
	...
	private class ChangeSpecAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			I_ExtendByRefPart newPart = (I_ExtendByRefPart) current.makeAnalog(
					current.getStatusNid(), current.getPathNid(),
					Long.MAX_VALUE);
