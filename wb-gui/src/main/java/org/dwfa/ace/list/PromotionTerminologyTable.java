...
public class PromotionTerminologyTable extends JTable {

    PromotionTerminologyTableModel tableModel;
    I_ConfigAceFrame config;
    private boolean confirmDelete = false;

    public PromotionTerminologyTable(PromotionTerminologyTableModel dataModel, I_ConfigAceFrame config) {
        super(dataModel);
    ...
    private void init(boolean allowDelete) {
        ...
        TableRowSorter<PromotionTerminologyTableModel> sorter = 
                new TableRowSorter<PromotionTerminologyTableModel>((PromotionTerminologyTableModel) getModel());
        ...
        if (allowDelete) {
            PromotionTerminologyTable.DeleteAction delete = new PromotionTerminologyTable.DeleteAction();
    ...
    public TableCellRenderer getCellRenderer(int row, int column) {
        return new PromotionTerminologyTableRenderer(config);
    }
    ...
    private class DeleteAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            if (confirmDelete) {
                ...
                if (index >= 0) {
                    PromotionTerminologyTableModel tm = (PromotionTerminologyTableModel) getModel();
                    ConceptChronicleBI element = tm.getElementAt(index);
                    if (element.isUncommitted()) {
                        JOptionPane.showMessageDialog(PromotionTerminologyTable.this,
                                "<html>Uncommitted items cannot be removed from the list.<br><br>"
                                + "Either cancel or commit changes prior to removal.");
                    } else {
                        int option = JOptionPane.showConfirmDialog(PromotionTerminologyTable.this,
