...
public class WorklistMemberReAssignment extends JPanel {
    ...
    public WorklistMemberReAssignment(WorkList workList, I_ConfigAceFrame config) {
        initComponents();
        JButton batchApproveButton = new JButton("Batch Approve and Promote");
        batchApproveButton.setFont(new Font("Lucida Grande", Font.PLAIN, 11));
        panel5.add(batchApproveButton, new GridBagConstraints(1, 3, 1, 1, 0.0, 0.0, GridBagConstraints.PAGE_END, GridBagConstraints.NONE, new Insets(0, 0, 0, 5), 0, 0));
        batchApproveButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                performBatchApproveDirect(e);
            }
        });
    ...
    private void performBatchApproveDirect(ActionEvent e) {
        DefaultTableModel model = (DefaultTableModel) membersTable2.getModel();
        if (model.getRowCount() > 0) {
            pBarW.setVisible(true);
            ...
            int n = JOptionPane.showConfirmDialog(
                    this,
                    "This function will Approve and Promote all selected content (" + model.getRowCount() + " items).\n" +
                    "Workflow logic will not be applied, the final state may differ \n" +
                    "from the results of normal workflow progression. \n\n" +
                    "Do you want to proceed?",
                    "Workflow skip warning",
            ...
            if (n == JOptionPane.YES_OPTION) {
                ...
                Thread queryThread = new Thread() {
                    public void run() {
                        try {
                            DefaultTableModel model = (DefaultTableModel) membersTable2.getModel();
                            Concept approved = Concept.APPROVED_FOR_PUBLICATION_STATUS;
                            I_GetConceptData stateConcept = Terms.get().getConcept(approved.getUids());
                            I_ConfigAceFrame config = Terms.get().getActiveAceFrameConfig();
                            I_GetConceptData defaultWorkflow = config.getDefaultWorkflowForChangedConcept();
                            config.setDefaultWorkflowForChangedConcept(null);
                            ...
                            while (model.getRowCount() > 0) {
                                WorkListMember wlMember = (WorkListMember) model.getValueAt(0, 0);
                                wlMember.setActivityStatus(stateConcept);
                                TerminologyProjectDAO.updateWorkListMemberMetadata(wlMember, config);
                                promotionNids.setMember(wlMember.getConcept().getNid());
                                model.removeRow(0);
                            }
                            System.out.println("Nids to promote: " + promotionNids.cardinality());
                            TerminologyProjectDAO.promoteContent(promotionNids,
                                    config.getViewCoordinate(), project.getReleasePath().getNid());
                            Terms.get().commit();
                            config.setDefaultWorkflowForChangedConcept(defaultWorkflow);
                            Terms.get().setActiveAceFrameConfig(config);
                            pBarW.setVisible(false);
                            JOptionPane.showMessageDialog(WorklistMemberReAssignment.this, "Worklist members sent with Direct Batch Approval!", "Message", JOptionPane.INFORMATION_MESSAGE);
                            pBarW.setVisible(false);
                ...
                // cancelled
                JOptionPane.showMessageDialog(WorklistMemberReAssignment.this, "Action cancelled", "Message", JOptionPane.INFORMATION_MESSAGE);
                pBarW.setVisible(false);
            } else {
                // no option selected
                JOptionPane.showMessageDialog(WorklistMemberReAssignment.this, "No action taken", "Message", JOptionPane.INFORMATION_MESSAGE);
                pBarW.setVisible(false);
            }
        }
    }
