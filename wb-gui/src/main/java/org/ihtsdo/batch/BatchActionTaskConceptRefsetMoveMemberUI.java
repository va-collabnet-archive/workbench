 ...
 * @author marc
 */
public class BatchActionTaskConceptRefsetMoveMemberUI extends javax.swing.JPanel implements I_BatchActionTask {
    ...
    public BatchActionTask getTask(EditCoordinate ec, ViewCoordinate vc, List<ConceptChronicleBI> concepts) {
        ...
        if (refsetBI != null) {
            ...
            ((BatchActionTaskConceptRefsetMoveMember) task).setCollectionFromNid(refsetMoveFromNid);
        } else {
            BatchActionEventReporter.add(new BatchActionEvent(null, BatchActionTaskType.CONCEPT_REFSET_MOVE_MEMBER,
        ...
        if (refsetMoveToCB != null) {
            ((BatchActionTaskConceptRefsetMoveMember) task).setCollectionToNid(refsetMoveToCB.getConceptNid());
        } else {
            BatchActionEventReporter.add(new BatchActionEvent(null, BatchActionTaskType.CONCEPT_REFSET_MOVE_MEMBER,
        ...
        if (jCheckBoxMatch.isSelected() == false) {
            ((BatchActionTaskConceptRefsetMoveMember) task).setRefsetType(null);
            ((BatchActionTaskConceptRefsetMoveMember) task).setMatchValue(null);
            return task;
        }
        ...
        switch (jComboBoxType.getSelectedIndex()) {
            case 0:
                ...
                if (valBoolean != null) {
                    ((BatchActionTaskConceptRefsetMoveMember) task).setMatchValue(valBoolean);
                    return task;
                }
                break;
            case 1:
                ((BatchActionTaskConceptRefsetMoveMember) task).setRefsetType(TK_REFEX_TYPE.CID);
                Integer valConcept = ((ValueDndNidUI) jPanelValueMatch).getValue();
                if (valConcept != null) {
                    ((BatchActionTaskConceptRefsetMoveMember) task).setMatchValue(valConcept);
                    return task;
                }
                break;
            case 2:
                ((BatchActionTaskConceptRefsetMoveMember) task).setRefsetType(TK_REFEX_TYPE.INT);
                Integer valInt = ((ValueIntUI) jPanelValueMatch).getValue();
                if (valInt != null) {
                    ((BatchActionTaskConceptRefsetMoveMember) task).setMatchValue(valInt);
                    return task;
                }
                break;
            case 3:
                ((BatchActionTaskConceptRefsetMoveMember) task).setRefsetType(TK_REFEX_TYPE.STR);
                String valStr = ((ValueStringUI) jPanelValueMatch).getValue();
                if (valStr != null) {
                    ((BatchActionTaskConceptRefsetMoveMember) task).setMatchValue(valStr);
                    return task;
                }
                break;
            default:
