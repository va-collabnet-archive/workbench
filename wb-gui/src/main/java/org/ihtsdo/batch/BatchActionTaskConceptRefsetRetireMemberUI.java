 ...
 * @author marc
 */
public class BatchActionTaskConceptRefsetRetireMemberUI extends javax.swing.JPanel implements I_BatchActionTask {
    ...
    public BatchActionTask getTask(EditCoordinate ec, ViewCoordinate vc,
        ...
        if (refsetBI != null) {
            ...
            ((BatchActionTaskConceptRefsetRetireMember) task).setCollectionNid(refsetNid);
        } else {
            BatchActionEventReporter.add(new BatchActionEvent(null, BatchActionTaskType.CONCEPT_REFSET_RETIRE_MEMBER,
        ...
        if (jCheckBoxMatch.isSelected() == false) {
            ((BatchActionTaskConceptRefsetRetireMember) task).setRefsetType(null);
            ((BatchActionTaskConceptRefsetRetireMember) task).setMatchValue(null);
            return task;
        }
        ...
        switch (currentValueTypeIdx) {
            case 0:
                ...
                if (valBoolean != null) {
                    ((BatchActionTaskConceptRefsetRetireMember) task).setMatchValue(valBoolean);
                    return task;
                }
                break;
            case 1:
                ((BatchActionTaskConceptRefsetRetireMember) task).setRefsetType(TK_REFEX_TYPE.CID);
                Integer valConcept = ((ValueDndNidUI) jPanelValueMatch).getValue();
                if (valConcept != null) {
                    ((BatchActionTaskConceptRefsetRetireMember) task).setMatchValue(valConcept);
                    return task;
                }
                break;
            case 2:
                ((BatchActionTaskConceptRefsetRetireMember) task).setRefsetType(TK_REFEX_TYPE.INT);
                Integer valInt = ((ValueIntUI) jPanelValueMatch).getValue();
                if (valInt != null) {
                    ((BatchActionTaskConceptRefsetRetireMember) task).setMatchValue(valInt);
                    return task;
                }
                break;
            case 3:
                ((BatchActionTaskConceptRefsetRetireMember) task).setRefsetType(TK_REFEX_TYPE.STR);
                String valStr = ((ValueStringUI) jPanelValueMatch).getValue();
                if (valStr != null) {
                    ((BatchActionTaskConceptRefsetRetireMember) task).setMatchValue(valStr);
                    return task;
                }
                break;
            default:
