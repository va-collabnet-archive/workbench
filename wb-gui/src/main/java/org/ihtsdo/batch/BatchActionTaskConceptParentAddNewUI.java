...
public class BatchActionTaskConceptParentAddNewUI extends javax.swing.JPanel implements I_BatchActionTask {
    ...
    public BatchActionTask getTask(EditCoordinate ec, ViewCoordinate vc, List<ConceptChronicleBI> concepts) throws IOException {
        ...
        if (termNewParentDest != null && termNewParentDest.getUUIDs().size() > 0) {
            ...
            ((BatchActionTaskConceptParentAddNew) task).setSelectedRoleTypeUuid(uuidParentLinkage);
            ((BatchActionTaskConceptParentAddNew) task).setSelectedDestUuid(uuidNewParentDest);
            return task;
        } else {
            BatchActionEventReporter.add(new BatchActionEvent(null, BatchActionTaskType.CONCEPT_PARENT_ADD_NEW,
