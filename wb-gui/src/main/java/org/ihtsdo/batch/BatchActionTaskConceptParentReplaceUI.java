...
public class BatchActionTaskConceptParentReplaceUI extends javax.swing.JPanel implements I_BatchActionTask {
    ...
    public BatchActionTask getTask(EditCoordinate ec, ViewCoordinate vc,
        ...
        if (fromParentBI != null && termParentTo != null && termParentTo.getUUIDs().size() > 0) {
            ...
            ((BatchActionTaskConceptParentReplace) task).setMoveFromRoleTypeNid(nidLinkage);
            ((BatchActionTaskConceptParentReplace) task).setMoveFromDestNid(nidOldParent);
            ((BatchActionTaskConceptParentReplace) task).setMoveToRoleTypeUuid(uuidLinkage);
            ((BatchActionTaskConceptParentReplace) task).setMoveToDestUuid(uuidNewParent);
            return task;
        } else {
            BatchActionEventReporter.add(new BatchActionEvent(null, BatchActionTaskType.CONCEPT_PARENT_REPLACE,
