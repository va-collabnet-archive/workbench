...
/**
 * The Class UpdateQueueConfigFile.
 */
@BeanList(specs = 
{ @Spec(directory = "tasks/ide/issues", type = BeanType.TASK_BEAN)})
public class UpdateQueueConfigFile extends AbstractTask {
...
//		}
//
//		if (!configFile.getName().equals("queue.config")) {
//			throw new TaskFailedException("Only queue.config files must be selected...(" + 
//					configFile.getName() + ")");
//		}
				...
				while (( line = input.readLine()) != null){
					if (selectedWorker.equals("IssueRepositoryInboxWorker")) {
						if (line.trim().startsWith("org.dwfa.queue.bpa.worker.InboxQueueWorker")) {
							updatedConfig.append("org.ihtsdo.issue.integration.workers.IssueRepositoryInboxQueueWorker {");
							updatedConfig.append(System.getProperty("line.separator"));
							updatedConfig.append("issueRepoUid = \"" + repository.getUuid().toString() + "\";");
							updatedConfig.append(System.getProperty("line.separator"));
						} else if (line.trim().startsWith("username = \"**mailUsername**\";")){
							updatedConfig.append("username = \"" + Terms.get().getActiveAceFrameConfig().getUsername().trim() + "\";");
							updatedConfig.append(System.getProperty("line.separator"));
						} else {
						...
						if (line.trim().startsWith("workerSpecs = new QueueWorkerSpec")) {
							updatedConfig.append("new QueueWorkerSpec(org.ihtsdo.issue.integration.workers.IssueRepositoryInboxQueueWorker.class, \"Inbox Issue repository worker\", UUID.randomUUID(),new SelectAll())");
							updatedConfig.append(System.getProperty("line.separator"));
						} 
					} else if (selectedWorker.equals("IssueRepositoryOutboxWorker")) {
						if (line.trim().startsWith("org.dwfa.queue.bpa.worker.OutboxQueueWorker")) {
							updatedConfig.append("org.ihtsdo.issue.integration.workers.IssueRepositoryOutboxQueueWorker {");
							updatedConfig.append(System.getProperty("line.separator"));
							// Gets the repository from the issue
						} else if (line.trim().startsWith("workerSpecs = new QueueWorkerSpec")) {
							updatedConfig.append("workerSpecs = new QueueWorkerSpec[] { new QueueWorkerSpec(org.ihtsdo.issue.integration.workers.IssueRepositoryOutboxQueueWorker.class, \"Outbox Issue repository worker\", UUID.randomUUID(),");
							updatedConfig.append(System.getProperty("line.separator"));
						} else {
							updatedConfig.append(line);
							updatedConfig.append(System.getProperty("line.separator"));
