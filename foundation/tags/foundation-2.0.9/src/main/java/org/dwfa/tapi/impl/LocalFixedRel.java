...
public class LocalFixedRel implements I_RelateConceptsLocally {
	...
	private int nid, c1nid, relTypeNid, c2nid, characteristicNid,
			refinabilityNid, relGrp;
	...
	public LocalFixedRel(int nid, int c1nid, int relTypeNid, int c2nid,
			int characteristicNid, int refinabilityNid, int relGrp) {
		super();
		this.nid = nid;
		this.c1nid = c1nid;
	...
	public I_RelateConceptsUniversally universalize() throws IOException, TerminologyException {
		...
		return new UniversalFixedRel(sourceServer.getUids(nid), sourceServer
				.getUids(c1nid), sourceServer.getUids(relTypeNid), sourceServer
				.getUids(c2nid), sourceServer.getUids(characteristicNid),
	...
	public String toString() {
		...
		try {
			...
			e1.printStackTrace();
			return this.getClass().getSimpleName() + "nid: " + nid + " c1nid: "
			+ c1nid + " relTypeNid: " + relTypeNid + " c2nid: " + c2nid
			+ " characteristicNid: " + characteristicNid
			+ " refinabilityNid: " + refinabilityNid + " relGrp: " + relGrp;
		}
