...
public class RF2VTMImpl extends RF2AbstractImpl implements I_ProcessConcepts {

	/** The logger. */
	private static Logger logger = Logger.getLogger(RF2VTMImpl.class);

	/**
	 * Instantiates a new r f2 vtm impl.
	 *
	 * @param config the config
	 */
	public RF2VTMImpl(Config config) {
		super(config);
	}
	...
	public void export(I_GetConceptData concept, String conceptid) throws IOException {
		...
		try {
			String refsetId = I_Constants.VTM_REFSET_ID;
			String moduleId = "";

			int refsetTermAuxId = getNid(I_Constants.VTM_REFSET_UID_TERM_AUX);
...
//										moduleId=getModuleSCTIDForStampNid(intModuleId);
										...
										writeRF2TypeLine(refsetuuid, "", active, moduleId, refsetId, conceptid);
									}
								}
							}
						}
