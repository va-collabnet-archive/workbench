						...
						if(description.getStatusNid()==activeNid){
							...
							if (conceptAttributes != null && !conceptAttributes.isEmpty()) {
								...
								if (!modified){
									...
									String conceptEffectiveTime = getDateFormat().format(et);
									if (!(conceptEffectiveTime.compareTo(getConfig().getPreviousReleaseDate())>0) ||
											!(conceptEffectiveTime.compareTo(getConfig().getReleaseDate())<=0)){
										
										continue;
									}else{
										effectiveTime= conceptEffectiveTime;
									}
								}
