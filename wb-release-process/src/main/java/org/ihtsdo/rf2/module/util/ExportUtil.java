...
public class ExportUtil {
	...
	private static void CompleteMapSCTIDtoModule(Config config,HashMap<UUID, String> allModuleIds) {
		...
		String part="00";
		if (config!=null && config.getNamespaceId()!="0"){
			part="10";
		}
		for (UUID uuid:allModuleIds.keySet()){
			try {
				String moduleSCTId="";
				if (config!=null && config.getEndPoint()!=null && !config.getEndPoint().equals("")){
					moduleSCTId=getSCTId(config,uuid, Integer.parseInt(config.getNamespaceId()) , part, config.getReleaseId(),config.getExecutionId(), "");
				}else{
					moduleSCTId=getSctId(getTermFactory().uuidToNative(uuid));
				}
				if (moduleSCTId==null || moduleSCTId.equals("")){
					moduleSCTId=uuid.toString();
				}
