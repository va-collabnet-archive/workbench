	...
	try {
		...
		for (I_ExtendByRef extension : extensions) {
			if (extension.getRefsetId() == issueReposRefset.getConceptNid()) {
				...
				List<? extends I_ExtendByRefVersion> extTuples = extension.getTuples(config.getConflictResolutionStrategy());
				I_ExtendByRefPartStr originalPart = (I_ExtendByRefPartStr) extTuples.iterator().next().getMutablePart();
                                I_ExtendByRefPartStr part = (I_ExtendByRefPartStr) originalPart.makePromotionPart(config.getEditingPathSet().iterator().next(), 
		...
		for (I_ExtendByRef extension : extensions) {
			if (extension.getRefsetId() == conceptToRetire.getConceptNid()) {
				...
				for (PathBI editPath : config.getEditingPathSetReadOnly()) {
					...
					lastPart.makePromotionPart(editPath, config.getDbConfig().getUserConcept().getNid());
                                        part.setStatusNid(ArchitectonicAuxiliary.Concept.RETIRED.localize().getNid());
	...
	try {
		...
		//			Terms.get().commit();
                ...
                termComponent.promote(viewPosition, config.getPromotionPathSetReadOnly(), 
                        allowedStatusWithRetired, Precedence.TIME, config.getDbConfig().getUserConcept().getNid());
