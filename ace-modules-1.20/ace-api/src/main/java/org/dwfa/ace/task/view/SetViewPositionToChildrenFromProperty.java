...
public class SetViewPositionToChildrenFromProperty extends AbstractTask {
    ...
    public Condition evaluate(I_EncodeBusinessProcess process, I_Work worker) throws TaskFailedException {
        try {
            ...
            HashSet<I_Path> pathsToView = new HashSet<I_Path>();
            for (I_GetConceptData child : new LineageHelper().getAllDescendants(viewPathConcept)) {
                try {
                    pathsToView.add(tf.getPath(child.getUids()));
                } catch (PathNotExistsException e) {
                    // just skip if not a real path
                }
            }
            ...
            for (I_Path viewPath : pathsToView) {
