    ...
    public WorkbenchRunner(String[] args, LifeCycle lc) {
        try {
            ...
            if (acePropertiesFileExists == false || initialized == false) {
                try {
                    new SvnHelper(WorkbenchRunner.class, jiniConfig).initialSubversionOperationsAndChangeSetImport(wbPropertiesFile);
                } catch (Exception ex) {
            ...
            if ((profileDir.exists() == false && initializeFromSubversion) || (svnUpdateOnStart != null)) {
                new SvnHelper(WorkbenchRunner.class, jiniConfig).initialSubversionOperationsAndChangeSetImport(new File(
                    "config", WB_PROPERTIES));
            } else if (profileDir.exists()) {
                ArrayList<File> profileLoc = new ArrayList<File>();
                profileLoc.add(profileDir);
                new SvnHelper(WorkbenchRunner.class, jiniConfig).doChangeSetImport(profileLoc);
            }
            ...
            public void run() {
                try {
                    ...
                    if (ace.getViewPositionSet() == null || ace.getViewPositionSet().size() == 0) {
                    	...
                    	viewPositions.add(new Position(Integer.MAX_VALUE, 
                    			Bdb.getPathManager().get(ArchitectonicAuxiliary.Concept.SNOMED_CORE.localize().getNid())));
                    	viewPositions.add(new Position(Integer.MAX_VALUE, 
                    			Bdb.getPathManager().get(ArchitectonicAuxiliary.Concept.ARCHITECTONIC_BRANCH.localize().getNid())));
