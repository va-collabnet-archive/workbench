            ...
            for (Component c : workflowDetailsSheet.getComponents()) {
                if (PanelRefsetAndParameters.class.isAssignableFrom(c.getClass())) {
                    ...
                    if (reviewer != null) {
                        ...
                        // Determine the reviewer's inbox and save it in a property
                        String reviewerInboxAddress = wizard.getInbox(reviewer);
                        if (reviewerInboxAddress == null) {
                            JOptionPane.showMessageDialog(LogWithAlerts.getActiveFrame(null),
                                "Refresh Refset process cannot continue... The selected reviewer has no assigned inbox: "
                                    + reviewer, "", JOptionPane.ERROR_MESSAGE);
                            return Condition.ITEM_CANCELED;
                        } else {
                            process.setProperty(reviewerInboxPropName, reviewerInboxAddress);
                        }
                    }
