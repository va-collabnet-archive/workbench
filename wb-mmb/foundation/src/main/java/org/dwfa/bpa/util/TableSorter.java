...
public class TableSorter extends AbstractTableModel {
    ...
    private static Directive EMPTY_DIRECTIVE = new Directive(-1, SortOrder.NOT_SORTED);
    ...
    public void setSortingStatus(int column, SortOrder order) {
        Directive directive = getDirective(column);
        if (directive != EMPTY_DIRECTIVE) {
            sortingColumns.remove(directive);
        }
        if (order != SortOrder.NOT_SORTED) {
            sortingColumns.add(new Directive(column, order));
        }
    ...
    protected Icon getHeaderRendererIcon(int column, int size) {
        ...
        return new Arrow(directive.direction == SortOrder.DESCENDING, size, sortingColumns.indexOf(directive));
    }
    ...
    private class Row implements Comparable {
        ...
        public int compareTo(Object o) {
            ...
            for (Iterator<Directive> it = sortingColumns.iterator(); it.hasNext();) {
                ...
                if (comparison != 0) {
                    return directive.direction == SortOrder.DESCENDING ? -comparison : comparison;
                }
            }
            return 0;
        }
