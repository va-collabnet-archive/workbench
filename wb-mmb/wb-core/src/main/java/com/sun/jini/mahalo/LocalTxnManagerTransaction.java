...
/**
 * LocalTxnManagerTransaction is a class which captures the internal
 * representation of a transaction in the TxnManagerImpl server. This class is
 * associated with a transaction id. The information encapsulated includes the
 * list of participants which have joined the transaction, the state of the
 * transaction, the crash.
 * 
 * The user of a ParticipantHolder must make the association between an instance
 * of a ParticipantHolder and some sort of key or index.
 * 
 * @author Sun Microsystems, Inc.
 * 
 */
class LocalTxnManagerTransaction implements TransactionConstants, TimeConstants, LeasedResource {
    static final long serialVersionUID = -2088463193687796098L;

    /*
     * Table of valid state transitions which a TransactionManager may make.
     * 
     * This represents the following diagram from the Jini(TM) Transaction Spec:
     * 
     ...
     * ----------------------------------------------------------------------
     * ACTIVE true true false false false true VOTING false true false false
     * true true PREPARED false false false false false false NOTCHANGED false
     * false false false false false COMMITTED false false false false true
     * false ABORTED false false false false false true
     * 
     * The table is indexed using the ordered pair <current_state, next_state>.
     * A value of true means that the transition is possible, while a false
     * means that the transition is not possible.
     * 
     * Note: Some rows are "{false, false, false, false}" as unused filler to
     * account for the fact that the TransactionManager's valid states are a
     * subset of the TransactionConstants.
     * 
     * <zero> ACTIVE = 1 VOTING = 2 PREPARED = 3 NOTCHANGED = 4 COMMITTED = 5
     * ABORTED = 6
     */
     ...
     * @serial
     */
    private List<LocalParticipantHandle> parts = new Vector<LocalParticipantHandle>();
    ...
    /**
     * "Parallelizing" the interaction between the manager and participants
     * means using threads to interact with participants on behalf of the
     * manager. In the thread pool model, a TaskManager provides a finite set of
     * threads used to accomplish a variety of tasks. A Job encapsulates a body
     * of work which needs to be performed during the two-phase commit:
     * preparing, committing and aborting. Each work item is broken into smaller
     * pieces of work- interactions with a single participant- each assigned to
     * a task.
     * 
     * When a transaction is committing, a LocalPrepareJob is created and its
     * tasks
     * are scheduled. After completion, the LocalPrepareJob's outcome is
     * computed.
     * Depending on the outcome, either an LocalAbortJob or CommitJob is
     * scheduled.
     * When a transaction is aborted, an LocalAbortJob is scheduled.
     * 
     * A caller may specify a timeout value for commit/abort. The timeout
     * represents the length of time a caller is willing to wait for
     * participants to be instructed to roll-forward/back. Should this timeout
     * expire, a TimeoutExpiredException is thrown. This causes the caller's
     * thread to return back to the caller. Someone needs to finish contacting
     * all the participants. This is accomplished by the SettlerTask.
     * SettlerTasks must use a different thread pool from what is used by the
    ...
    /**
     * Interlock for the expiration time since lease renewal which set it may
     * compete against lease checks which read it.
     * 
     * @serial
     */
    ...
    /**
     * Interlock for Jobs is needed since many threads on behalf of many clients
     * can simultaneously access or modify the Job associated with this
     * transaction when when attempting to prepare, roll forward or roll back
     * participants.
     * 
     * @serial
     */
    ...
    /**
     * Interlock for transaction state needed since many threads on behalf of
     * many clients can simultaneously access or attempt to modify this
     * transaction's state as a side effect of calling commit or abort.
     * 
     * @serial
     */
     ...
     * 
     * @param mgr
     *            <code>TransactionManager</code> which owns this internal
     *            representation.
     * @param logmgr
     *            <code>LogManager</code> responsible for recording COMMITTED
     *            and ABORTED transactions to stable storage.
     * 
     * @param id
     *            The transaction id
     * 
     * @param threadpool
     *            The <code>TaskManager</code> which provides the pool of
     *            threads used to interact with participants.
     * 
     * @param settler
     *            TxnSettler responsible for this transaction if unsettled.
     */
    LocalTxnManagerTransaction(TransactionManager mgr, LogManager logmgr, long id, TaskManager threadpool,
            WakeupManager wm, TxnSettler settler, Uuid uuid) {
        if (logmgr == null)
    ...
    /**
     * Convenience method which adds a given <code>LocalParticipantHandle</code>
     ...
     *            The added handle
     */
    void add(LocalParticipantHandle handle) throws InternalManagerException {
        ...
        try {
            if (transactionsLogger.isLoggable(Level.FINEST)) {
                transactionsLogger.log(Level.FINEST, "Adding LocalParticipantHandle: {0}", handle);
            }
            parts.add(handle);
        } catch (Exception e) {
            if (transactionsLogger.isLoggable(Level.SEVERE)) {
                transactionsLogger.log(Level.SEVERE, "Unable to add LocalParticipantHandle", e);
            }
    ...
    /**
     * Convenience method which allows the caller to modify the prepState
     * associated with a given <code>LocalParticipantHandle</code>
     * 
     * @param handle
     *            The <code>LocalParticipantHandle</code> being modified
     * 
     * @param state
     *            The new prepstate
     */
    void modifyParticipant(LocalParticipantHandle handle, int state) {
        if (operationsLogger.isLoggable(Level.FINER)) {
            operationsLogger.entering(TxnManagerTransaction.class.getName(), "modifyParticipant",
                new Object[] { handle, new Integer(state) });
        }
        LocalParticipantHandle ph = null;

        if (handle == null)
            throw new NullPointerException("ParticipantHolder: " + "modifyParticipant: cannot modify null handle");

        if (parts.contains(ph))
            ph = (LocalParticipantHandle) parts.get(parts.indexOf(handle));
    ...
    /**
     * Changes the manager-side state of the transaction. This method makes only
     * valid state changes and informs the caller if the change was successful.
     * Calls to this method synchronize around the manager-side state variable
     * appropriately.
     * 
     * @param int
    ...
    boolean modifyTxnState(int state) {
        ...
        if (operationsLogger.isLoggable(Level.FINER)) {
            operationsLogger.exiting(TxnManagerTransaction.class.getName(), "modifyTxnState", Boolean.valueOf(result));
        }
        return result;
    }
     ...
     * 
     * @param part
     *            The joining <code>TransactionParticpant</code>
     * 
     * @param crashCount
     ...
     */
    public void join(TransactionParticipant preparedPart, long crashCount) throws CannotJoinException,
            CrashCountException, RemoteException {
        if (operationsLogger.isLoggable(Level.FINER)) {
            operationsLogger.entering(TxnManagerTransaction.class.getName(), "join",
                new Object[] { preparedPart, new Long(crashCount) });
        }
        ...
        // Create a LocalParticipantHandle for the new participant
        // and mark the transactional state as ACTIVE
        try {
            LocalParticipantHandle ph = new LocalParticipantHandle(preparedPart, crashCount);
            LocalParticipantHandle phtmp = (LocalParticipantHandle) ((parts.contains(ph))
                                                                                         ? parts.get(parts.indexOf(ph))
                                                                                         : null);

            if (transactionsLogger.isLoggable(Level.FINEST)) {
                transactionsLogger.log(Level.FINEST, "Retrieved LocalParticipantHandle: {0}", phtmp);
            }
            if (phtmp != null) {
                ...
                if (oldcount == crashCount) {
                    ...
                    throw new CrashCountException("TxnManagerTransaction: " + "join: old = " + oldcount + " new = "
                        + crashCount);
                }
            }
    ...
    /**
     * This method returns the state of the transaction. Since the purpose of
     * the set of ParticipantHolders is to associate a Transaction with a group
     * of participants joined the transaction, we would like to get the state of
     * the transaction associated with the aforementioned set.
     * 
     */
    public int getState() {
        if (operationsLogger.isLoggable(Level.FINER)) {
        ...
        synchronized (stateLock) {
            if (operationsLogger.isLoggable(Level.FINER)) {
                operationsLogger.exiting(TxnManagerTransaction.class.getName(), "getState", new Integer(trstate));
            }
            return trstate;
        }
    }
    ...
    /**
     * Commits the transaction. This initiates the two-phase commit protocol.
     * First, each
     * <code>net.jini.core.transaction.server.TransactionParticipant</code> in
     * the set of participants joined in the
     * <code>net.jini.core.transaction.server.Transaction</code> is instructed
     * to vote and the votes are tallied. This is the first phase (prepare
     * phase).
     * 
     * Depending on the outcome of the votes, the transaction is considered
     * committed or aborted. Once commit/abort status is known, the participants
     * are notified with a message to either roll-forward (commit case) or
     * roll-back (abort case). This is the roll-phase.
     * 
     * Since there may be a one-to-many relationship between a transaction and
     * its participants, <code>com.sun.jini.thread.TaskManager</code>s are
     * used as a generic mechanism to provide the threads needed to interact
    ...
    void commit(long waitFor) throws CannotCommitException, TimeoutExpiredException, RemoteException {
        ...
        try {
            ...
            if (transactionsLogger.isLoggable(Level.FINE)) {
                transactionsLogger.log(Level.FINE, "{0} TransactionParticipants have joined", new Integer(numparts));
            }
            ...
            // and schedule a Prepare or LocalPrepareAndCommitJob.
            // If the state transition is VOTING to VOTING,
            // then the LocalPrepareJob has already been created
            ...
            if (modifyTxnState(VOTING)) {
                ...
                if (oldstate == ACTIVE)
                    log.write(new LocalCommitRecord(phs));
                ...
                synchronized (jobLock) {
                    if (job == null) {
                        if (phs.length == 1) {
                            job = new LocalPrepareAndCommitJob(str, threadpool, wm, log, phs[0]);
                        } else {
                            job = new LocalPrepareJob(str, threadpool, wm, log, phs);
                        }
                        job.scheduleTasks();
                    }
                }

                // Wait for the LocalPrepareJob to complete.
                ...
                synchronized (jobLock) {
                    if ((job instanceof LocalPrepareJob) || (job instanceof LocalPrepareAndCommitJob)) {
                        try {
                            if (job.isCompleted(Long.MAX_VALUE)) {
                                ...
                                if (result.intValue() == ABORTED && job instanceof LocalPrepareAndCommitJob) {
            ...
            if (transactionsLogger.isLoggable(Level.FINE)) {
                transactionsLogger.log(Level.FINE, "Voting result: {0}", TxnConstants.getName(result.intValue()));
            }
            ...
            case ABORTED:
                ...
                if (alternateException == null) {
                    throw new CannotCommitException("Unable to commit transaction: " + getParticipantInfo());
                } else {
                    throw new RemoteException("Problem communicating with participant", alternateException);
                }

            case PREPARED:
                // This entrypoint is entered if a LocalPrepareJob
                ...
                if (modifyTxnState(COMMITTED)) {
                    ...
                    synchronized (jobLock) {
                        job = new LocalCommitJob(str, threadpool, wm, log, phs);
                        job.scheduleTasks();
                    }
                } else {
                    throw new CannotCommitException("attempt to commit " + "ABORTED transaction");
            ...
            case COMMITTED:
                ...
                //
                // b) The normal case where a LocalPrepareJob was found to
                ...
                //
                // c) A LocalPrepareAndCommitJob has successfully prepared
                ...
                synchronized (jobLock) {
                    // A LocalPrepareAndCommitJob is done at this
                    // point since the TransactionParticipant
                    // would have instructed itself to roll
                    // forward.

                    if (job instanceof LocalPrepareAndCommitJob) {
                        if (!modifyTxnState(COMMITTED))
                    ...
                    if (job instanceof LocalAbortJob)
                        throw new CannotCommitException("transaction " + "ABORTED");
                }

                if (getState() != COMMITTED)
                    throw new InternalManagerException("TxnManagerTransaction: " + "commit: " + job
                        + " got bad state: " + TxnConstants.getName(result.intValue()));
                ...
                try {
                    ...
                    synchronized (jobLock) {
                        if (remainder <= 0 || !job.isCompleted(remainder)) {
                            /*
                             * Note - SettlerTask will kick off another
                             * Commit/Abort task for the same txn which will try
                             * go through the VOTING->Commit states again.
                             */
                            // TODO - Kill off existing task? Postpone
                            // SettlerTask?
            ...
            default:
                throw new InternalManagerException("TxnManagerTransaction: " + "commit: " + job + " got bad state: "
    ...
    /**
     * Aborts the transaction. This method attempts to set the state of the
     * transaction to the ABORTED state. If successful, it then creates an
     * LocalAbortJob which schedules tasks executed on a thread pool. These
     * tasks
     * interact with the participants joined in this transaction to inform them
     * to roll back.
     * 
     * @param waitFor
     *            Timeout value which controls how long, the caller is willing
     *            to wait for the participants joined in the transaction to be
     *            instructed to roll-back.
     * 
     * @see com.sun.jini.mahalo.LocalAbortJob
    ...
    void abort(long waitFor) throws CannotAbortException, TimeoutExpiredException {
        ...
        try {
            ...
            //
            // If an abort is possible, and you find that an LocalAbortJob
            // is already in progress, let the existing LocalAbortJob
            // proceed.
            //
            // If an abort is possible, but a LocalPrepareJob is in progress,
            // go ahead an halt the LocalPrepareJob and replace it with
            // an LocalAbortJob.

            if (modifyTxnState(ABORTED)) {
                log.write(new LocalAbortRecord(phs));

                synchronized (jobLock) {
                    if (!(job instanceof LocalAbortJob)) {
                        if (job != null)
                            job.stop();
                        job = new LocalAbortJob(str, threadpool, wm, log, phs);
                        job.scheduleTasks();
                    }
                }
            } else {
            ...
            // This code waits for an LocalAbortJob to complete and
            // computes its resulting outcome. In addition,
            // the wait time is enforced. Should the wait time
            // expire, a SettlerTask is scheduled on a thread
            // pool. The SettlerTask is needed to complete
            ...
            if (!aborted)
                throw new InternalManagerException("TxnManagerTransaction: " + "abort: LocalAbortJob got bad state: "
                    + TxnConstants.getName(result.intValue()));
    ...
    public Transaction getTransaction() {
        ...
        if (operationsLogger.isLoggable(Level.FINER)) {
            operationsLogger.exiting(TxnManagerTransaction.class.getName(), "getTransaction", str);
        }
        return str;
    }
    ...
    public long getExpiration() {
        ...
        synchronized (leaseLock) {
            if (operationsLogger.isLoggable(Level.FINER)) {
                operationsLogger.exiting(TxnManagerTransaction.class.getName(), "getExpiration", new Date(expires));
            }
            return expires;
        }
    }
    ...
    public Uuid getCookie() {
        ...
        if (operationsLogger.isLoggable(Level.FINER)) {
            operationsLogger.exiting(TxnManagerTransaction.class.getName(), "getCookie", uuid);
        }
        return uuid;
    }
    ...
    synchronized boolean ensureCurrent() {
        ...
        if (useby > cur)
            result = true;
        if (operationsLogger.isLoggable(Level.FINER)) {
            operationsLogger.exiting(TxnManagerTransaction.class.getName(), "ensureCurrent", Boolean.valueOf(result));
        }
        return result;
    }

    private Vector<LocalParticipantHandle> parthandles() {
        if (operationsLogger.isLoggable(Level.FINER)) {
        ...
        Vector<LocalParticipantHandle> vect = new Vector<LocalParticipantHandle>(parts);
    ...
    private String getParticipantInfo() {
        ...
        if (transactionsLogger.isLoggable(Level.FINEST)) {
            transactionsLogger.log(Level.FINEST, "{0} participants joined", new Integer(parts.size()));
        }
        StringBuffer sb = new StringBuffer(parts.size() + " Participants: ");
        LocalParticipantHandle ph;
        for (int i = 0; i < parts.size(); i++) {
            ph = (LocalParticipantHandle) parts.get(i);
            sb.append("{" + i + ", " + ph.getPreParedParticipant().toString() + ", "
    ...
    void restoreTransientState(ProxyPreparer preparer) throws RemoteException {
        ...
        LocalParticipantHandle[] handles = (LocalParticipantHandle[]) parts.toArray(new LocalParticipantHandle[parts.size()]);
