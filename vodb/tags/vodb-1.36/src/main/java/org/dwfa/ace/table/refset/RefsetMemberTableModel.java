...
public class RefsetMemberTableModel extends AbstractTableModel implements
		PropertyChangeListener, I_HoldRefsetData, ActionListener {
	...
	public static class StringExtFieldEditor extends DefaultCellEditor {
		...
		public String getSelectedItem(Object value) {
			...
			switch (field) {
			case STRING_VALUE:
				return ((I_ThinExtByRefPartString) swet.getTuple().getPart())
						.getStringValue();
			default:
				throw new UnsupportedOperationException(
						"Can't do string combobox on " + field);
		...
		public ConceptBean getSelectedItem(Object value) {
			...
			case BOOLEAN_VALUE:
				throw new UnsupportedOperationException(
						"Can't do concept combobox on " + field);
			case CONCEPT_ID:
				return ConceptBean.get(((I_ThinExtByRefPartConcept) swet
						.getTuple().getPart()).getConceptId());
			case INTEGER_VALUE:
				throw new UnsupportedOperationException(
						"Can't do concept combobox on " + field);
			case ACCEPTABILITY:
				return ConceptBean.get(((I_ThinExtByRefPartLanguage) swet
						.getTuple().getPart()).getAcceptabilityId());
			case CORRECTNESS:
				return ConceptBean.get(((I_ThinExtByRefPartLanguage) swet
						.getTuple().getPart()).getCorrectnessId());
			case DEGREE_OF_SYNONYMY:
				return ConceptBean.get(((I_ThinExtByRefPartLanguage) swet
						.getTuple().getPart()).getDegreeOfSynonymyId());
			case TAG:
				return ConceptBean.get(((I_ThinExtByRefPartLanguageScoped) swet
						.getTuple().getPart()).getTagId());
			case SCOPE:
				return ConceptBean.get(((I_ThinExtByRefPartLanguageScoped) swet
						.getTuple().getPart()).getScopeId());
			case PRIORITY:
				throw new UnsupportedOperationException(
						"Can't do concept combobox on " + field);
			case MEASUREMENT_UNITS_ID:
				return ConceptBean.get(((I_ThinExtByRefPartMeasurement) swet
						.getTuple().getPart()).getUnitsOfMeasureId());
	...
	public enum REFSET_FIELDS {
		// All extensions
		REFSET_ID("refset", 5, 75, 1000), MEMBER_ID("member id", 5, 100, 100), COMPONENT_ID(
				"component id", 5, 100, 100), STATUS("status", 5, 50, 250), VERSION(
		...
		// Language extension
		ACCEPTABILITY("acceptability", 5, 125, 1000), CORRECTNESS(
				"correctness", 5, 125, 1000), DEGREE_OF_SYNONYMY("synonymy", 5,
				125, 1000),

		// Scoped language extension
		TAG("tag", 5, 100, 1000), SCOPE("scope", 5, 100, 1000), PRIORITY(
				"priority", 5, 50, 100),

		// Measurement extension
		MEASUREMENT_VALUE("measurement value", 75, 100, 1000), MEASUREMENT_UNITS_ID(
				"units of measure", 75, 100, 1000),

		// Template for rel extension
		T_VALUE_TYPE("value type", 5, 100, 1000), T_CARDINALITY("cardinality",
				5, 50, 100), T_SEMANTIC_STATUS("semantic status", 5, 100, 1000), T_BROWSE_ATTRIBUTE_ORDER(
				"attribute order", 5, 50, 100), T_BROWSE_VALUE_ORDER(
				"value order", 5, 50, 100), T_NOTES_SCREEN_ORDER(
				"screen order", 5, 50, 100), T_DISPLAY_STATUS("display status",
				5, 100, 1000), T_CHARACTERISTIC_STATUS("characteristic status",
				5, 100, 1000),
		...
		MAP_REFINABILITY("map refinability", 5, 100, 1000), MAP_ADDITIONAL_CODE(
				"additional code", 5, 100, 1000), MAP_ELEMENT_NO("element", 5,
				50, 100), MAP_BLOCK_NO("block", 5, 50, 100),

		// Cross mapping extension
		MAP_STATUS("map status", 5, 100, 1000), MAP_TARGET("target", 5, 100,
				1000)
	...
	private static REFSET_FIELDS[] booleanRefsetFields = new REFSET_FIELDS[] {
			...
			REFSET_FIELDS.BOOLEAN_VALUE, REFSET_FIELDS.STATUS,
	...
	private static REFSET_FIELDS[] stringRefsetFields = new REFSET_FIELDS[] {
			REFSET_FIELDS.REFSET_ID,
			// REFSET_FIELDS.MEMBER_ID, REFSET_FIELDS.COMPONENT_ID,
			REFSET_FIELDS.STRING_VALUE, REFSET_FIELDS.STATUS,
			REFSET_FIELDS.VERSION, REFSET_FIELDS.PATH };
	...
	private static REFSET_FIELDS[] conceptRefsetFields = new REFSET_FIELDS[] {
			REFSET_FIELDS.REFSET_ID,
			// REFSET_FIELDS.MEMBER_ID, REFSET_FIELDS.COMPONENT_ID,
			REFSET_FIELDS.CONCEPT_ID, REFSET_FIELDS.STATUS,
			REFSET_FIELDS.VERSION, REFSET_FIELDS.PATH };
	...
	private static REFSET_FIELDS[] conIntRefsetFields = new REFSET_FIELDS[] {
			REFSET_FIELDS.REFSET_ID,
			// REFSET_FIELDS.MEMBER_ID, REFSET_FIELDS.COMPONENT_ID,
			REFSET_FIELDS.CONCEPT_ID, REFSET_FIELDS.INTEGER_VALUE,
	...
	private static REFSET_FIELDS[] measurementRefsetFields = new REFSET_FIELDS[] {
			...
			REFSET_FIELDS.MEASUREMENT_UNITS_ID, REFSET_FIELDS.STATUS,
			REFSET_FIELDS.VERSION, REFSET_FIELDS.PATH };
	...
	private static REFSET_FIELDS[] measurementRefsetFieldsNoHistory = new REFSET_FIELDS[] {
			...
			// REFSET_FIELDS.MEMBER_ID, REFSET_FIELDS.COMPONENT_ID,
			REFSET_FIELDS.MEASUREMENT_VALUE,
			REFSET_FIELDS.MEASUREMENT_UNITS_ID, REFSET_FIELDS.STATUS, // REFSET_FIELDS.VERSION,
	// REFSET_FIELDS.BRANCH
	};
	...
	private static REFSET_FIELDS[] integerRefsetFields = new REFSET_FIELDS[] {
			REFSET_FIELDS.REFSET_ID,
			// REFSET_FIELDS.MEMBER_ID, REFSET_FIELDS.COMPONENT_ID,
			REFSET_FIELDS.INTEGER_VALUE, REFSET_FIELDS.STATUS,
			REFSET_FIELDS.VERSION, REFSET_FIELDS.PATH };
	...
	private static REFSET_FIELDS[] languageRefsetFields = new REFSET_FIELDS[] {
			REFSET_FIELDS.REFSET_ID,
			// REFSET_FIELDS.MEMBER_ID, REFSET_FIELDS.COMPONENT_ID,
			REFSET_FIELDS.ACCEPTABILITY, REFSET_FIELDS.CORRECTNESS,
			REFSET_FIELDS.DEGREE_OF_SYNONYMY, REFSET_FIELDS.STATUS,
			REFSET_FIELDS.VERSION, REFSET_FIELDS.PATH };
	...
	private static REFSET_FIELDS[] languageRefsetFieldsNoHistory = new REFSET_FIELDS[] {
			...
			// REFSET_FIELDS.MEMBER_ID, REFSET_FIELDS.COMPONENT_ID,
			REFSET_FIELDS.ACCEPTABILITY, REFSET_FIELDS.CORRECTNESS,
			REFSET_FIELDS.DEGREE_OF_SYNONYMY, REFSET_FIELDS.STATUS, // REFSET_FIELDS.VERSION,
	// REFSET_FIELDS.BRANCH
	};
	...
	private static REFSET_FIELDS[] scopedLanguageRefsetFields = new REFSET_FIELDS[] {
			...
			// REFSET_FIELDS.MEMBER_ID, REFSET_FIELDS.COMPONENT_ID,
			REFSET_FIELDS.ACCEPTABILITY, REFSET_FIELDS.CORRECTNESS,
			REFSET_FIELDS.DEGREE_OF_SYNONYMY, REFSET_FIELDS.TAG,
			REFSET_FIELDS.SCOPE, REFSET_FIELDS.PRIORITY, REFSET_FIELDS.STATUS,
			REFSET_FIELDS.VERSION, REFSET_FIELDS.PATH };
	...
	private static REFSET_FIELDS[] templateForRel = new REFSET_FIELDS[] {
			REFSET_FIELDS.REFSET_ID,
			// REFSET_FIELDS.MEMBER_ID, REFSET_FIELDS.COMPONENT_ID,
			REFSET_FIELDS.T_VALUE_TYPE, REFSET_FIELDS.T_CARDINALITY,
			REFSET_FIELDS.T_SEMANTIC_STATUS,
			REFSET_FIELDS.T_BROWSE_ATTRIBUTE_ORDER,
			REFSET_FIELDS.T_BROWSE_VALUE_ORDER,
			REFSET_FIELDS.T_NOTES_SCREEN_ORDER, REFSET_FIELDS.T_DISPLAY_STATUS,
			REFSET_FIELDS.T_CHARACTERISTIC_STATUS, REFSET_FIELDS.STATUS,
			REFSET_FIELDS.VERSION, REFSET_FIELDS.PATH
	...
	private static REFSET_FIELDS[] crossMapForRelFields = new REFSET_FIELDS[] {
			REFSET_FIELDS.REFSET_ID,
			// REFSET_FIELDS.MEMBER_ID, REFSET_FIELDS.COMPONENT_ID,
			REFSET_FIELDS.MAP_REFINABILITY, REFSET_FIELDS.MAP_ADDITIONAL_CODE,
			REFSET_FIELDS.MAP_ELEMENT_NO, REFSET_FIELDS.MAP_BLOCK_NO,
			REFSET_FIELDS.STATUS, REFSET_FIELDS.VERSION, REFSET_FIELDS.PATH };
	...
	private static REFSET_FIELDS[] crossMapFields = new REFSET_FIELDS[] {
			REFSET_FIELDS.REFSET_ID,
			// REFSET_FIELDS.MEMBER_ID, REFSET_FIELDS.COMPONENT_ID,
			REFSET_FIELDS.MAP_STATUS, REFSET_FIELDS.MAP_TARGET,
			REFSET_FIELDS.MAP_REFINABILITY, REFSET_FIELDS.MAP_ADDITIONAL_CODE,
			REFSET_FIELDS.MAP_ELEMENT_NO, REFSET_FIELDS.MAP_BLOCK_NO,
			REFSET_FIELDS.STATUS, REFSET_FIELDS.VERSION, REFSET_FIELDS.PATH
	...
	public static REFSET_FIELDS[] getRefsetColumns(I_HostConceptPlugins host,
			EXT_TYPE type) {
	...
	public RefsetMemberTableModel(I_HostConceptPlugins host,
			REFSET_FIELDS[] columns, EXT_TYPE refsetType,
	...
	public I_RefsetDefaults getRefsetPreferences() throws Exception {
		switch (refsetType) {
		case BOOLEAN:
			return host.getConfig().getRefsetPreferencesForToggle(toggle)
		...
		if (AceLog.getAppLog().isLoggable(Level.FINE)) {
			if (componentId == Integer.MIN_VALUE) {
				AceLog.getAppLog().fine(
						"Set component id to NULL for " + refsetType + " in "
								+ toggle);
			} else {
				AceLog.getAppLog().fine(
						"Set component id to: " + componentId + " for "
								+ refsetType + " in " + toggle);
			}
		}
		propertyChange(null);
	}
		...
		try {
			...
			case PATH:
				if (referencedConcepts.containsKey(tuple.getPathId())) {
					return new StringWithExtTuple(
							getPrefText(tuple.getPathId()), tuple);
				}
				return new StringWithExtTuple(Integer.toString(tuple
						.getPathId()), tuple);
				...
				if (ThinExtByRefPartConceptInt.class.isAssignableFrom(tuple
					...
					return new StringWithExtTuple(Integer
							.toString(((ThinExtByRefPartConceptInt) tuple
									.getPart()).getIntValue()), tuple);
				} else {
					return new StringWithExtTuple(Integer
							.toString(((I_ThinExtByRefPartInteger) tuple
									.getPart()).getValue()), tuple);
				}
				...
				if (referencedConcepts
					...
					return new StringWithExtTuple(
							getPrefText(((I_ThinExtByRefPartLanguage) tuple
									.getPart()).getAcceptabilityId()), tuple);
				}
				return new StringWithExtTuple(
			...
			case CORRECTNESS:
				if (referencedConcepts
					...
					return new StringWithExtTuple(
							getPrefText(((I_ThinExtByRefPartLanguage) tuple
									.getPart()).getCorrectnessId()), tuple);
				}
				return new StringWithExtTuple(
						Integer.toString(((I_ThinExtByRefPartLanguage) tuple
								.getPart()).getCorrectnessId()), tuple);
			case DEGREE_OF_SYNONYMY:
				if (referencedConcepts
					...
					return new StringWithExtTuple(
							getPrefText(((I_ThinExtByRefPartLanguage) tuple
									.getPart()).getDegreeOfSynonymyId()), tuple);
				}
				return new StringWithExtTuple(
						Integer.toString(((I_ThinExtByRefPartLanguage) tuple
								.getPart()).getDegreeOfSynonymyId()), tuple);
	...
	public void actionPerformed(ActionEvent arg0) {
		try {
			I_HoldRefsetPreferences preferences = host.getConfig()
			...
			default:
				throw new UnsupportedOperationException(
						"Can't handle ref set type: " + refsetType);
			}
			int refsetId = refsetDefaults.getDefaultRefset().getConceptId();
			int memberId = LocalVersionedTerminology.get()
					.uuidToNativeWithGeneration(
							UUID.randomUUID(),
							ArchitectonicAuxiliary.Concept.UNSPECIFIED_UUID
									.localize().getNid(),
							host.getConfig().getEditingPathSet(),
							Integer.MAX_VALUE);

			I_ThinExtByRefVersioned extension = new ThinExtByRefVersioned(
					refsetId, memberId, tableComponentId, ThinExtBinder
							.getExtensionTypeNid(refsetType));
			switch (refsetType) {
			case BOOLEAN:
			...
			case CON_INT:
				for (I_Path editPath : host.getConfig().getEditingPathSet()) {
					...
					conceptPart.setConceptId(preferences.getConIntPreferences()
							.getDefaultForConceptRefset().getConceptId());
					conceptPart.setIntValue(preferences.getConIntPreferences()
			...
			case MEASUREMENT:
				for (I_Path editPath : host.getConfig().getEditingPathSet()) {
					...
					measurementPart.setUnitsOfMeasureId(preferences
							...
							.getConceptId());
					measurementPart.setMeasurementValue(preferences
							.getMeasurementPreferences()
			...
			case LANGUAGE:
				for (I_Path editPath : host.getConfig().getEditingPathSet()) {
					...
					laguagePart.setAcceptabilityId(preferences
							...
							.getConceptId());
					laguagePart.setCorrectnessId(preferences
							.getLanguagePreferences()
							.getDefaultCorrectnessForLanguageRefset()
							.getConceptId());
					laguagePart.setDegreeOfSynonymyId(preferences
							.getLanguagePreferences()
			...
			case SCOPED_LANGUAGE:
				for (I_Path editPath : host.getConfig().getEditingPathSet()) {
					...
					scopedLaguagePart.setAcceptabilityId(preferences
							...
							.getDefaultAcceptabilityForLanguageRefset()
							.getConceptId());
					scopedLaguagePart.setCorrectnessId(preferences
							.getLanguageScopedPreferences()
							.getDefaultCorrectnessForLanguageRefset()
							.getConceptId());
					scopedLaguagePart.setDegreeOfSynonymyId(preferences
							.getLanguageScopedPreferences()
							.getDefaultDegreeOfSynonymyForLanguageRefset()
							.getConceptId());
					scopedLaguagePart.setScopeId(preferences
							.getLanguageScopedPreferences()
							.getDefaultScopeForScopedLanguageRefset()
							.getConceptId());
					scopedLaguagePart.setTagId(preferences
							.getLanguageScopedPreferences()
							.getDefaultTagForScopedLanguageRefset()
							.getConceptId());
					scopedLaguagePart.setPriority(preferences
							.getLanguageScopedPreferences()
			...
			case CROSS_MAP:
				for (I_Path editPath : host.getConfig().getEditingPathSet()) {
					...
					part.setAdditionalCodeId(preferences
							.getCrossMapPreferences().getAdditionalCode()
							.getConceptId());
					part.setBlockNo(preferences.getCrossMapPreferences()
							.getDefaultBlockNo());
					part.setElementNo(preferences.getCrossMapPreferences()
							.getDefaultElementNo());
					part.setRefineFlagId(preferences.getCrossMapPreferences()
							.getRefineFlag().getConceptId());
					if (host.getConfig().getHierarchySelection() != null) {
						part.setTargetCodeId(host.getConfig()
								.getHierarchySelection().getConceptId());
					} else {
						part
								.setTargetCodeId(ArchitectonicAuxiliary.Concept.ARCHITECTONIC_ROOT_CONCEPT
										.localize().getNid());
					}
					part.setMapStatusId(preferences.getCrossMapPreferences()
							.getMapStatus().getConceptId());
			...
			case CROSS_MAP_FOR_REL:
				for (I_Path editPath : host.getConfig().getEditingPathSet()) {
					...
					part.setVersion(Integer.MAX_VALUE);
					part.setAdditionalCodeId(preferences
							.getCrossMapForRelPreferences().getAdditionalCode()
							.getConceptId());
					part.setBlockNo(preferences.getCrossMapForRelPreferences()
							.getDefaultBlockNo());
					part.setElementNo(preferences
							.getCrossMapForRelPreferences()
							.getDefaultElementNo());
					part.setRefineFlagId(preferences
							.getCrossMapForRelPreferences().getRefineFlag()
							.getConceptId());
					extension.addVersion(part);
				}
				break;
			case TEMPLATE:
				for (I_Path editPath : host.getConfig().getEditingPathSet()) {
					...
					part.setAttributeDisplayStatusId(preferences
							.getTemplatePreferences()
							.getAttributeDisplayStatus().getConceptId());
					part
							.setBrowseAttributeOrder(preferences
									.getTemplatePreferences()
									.getBrowseAttributeOrder());
					part.setBrowseValueOrder(preferences
							.getTemplatePreferences().getBrowseValueOrder());
					part.setCardinality(preferences.getTemplatePreferences()
							.getCardinality());
					part.setCharacteristicStatusId(preferences
							.getTemplatePreferences().getCharacteristicStatus()
							.getConceptId());
					part.setNotesScreenOrder(preferences
							.getTemplatePreferences().getNotesScreenOrder());
					part.setSemanticStatusId(preferences
							.getTemplatePreferences().getSemanticStatus()
							.getConceptId());
					part.setValueTypeId(preferences.getTemplatePreferences()
							.getValueType().getConceptId());
					if (host.getConfig().getHierarchySelection() != null) {
						part.setTargetCodeId(host.getConfig()
								.getHierarchySelection().getConceptId());
					} else {
					...
					part.setAttributeId(preferences.getTemplatePreferences()
							.getAttribute().getConceptId());
					extension.addVersion(part);
				}
				break;
			case TEMPLATE_FOR_REL:
				for (I_Path editPath : host.getConfig().getEditingPathSet()) {
					...
					part.setAttributeDisplayStatusId(preferences
							.getTemplateForRelPreferences()
							.getAttributeDisplayStatus().getConceptId());
					part.setBrowseAttributeOrder(preferences
							.getTemplateForRelPreferences()
							.getBrowseAttributeOrder());
					part.setBrowseValueOrder(preferences
							.getTemplateForRelPreferences()
							.getBrowseValueOrder());
					part.setCardinality(preferences
							.getTemplateForRelPreferences().getCardinality());
					part.setCharacteristicStatusId(preferences
							.getTemplateForRelPreferences()
							.getCharacteristicStatus().getConceptId());
					part.setNotesScreenOrder(preferences
							.getTemplateForRelPreferences()
							.getNotesScreenOrder());
					part.setSemanticStatusId(preferences
							.getTemplateForRelPreferences().getSemanticStatus()
							.getConceptId());
					part.setValueTypeId(preferences
							.getTemplateForRelPreferences().getValueType()
							.getConceptId());
					extension.addVersion(part);
				}
				break;
			...
			ExtensionByReferenceBean ebrBean = ExtensionByReferenceBean
					.makeNew(extension.getMemberId(), extension);
	...
	public void setValueAt(Object value, int row, int col) {
		...
		if (extTuple.getVersion() == Integer.MAX_VALUE) {
			...
			case BOOLEAN_VALUE:
				...
				if (booleanValue != ((I_ThinExtByRefPartBoolean) extTuple
						.getPart()).getValue()) {
					((I_ThinExtByRefPartBoolean) extTuple.getPart())
			...
			case STRING_VALUE:
				...
				if (stringValue.equals(((I_ThinExtByRefPartString) extTuple
						.getPart()).getStringValue()) == false) {
					((I_ThinExtByRefPartString) extTuple.getPart())
			...
			case CONCEPT_ID:
				...
				((I_ThinExtByRefPartConcept) extTuple.getPart())
			...
			case INTEGER_VALUE:
				...
				if (I_ThinExtByRefPartConceptInt.class
						.isAssignableFrom(extTuple.getPart().getClass())) {
					((I_ThinExtByRefPartConceptInt) extTuple.getPart())
							.setIntValue(intValue);
				} else {
					((I_ThinExtByRefPartInteger) extTuple.getPart())
							.setValue(intValue);
				}
				changed = true;
				break;
			case ACCEPTABILITY:
				...
				((I_ThinExtByRefPartLanguage) extTuple.getPart())
						.setAcceptabilityId(acceptabilityId);
				referencedConcepts.put(acceptabilityId, ConceptBean
						.get(acceptabilityId));
				changed = true;
				break;
			case CORRECTNESS:
				...
				((I_ThinExtByRefPartLanguage) extTuple.getPart())
						.setCorrectnessId(correctnessId);
				referencedConcepts.put(correctnessId, ConceptBean
						.get(correctnessId));
				changed = true;
				break;
			case DEGREE_OF_SYNONYMY:
			...
			case MEASUREMENT_UNITS_ID:
				...
				((I_ThinExtByRefPartMeasurement) extTuple.getPart())
						.setUnitsOfMeasureId(unitsId);
	...
	public RefsetPopupListener makePopupListener(JTable table,
			I_ConfigAceFrame config) throws Exception {
		return new RefsetPopupListener(table, config, this
				.getRefsetPreferences(), this);
	}
