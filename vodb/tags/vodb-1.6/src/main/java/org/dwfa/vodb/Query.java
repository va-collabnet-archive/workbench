...
public class Query {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            doConceptQuery(vodb, descBinder, descForConceptKeyCreator,
                "SNOMED CT Concept");
            doConceptQuery(vodb, descBinder, descForConceptKeyCreator,
                "SNOMED CT Concept");
    ...
    private static void doQuery(VodbEnv vodb,
            ThinDescVersionedBinding descBinder, String queryString)
        ...
        while (descC.getNext(descKey, descValue, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
            ...
            I_DescriptionVersioned vdesc =
                    (I_DescriptionVersioned) descBinder
    ...
    private static void doQuickQuery(VodbEnv vodb,
            ThinDescVersionedBinding descBinder, String queryString)
            throws DatabaseException {
        Date oldStart = start;
        start = new Date();
        ...
        AceLog.getAppLog().info("------------------------------------------");
        AceLog.getAppLog().info(
            "Started quick query cursor for: " + queryString);
        DatabaseEntry descKey = new DatabaseEntry();
        DatabaseEntry descValue = new DatabaseEntry();
        while (descC.getNext(descKey, descValue, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
            descCount++;
