...
public class ExtensionByReferenceBean implements I_Transact, I_GetExtensionData {
	...
	public static ExtensionByReferenceBean get(int memberId) {
		...
		synchronized (ebrBeans) {
			if (ebrBeans.containsKey(memberId)) {
				Reference<ExtensionByReferenceBean> ref = ebrBeans
						.get(memberId);
				if (ref.isEnqueued()) {
			...
			ExtensionByReferenceBean ebrBean = new ExtensionByReferenceBean(
					memberId);
			if (refType == REF_TYPE.SOFT) {
				ebrBeans.put(memberId,
						new SoftReference<ExtensionByReferenceBean>(ebrBean));
			} else {
				ebrBeans.put(memberId,
						new WeakReference<ExtensionByReferenceBean>(ebrBean));
			}
			return ebrBean;
		}
	}

	public static I_GetExtensionData make(UUID uid,
			I_ThinExtByRefVersioned extension) throws TerminologyException,
			IOException {
		return make(AceConfig.getVodb().uuidToNative(uid), extension);
	}

	public static I_GetExtensionData makeNew(UUID uid,
			I_ThinExtByRefVersioned extension) throws TerminologyException,
			IOException {
		return makeNew(AceConfig.getVodb().uuidToNative(uid), extension);
	}

	public static ExtensionByReferenceBean makeNew(int memberId,
	...
	public static ExtensionByReferenceBean make(int memberId,
			I_ThinExtByRefVersioned extension) {
		Reference<ExtensionByReferenceBean> ref = ebrBeans.get(memberId);
	...
	public static I_GetExtensionData get(UUID uid) throws TerminologyException,
			IOException {
		return get(AceConfig.getVodb().uuidToNative(uid));
	}

	public static I_GetExtensionData get(Collection<UUID> uids)
	...
	public void commit(int version, Set<TimePathId> values) throws IOException {
		if (AceLog.getEditLog().isLoggable(Level.FINE)) {
			AceLog.getEditLog().fine(
					"Starting commit for ExtensionByReferenceBean: "
							+ this.memberId);
		}
		...
		if (extension != null) {
			for (I_ThinExtByRefPart p : extension.getVersions()) {
				...
				if (p.getVersion() == Integer.MAX_VALUE) {
					...
					if (buff != null) {
						buff.append("\n  Committing member: "
								+ extension.getMemberId() + " for component: "
		...
		if (AceLog.getAppLog().isLoggable(Level.FINE)) {
			AceLog.getAppLog().fine(
					"Finished commit for ExtensionByReferenceBean: " + this);
		}
		if (AceLog.getEditLog().isLoggable(Level.FINE)) {
	...
	public UniversalAceExtByRefBean getUniversalAceBean()
		...
		UniversalAceExtByRefBean uEbrBean = new UniversalAceExtByRefBean(tf
				.getUids(getExtension().getRefsetId()), tf
				.getUids(getExtension().getMemberId()), tf
				.getUids(getExtension().getComponentId()), tf
