...
public abstract class AceTableRenderer extends DefaultTableCellRenderer {
    ...
    protected void setBorder(int column, JLabel renderComponent, boolean same,
            boolean uncommitted) {
        if (!same) {
            if (uncommitted && column == 0) {
                renderComponent.setBorder(BorderFactory.createCompoundBorder(
                    BorderFactory.createMatteBorder(1, 0, 0, 0,
                        Color.LIGHT_GRAY), BorderFactory.createMatteBorder(0,
                        3, 0, 0, UNCOMMITTED_COLOR)));

            } else {
                renderComponent.setBorder(BorderFactory.createMatteBorder(1, 0,
    ...
    protected void setBackground(boolean isSelected, int row,
