...
//			new AceSvn(AceRunner.class, jiniConfig).handleSvnProfileCheckout(aceProperties);
			...
			if (aceConfigFile != null && aceConfigFile.exists() && acePropertiesFile.exists()) {
				...
				//		take the username and password from the first SVN entry - overwrite the profile username and password
				//		update profile svn entries with details from ace properties file
				//	end
				//end
				    ...
				    for(I_ConfigAceFrame configAceFrame : AceConfig.config.aceFrames) {
                        ...
                        if(!profileSubversionMap.isEmpty()){
                            ...
                            for(;propertyNames.hasMoreElements();){
                                ...
                                if(name.startsWith(SVN_PROPERTY_PREFIX)) {
                                    ...
                                    configAceFrame.getDbConfig().setChangeSetRoot(
                                        new File(aceProperties.getProperty(USER_CHANGE_SET_ROOT) + File.pathSeparator + username));
                                }
                            ...
                            if (!profileSubversionMap.values().equals(relativeSubversionMap.values())) {
                                AceLog.getAppLog().warning("Profile "
...
//	 */
//	private Map<String, SubversionData> getSubversionWorkingLocationMap(Collection<SubversionData> subversionDataList) {
//        Map<String, SubversionData> aceSvnData = new HashMap<String, SubversionData>();
//
