...
public class SvnPanel extends JPanel {
    ...
    private static void logDetails(SubversionData svd) throws ClientException {
        if (SvnLog.isLoggable(Level.FINE)) {
            SvnLog.fine("working copy Author: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr()).getAuthor());
            SvnLog
                .fine("working copy CopyRev: "
                    + Svn.getSvnClient().info(svd.getWorkingCopyStr())
                        .getCopyRev());
            SvnLog
                .fine("working copy CopyUrl: "
                    + Svn.getSvnClient().info(svd.getWorkingCopyStr())
                        .getCopyUrl());
            SvnLog.fine("working copy LastChangedRevision: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr())
                    .getLastChangedRevision());
            SvnLog.fine("working copy Name: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr()).getName());
            SvnLog.fine("working copy NodeKind: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr())
                    .getNodeKind());
            SvnLog.fine("working copy Repository: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr())
                    .getRepository());
            SvnLog.fine("working copy Revision: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr())
                    .getRevision());
            SvnLog.fine("working copy Schedule: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr())
                    .getSchedule());
            SvnLog.fine("working copy Url: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr()).getUrl());
            SvnLog.fine("working copy Uuid: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr()).getUuid());
            SvnLog.fine("working copy LastChangedDate: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr())
                    .getLastChangedDate());
            SvnLog.fine("working copy LastChangedRevision: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr())
                    .getLastChangedRevision());
            SvnLog.fine("working copy LastDatePropsUpdate: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr())
                    .getLastDatePropsUpdate());
            SvnLog.fine("working copy LastDateTextUpdate: "
                + Svn.getSvnClient().info(svd.getWorkingCopyStr())
