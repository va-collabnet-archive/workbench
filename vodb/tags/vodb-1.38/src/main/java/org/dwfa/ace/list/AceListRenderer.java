...
public class AceListRenderer extends DefaultListCellRenderer {
	...
	public Component getListCellRendererComponent(final JList list,
		...
		JLabel renderComponent = (JLabel) super.getListCellRendererComponent(
				list, "X", index, isSelected, cellHasFocus);

		if (value != null) {
			if (I_GetConceptData.class.isAssignableFrom(value.getClass())) {
				try {
					I_GetConceptData concept = (I_GetConceptData) value;
					I_DescriptionTuple desc = concept.getDescTuple(config
							.getShortLabelDescPreferenceList(), config);
					if (desc != null) {
						renderComponent.setText(desc.getText());
					} else {
						AceLog.getAppLog().info(
								"element: " + index + " descTuple is null: "
										+ concept.getConceptId());
						renderComponent
								.setText("removing concept with null descTuple: "
										+ concept.getConceptId());
