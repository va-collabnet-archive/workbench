...
public abstract class ProcessAceFormatSources extends ProcessSources {
    ...
    enum REFSET_FILE_TYPES {
        BOOLEAN("boolean.refset"), CONCEPT("concept.refset"), CONINT("conint.refset"), MEASUREMENT("measurement.refset"), INTEGER("integer.refset"), LANGUAGE("language.refset"), STRING("string.refset");
    ...
    protected static ExecutorService executors = Executors.newFixedThreadPool(8, new NamedThreadFactory(
        "import executor"));
    private static ExecutorService refsetExecutors = Executors.newFixedThreadPool(6, new NamedThreadFactory(
        "refset executor"));
        ...
        for (File releaseDir : constantDir.listFiles(new FilenameFilter() {
            ...
            for (File contentFile : releaseDir.listFiles(new FilenameFilter() {
                ...
                if (contentFile.getName().startsWith("concepts")) {
                    ...
                    getLog().info("(1) Don't know what to do with file: " + contentFile.getName());
                }
                br.close();
            }
        }
    ...
    public void executeFromDir(File dataDir, String encoding) throws Exception {
        ...
        File[] dataFiles = dataDir.listFiles(new FileFilter() {
            public boolean accept(File f) {
                return f.getName().endsWith(".txt")
                    && ((f.getName().equals("ids.txt") == false) && (f.getName().toLowerCase().contains("_report") == false));
            }
        });
        ...
        for (File dataFile : dataFiles) {
            ...
            if (dataFile.getName().contains("concepts")) {
                ...
                LoadDescriptionCallable descriptionLoader = new LoadDescriptionCallable(dataFile, encoding, format,
                    descriptionLatch);
                ...
                getLog().info("(2) Don't know what to do with file: " + dataFile.getName());
            }
        }
    ...
    protected abstract void finishRefsetRead(REFSET_FILE_TYPES refsetFileType, File rf, CountDownLatch refsetLatch)
            throws IOException, Exception;

    protected abstract void readConIntMember(StreamTokenizer st, UUID refsetUuid, UUID memberUuid, UUID statusUuid,
            UUID componentUuid, Date statusDate, UUID pathUuid) throws IOException, ParseException, DatabaseException,
            Exception;
    ...
    public void execute(File jarFile, String dataDir, FORMAT format) throws Exception {
        ...
        if (format == FORMAT.ACE) {
            ...
            // Need to depricate this method for processing SNOMED, and
            // inferring history.
            ...
            while (jarEnum.hasMoreElements()) {
                ...
                if (je.getName().startsWith(dataDir) && je.getName().endsWith(".txt")) {
                    ...
                    if (je.getName().contains("concepts")) {
                        InputStreamReader isr = new InputStreamReader(constantJar.getInputStream(je));
                        ...
                        getLog().info("Don't know what to do with jar entry: " + je.getName());
                    }
                }
                cleanupSNOMED(null);
            }
