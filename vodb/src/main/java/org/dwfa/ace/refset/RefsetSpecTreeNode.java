...
public class RefsetSpecTreeNode extends DefaultMutableTreeNode implements Comparable<RefsetSpecTreeNode> {
    ...
    public int getTruthId() {
        if (truthId == Integer.MAX_VALUE) {
            truthId = ((I_ThinExtByRefPartConceptConcept) getExtension().getMutablePart()).getC1id();
        }
        return truthId;
    }
    ...
    private int compareString(I_ThinExtByRefTuple thisExt, I_ThinExtByRefTuple otherExt) throws IOException {
        String thisExtStr = ((I_ThinExtByRefPartConceptConceptString) thisExt.getMutablePart()).getStr();
        String otherExtStr = ((I_ThinExtByRefPartConceptConceptString) otherExt.getMutablePart()).getStr();
