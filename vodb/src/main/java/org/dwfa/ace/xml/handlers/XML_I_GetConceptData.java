...
import org.dwfa.ace.xml.cache.ObjectCache;
import org.dwfa.ace.xml.common.CommonXMLStatics;
import org.dwfa.ace.xml.util.XMLUtil;
import org.w3c.dom.Document;
import org.w3c.dom.Element;

public class XML_I_GetConceptData extends XML_basic implements I_Handle_XML  {
	
	private static final Logger log = Logger.getLogger(I_Handle_XML.class.getName());
	
	/** A drill down depth for concepts
	If Set to -1 then infinite depth **/
	public int depth = 0;
	...
	public XML_I_GetConceptData(I_GetConceptData concept) {
		...
		try {
			...
			log.log(Level.SEVERE, "Err thrown in XML_I_GetConceptData process", e);
		};
	}
	...
	public void process() throws Exception{
		...
		if(ObjectCache.get(conIdi_S) == null){
			...
			if(concept.getConceptAttributes() != null){
				XML_I_ConceptAttributeVersioned icav_h = new XML_I_ConceptAttributeVersioned(concept.getConceptAttributes(),parent);
			}
			//Descriptions
			Element descsE = Doc.createElement(CommonXMLStatics.DESCRIPTIONS_ENAME);
			...
			//Src Rels
			Element srcRelsE = Doc.createElement(CommonXMLStatics.SRCRELS_ENAME);
			...
			//destRels
			Element destRelsE = Doc.createElement(CommonXMLStatics.DESTRELS_ENAME);
			...
			Element ExtE = Doc.createElement(CommonXMLStatics.EXTENSIONS_ENAME);
			 for (I_ThinExtByRefVersioned ext: concept.getExtensions()) {
				 new XML_I_ThinExtByRefVersioned(ext,ExtE);
			 }
			parent.appendChild(ExtE);
			} catch(Exception e)
			{ 
				System.out.println("Err thrown in XML_I_GetConceptData process");
				log.log(Level.SEVERE, "Err thrown in XML_I_GetConceptData process", e);
			e.printStackTrace();
			}
			...
			Element descs = Doc.createElement(CommonXMLStatics.DESCRIPTIONS_ENAME);
			...
			parent.appendChild(ImagesE);
			Element ExtE = Doc.createElement(CommonXMLStatics.EXTENSIONS_ENAME);	 
				 new XML_I_ThinExtByRefVersioned(debug,ExtE);
			parent.appendChild(ExtE);
