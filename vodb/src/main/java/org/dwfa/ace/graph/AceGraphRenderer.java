...
/**
 * <p>
 * A renderer with all sorts of buttons to press and dials to turn. Using the
 * appropriate methods, the user can override the default properties/behaviors
 * for vertex paint, stroke, shape, label, label font, and label centering; and
 * for edge paint, stroke, label, arrows, label font, label positioning, and
 * drawing.
 * </p>
 * <p>
 * Notes on these decorators:
 * <ul>
 * <li/>The decorators are all orthogonal; changing one does not change the
 * behavior of any other (unless your decorator implementations depend on one
 * another).
 * <li/>The default properties apply to each vertices/edges, but the decorators
 * allow these properties to be specified individually for each vertex/edge. See
 * the documentation for each of these decorators for specific instructions on
 * their use.
 * <li/>Implementations of these decorator interfaces are provided that allow
 * the user to specify a single (constant) property to apply to all
 * vertices/edges.
 * <li/>There are additional interfaces and classes that allow the size and
 * aspect ratio of the vertex shape to be independently manipulated, and that
 * provide factory methods for generating various standard shapes; see
 * <code>SettableVertexShapeFunction</code>,
 * <code>AbstractVertexShapeFunction</code>, <code>VertexShapeFactory</code>,
 * and the sample <code>samples.graph.PluggableRendererDemo</code>.
 * <li/>This class provides default <code>Stroke</code> implementations for
 * dotted and dashed lines: the <code>DOTTED</code> and <code>DASHED</code>
 * static constants, respectively.
 ...
 * vertex <code>v</code> is not to be drawn, none of its incident edges will be
 * drawn either.
 * </ul>
 * 
 * <p>
 ...
 * 
 * <p>
 * By default, undirected edges are drawn as straight lines, directed edges are
 * drawn as bent lines, and parallel edges are drawn on top of one another.
 * </p>
 * 
 * <p>
 * Arrowheads are drawn so that the point of the arrow is at the boundary of the
 * vertex shapes. In the current implementation, finding the place where arrows
 * are drawn is fairly slow; for large graphs, this should be turned off.
 * </p>
 * 
 * <p>
 * Setting a stroke width other than 1, or using transparency, may slow down
...
public class AceGraphRenderer extends AbstractRenderer implements PickedInfo, HasShapeFunctions {
    ...
    /**
     * A stroke for a dotted line: 1 pixel width, round caps, round joins, and
     * an
     * array of {1.0f, 3.0f}.
     */
    public final static Stroke DOTTED = new BasicStroke(1.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 1.0f,
        dotting, 0f);

    protected final static float[] dashing = { 5.0f };
    /**
     * A stroke for a dashed line: 1 pixel width, square caps, beveled joins,
     * and an
     * array of {5.0f}.
     */
    public final static Stroke DASHED = new BasicStroke(1.0f, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_BEVEL, 1.0f,
        dashing, 0f);
    ...
    /**
     * Specifies the maximum number of iterations to run the edge subdivision
     * loop
    ...
    protected VertexShapeFunction vertexShapeFunction = new EllipseVertexShapeFunction(new ConstantVertexSizeFunction(
        20), new ConstantVertexAspectRatioFunction(1.0f));
    protected VertexStringer vertexStringer = new ConstantVertexStringer(null);
    protected VertexIconFunction vertexIconFunction;
    protected VertexFontFunction vertexFontFunction = new ConstantVertexFontFunction(new Font("Helvetica", Font.PLAIN,
        12));
    protected boolean centerVertexLabel = false;

    protected VertexPaintFunction vertexPaintFunction = new PickableVertexPaintFunction(this, Color.BLACK, Color.RED,
        Color.ORANGE);
    ...
    protected EdgeFontFunction edgeFontFunction = new ConstantEdgeFontFunction(new Font("Helvetica", Font.PLAIN, 12));
     ...
     */
    protected GraphLabelRenderer graphLabelRenderer = new AceGraphLabelRenderer(Color.blue, Color.cyan);
     ...
     * for any edge for which the edge arrow <code>Predicate</code> returns
     * <code>true</code>. <br>
     * Default: wedge arrows for undirected edges, notched arrows for directed
     * edges
     ...
     * for that edge. <br>
     * Default: only directed edges have arrows (
     * <code>Graph.DIRECTED_EDGE</code> instance)
     * 
     ...
     * emphasize (or to de-emphasize) edges that have a specific property. <br>
     ...
     * or everything except self-loops). <br>
     * Default: all edges drawn (<code>TruePredicate</code> instance)
     * 
     ...
     * is undefined. This function is not used for self-loops. <br>
     * Default: 0.5 for undirected edges, 0.75 for directed edges
     ...
     * <code>Stroke</code> to use when drawing each edge. <br>
    ...
    /**
     * <p>
     * Sets the <code>VertexPaintFunction</code> to the parameter
     * 
     * @see #setVertexPaintFunction(VertexPaintFunction)
     * @see VertexColorFunction
     * @see VertexColorToVertexPaintConverter
     * @deprecated Use setVertexPaintFunction with a VertexPaintFunction if you
     *             can
     */
    ...
    /**
     * <p>
     * Sets the <code>VertexPaintFunction</code> which specifies the draw
     * (border and text) and fill (interior) Paint for each vertex.
     * </p>
     * <p>
     * If users want the <code>VertexPaintFunction</code> implementation to
     * highlight selected vertices, they should take this PluggableRenderer
     * instance as a constructor parameter, and call the <code>isPicked</code>
     ...
     * emphasize (or to de-emphasize) vertices that have a specific property. <br>
     * Default: 12-point Helvetica
     * 
     ...
     * specific property, such as a particular decoration or value. <br>
     * Default: all vertices drawn (<code>TruePredicate</code> instance)
     * 
     * @see org.apache.commons.collections.Predicate
     */
     ...
     * position (<code>true</code>) or offset to one side (<code>false</code>). <br>
     * Default: offset
     */
     ...
     * at the <code>SettableVertexShapeFunction</code> interface and the
     * <code>AbstractVertexShapeFunction</code> abstract class. <br>
     * Default: 8-pixel-diameter circle
    ...
    public Component prepareRenderer(GraphLabelRenderer graphLabelRenderer, Object value, boolean isSelected,
            Vertex vertex) {
    ...
    public Component prepareRenderer(GraphLabelRenderer renderer, Object value, boolean isSelected, Edge edge) {
        return graphLabelRenderer.getGraphLabelRendererComponent(screenDevice, value, edgeFontFunction.getFont(edge),
            isSelected, edge);
    }

    /**
     * Labels the specified non-self-loop edge with the specified label.
     * Uses the font specified by this instance's <code>EdgeFontFunction</code>.
    ...
    public void paintIconForVertex(Graphics g, Vertex v, int x, int y) {
        ...
        if (icon == null) {
            Shape s = AffineTransform.getTranslateInstance(x, y).createTransformedShape(
    ...
    protected void labelVertex(Graphics g, Vertex v, String label, int x, int y) {
        ...
        rendererPane.paintComponent(g, component, screenDevice, x + h_offset, y + v_offset, d.width, d.height, true);
