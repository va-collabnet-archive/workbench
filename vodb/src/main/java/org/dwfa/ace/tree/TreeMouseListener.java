...
public class TreeMouseListener implements MouseListener {
    ...
    private JPopupMenu makePopup(MouseEvent e, I_GetConceptData selectedConcept) throws Exception {
        ...
        ProcessPopupUtil.addSubMenuItems(popup, new File(AceFrame.pluginRoot, "taxonomy"), ace.getAceFrameConfig());

        String userDir = ace.getAceFrameConfig().getDbConfig().getProfileFile().getParentFile().getCanonicalPath();
        JMenu userMenu = new JMenu("User Defined");
        ProcessPopupUtil.addSubMenuItems(userMenu, new File(userDir, "process"), ace.getAceFrameConfig());
    ...
    private void addRefsetItems(JPopupMenu popup, File directory, I_ThinExtByRefVersioned specPart)
            throws TerminologyException {
        ProcessPopupUtil.addSubMenuItems(popup, directory, ace.getAceFrameConfig().getWorker(), null);
    }
        ...
        if (addNodes) {
            ...
            try {
                tuples = treeBean.getSourceRelTuples(ace.getAceFrameConfig().getAllowedStatus(),
                    ace.getAceFrameConfig().getDestRelTypes(), ace.getAceFrameConfig().getViewPositionSet(), false);
