...
public class RefreshSpecClausePanel extends JPanel implements ActionListener {
	...
	private JComboBox updateOptions = new JComboBox(new String[] {
			REPLACE_OPTION, RETIRE_OPTION, SKIP_OPTION });
	...
	public RefreshSpecClausePanel(I_GetConceptData refsetIdentityConcept,
		...
		for (I_ThinExtByRefTuple tuple: tuples) {
			if (tuple.getTypeId() == RefsetAuxiliary.Concept.CONCEPT_CONCEPT_EXTENSION.localize().getNid()) {
				...
				I_ThinExtByRefPartConceptConceptConcept cccPart = (I_ThinExtByRefPartConceptConceptConcept) tuple.getMutablePart();
