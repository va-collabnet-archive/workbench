...
public class PropBasedTransWorkFlowStep4Outputs extends AbstractTask {
	...
	private String exit4PropName;

	/** The exit4 label. */
	private String exit4Label;

	/** The exit4 destination. */
	private String exit4Destination;
	...
	private void writeObject(ObjectOutputStream out) throws IOException {
		...
		out.writeObject(exit4PropName);
		out.writeObject(exit4Label);
	...
	ClassNotFoundException {
		...
		if (objDataVersion == 1) {
			...
			exit4PropName = (String) in.readObject();
			exit4Label = (String) in.readObject();
	 ...
	 */
	public PropBasedTransWorkFlowStep4Outputs() throws MalformedURLException {
		super();
	}
...
//				process.setProperty(getProfilePropName(), config);
			...
			SwingUtilities.invokeAndWait(new Runnable() {
				...
				public void run() {
					...
					exit4Button.setText(exit4Label);
					try {
						exit4Destination = (String) process.getProperty(getExit4PropName());
					} catch (IllegalArgumentException e) {
						e.printStackTrace();
					} catch (IntrospectionException e) {
						e.printStackTrace();
					...
					//---- exit6Button ----
					JButton exit6Button=new JButton();
					exit6Button.setText("Save as To Do");
					exit6Button.addActionListener(new Exit6ActionListener());
	...
	private class Exit1ActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (uiPanel.verifySavePending(null,LanguageUtil.isItemBeingSent(e))) {
				...
				synchronized (PropBasedTransWorkFlowStep4Outputs.this) {
					PropBasedTransWorkFlowStep4Outputs.this.notifyAll();
				}
			}
		}
	}
	...
	/**
	 * The listener interface for receiving exit5Action events.
	 * The class that is interested in processing a exit5Action
	 * event implements this interface, and the object created
	 * with that class is registered with a component using the
	 * component's <code>addExit5ActionListener<code> method. When
	 * the exit5Action event occurs, that object's appropriate
	 * method is invoked.
	 *
	 * @see Exit5ActionEvent
	 */
	...
	/**
	 * The listener interface for receiving exit6Action events.
	 * The class that is interested in processing a exit6Action
	 * event implements this interface, and the object created
	 * with that class is registered with a component using the
	 * component's <code>addExit6ActionListener<code> method. When
	 * the exit6Action event occurs, that object's appropriate
	 * method is invoked.
	 *
	 * @see Exit6ActionEvent
	 */
	private class Exit6ActionListener implements ActionListener {
	...
	/**
	 * Gets the exit3 prop name.
	 *
	 * @return the exit3 prop name
	 */
	...
	/**
	 * Sets the exit3 prop name.
	 *
	 * @param exit3PropName the new exit3 prop name
	 */
	...
	/**
	 * Gets the exit3 label.
	 *
	 * @return the exit3 label
	 */
	...
	/**
	 * Sets the exit3 label.
	 *
	 * @param exit3Label the new exit3 label
	 */
	...
	/**
	 * Gets the exit3 destination.
	 *
	 * @return the exit3 destination
	 */
	...
	/**
	 * Sets the exit3 destination.
	 *
	 * @param exit3Destination the new exit3 destination
	 */
	...
	/**
	 * Gets the exit4 prop name.
	 *
	 * @return the exit4 prop name
	 */
	public String getExit4PropName() {
		return exit4PropName;
	}

	/**
	 * Sets the exit4 prop name.
	 *
	 * @param exit4PropName the new exit4 prop name
	 */
	public void setExit4PropName(String exit4PropName) {
		this.exit4PropName = exit4PropName;
	}

	/**
	 * Gets the exit4 label.
	 *
	 * @return the exit4 label
	 */
	public String getExit4Label() {
		return exit4Label;
	}

	/**
	 * Sets the exit4 label.
	 *
	 * @param exit4Label the new exit4 label
	 */
	public void setExit4Label(String exit4Label) {
		this.exit4Label = exit4Label;
	}

	/**
	 * Gets the exit4 destination.
	 *
	 * @return the exit4 destination
	 */
	public String getExit4Destination() {
		return exit4Destination;
	}

	/**
	 * Sets the exit4 destination.
	 *
	 * @param exit4Destination the new exit4 destination
	 */
	public void setExit4Destination(String exit4Destination) {
		this.exit4Destination = exit4Destination;
	}
