...
/**
 * The Class ListContextualizedDescriptionsToOutput.
 */
@BeanList(specs = 
{ @Spec(directory = "tasks/translation tasks", type = BeanType.TASK_BEAN)})
public class ListContextualizedDescriptionsToOutput extends AbstractTask {
	...
	throws TaskFailedException {
		...
		try{
			...
			List<ContextualizedDescription> descriptions = LanguageUtil.getContextualizedDescriptions(conceptConcept.getConceptNid(), 
					languageRefsetConcept.getConceptNid(), true);
			...
			for (I_ContextualizeDescription description : descriptions) {
				if (description.getLanguageExtension() != null) {
					...
					System.out.println("~~~~~~~~~~~~~~~ " + description.toString() + " - Acceptability: " + 
							acceptabilityConcept.toString());
				} else {
					System.out.println("~~~~~~~~~~~~~~~ " + description.toString() + " - No Acceptability");
				}
			}
