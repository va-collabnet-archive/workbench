					...
					while (possibleItr.next()) {
						...
						for (I_DescriptionVersioned desc:descs){
							for (RefexChronicleBI refDesc:desc.getAnnotations()){
								if (reqProjForConcept.containsKey(refDesc.getCollectionNid())){
									continue;
								}
								if (targetLangProjs.containsKey(refDesc.getCollectionNid())){
									...
									List<Integer> wSetList = tgtLangWSets.get(refDesc.getCollectionNid());
									isInWorkset=false;
									if (wSetList!=null){
										for (RefexChronicleBI refConc:concept.getAnnotations()){
											if (wSetList.contains(refConc.getCollectionNid())){
												isInWorkset=true;
												break;
											}
										}
									...
									if (!isInWorkset){
										reqProjForConcept.put(refDesc.getCollectionNid(), targetLangProjs.get(refDesc.getCollectionNid()));
									}
								}
							}
						}
					}
					if (reqProjForConcept.size()>0){
