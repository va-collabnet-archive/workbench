...
/**
 * The Class TransformersConfigApi.<br> This class provides an simplified access
 * to the XML config files used to define the transformation parameters.<p>
 * Example:<br>
 * {@code
 * <config>
 * <transformer>
 ...
 *</config>
 * }
 * In the example above the transformer preferences are stored within a 'config'
 * tag, each transformer has an id, a reference to the class fully qualified
 * name and a set of parameters.<br> The refset key contains a concept
 * descriptor as values, a reference to both the UUID and a description of a
 * concept. And the scalar key contains a string value.
 */
public class TransformersConfigApi {
    ...
    /**
     * Gets a concept descriptor from a transformer's preferences, identified by
     * the index and the preference key (path).
     *
    ...
    /**
     * Gets the String value for transformers key, identified by the index and
    ...
    /**
     * Gets the collection value for transformers key, identified by the index
     * and the preference key (path).
     *
     * @param intId the int id
     * @param path the path
