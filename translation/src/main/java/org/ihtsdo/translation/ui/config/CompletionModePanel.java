...
/**
 * The Class CompletionModePanel.
 * 
 * @author Guillermo Reynoso
 */
public class CompletionModePanel extends JPanel {
	 ...
	 *            the conf trans2
	 */
	public CompletionModePanel(I_ConfigAceFrame config, ConfigTranslationModule confTrans2) {
		super();
		initComponents();
	...
	private void initCustomComponents() {
		...
		CompletionMode[] completionModes = CompletionMode.values();
		List<CompletionModeRadioButton> radioButtons = new ArrayList<CompletionModeRadioButton>();
		...
		for (final CompletionMode loopMode : completionModes) {
			CompletionModeRadioButton button = new CompletionModeRadioButton();
			button.setText(loopMode.toString());
			button.setCompletionMode(loopMode);
			radioButtons.add(button);
			button.addChangeListener(cl);
			// button.addActionListener(radioList);
			if (confTrans != null && confTrans.getCompletionMode() != null && confTrans.getCompletionMode() == loopMode) {
		...
		int buttonNum = 1;
		for (CompletionModeRadioButton jRadioButton : radioButtons) {
		...
		applyButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				while (buttons.hasMoreElements()) {
					CompletionModeRadioButton button = (CompletionModeRadioButton) buttons.nextElement();
					if (button.isSelected()) {
						confTrans.setCompletionMode(button.getCompletionMode());
						setButtonsEnabled(false);
					}
				}
			}
	...
	public void selectCurrentConfButton() {
		...
		if (confTrans != null) {
			while (buttons.hasMoreElements()) {
				CompletionModeRadioButton button = (CompletionModeRadioButton) buttons.nextElement();
				if (button.getCompletionMode() == confTrans.getCompletionMode()) {
					button.setSelected(true);
				}
			}
		}
	...
	/**
	 * The Class CompletionModeRadioButton.
	 */
	private class CompletionModeRadioButton extends JRadioButton {
		 ...
		 *            the new ics generation strategy
		 */
