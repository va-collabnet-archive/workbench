...
public class GenericWorker extends Worker implements Runnable {
     ...
     */
    public GenericWorker(Configuration config, UUID id, String desc, InetAddress host,
            JavaSpace05 space, WorkerTableModel model, int tableRow) throws ConfigurationException, LoginException, IOException, PrivilegedActionException {
     ...
     * @see org.dwfa.bpa.process.I_Work#createWorkspace(UUID,
     *      java.lang.String, org.dwfa.bpa.gui.TerminologyConfiguration)
     */
    public I_Workspace createWorkspace(UUID workspaceId, String title, File menuDir) 
    throws WorkspaceActiveException {
        throw new UnsupportedOperationException();
    }
    
    public I_Workspace createWorkspace(UUID workspaceId, String title, I_ManageUserTransactions transactionInterface, File menuDir) 
    throws WorkspaceActiveException {
        throw new UnsupportedOperationException();
    }
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                GenericWorkerEntry gwe = new GenericWorkerEntry(this.getDescWithHost(),
                        this.id);
                Entry e = space.readIfExists(gwe, null, 1000 * 60);
                if (e == null) {
                    //If not, then add one...
                    Lease lease = space.write(gwe, null, 1000 * 60);
                    this.renewFor(lease, Lease.FOREVER,
                            1000 * 60, null);
                    if (logger.isLoggable(Level.INFO)) {
                        logger.info(this.getWorkerDesc()
                                + " recorded in space. " + gwe);
                ...
                while (true) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            this.currentTask = (GenericTaskEntry) space.take(
                                taskTemplate, t, Long.MAX_VALUE);
                            this.updateTableCell(WorkerTableModel.TASK);
                        } catch (TransactionException ex) {
                            if (ex.getMessage().startsWith("completed while operation in progress")) {
                                if (logger.isLoggable(Level.FINE)) {
                                    logger.fine(this.getWorkerDesc()
                                            + " exception: " + ex.getClass().getName() + ": " + ex.getMessage() + " (system should self heal)");
                                }
                        ...
                        if (this.currentTask != null) {
                            ...
                            if (logger.isLoggable(Level.FINE)) {
                                logger.fine(this.getWorkerDesc()
                                        + " executed task: " + this.currentTask + ", with result: " + result);
                            }
                            if (result != null) {
                ...
                logger.log(Level.INFO, this.getWorkerDesc() + " exception: " + ex.getClass().getName() + ": " + ex.getMessage() + " (system should self heal)");
    ...
    public String getDescWithHost() {
        return this.getWorkerDesc() + " " + " host: "
        + host.getHostName() + " ip: " + host;   
    }
	/**
