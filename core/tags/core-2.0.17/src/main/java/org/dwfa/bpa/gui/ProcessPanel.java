...
public class ProcessPanel extends JPanel implements PropertyChangeListener {
	...
	private JComboBox viewCombo = new JComboBox(new Object[] { VIEW_HISTORY,
			VIEW_TASKS, VIEW_MESSAGE, VIEW_DOCUMENTATION });
	 ...
	 * @throws Exception
	 */
	public ProcessPanel(I_EncodeBusinessProcess process, I_Work worker, I_HandleDoubleClickInTaskProcess doubleClickHandler)
		...
		addEmptyAttachmentKey
				.addActionListener(new AddEmptyAttachmentActionListener());
	...
	private void setTopPanel() throws NoSuchMethodException,
		...
		if (viewAttachments.isSelected()) {
			...
			attachmentSortingTable.setTableHeader(attachmentTable
					.getTableHeader());
	...
	private void setBottomPanel() throws PropertyVetoException, Exception {
		...
		if (viewString.equals(VIEW_HISTORY)) {
			...
			executionSortingTable.setTableHeader(executionTable
					.getTableHeader());
			...
			ProcessDiagramPanel processDiagram = new ProcessDiagramPanel(
					this.process, worker, doubleClickHandler);
			...
			processDocEditor.addPropertyChangeListener(new PropertyListenerGlue("setProcessDocumentationSource", String.class, process));
			...
			intermediary.setBorder(BorderFactory
					.createTitledBorder("Process documentation:"));
			headerProcessSplit.setBottomComponent(intermediary);
		}
	}
