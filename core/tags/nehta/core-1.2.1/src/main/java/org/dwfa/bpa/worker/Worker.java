...
public abstract class Worker implements I_Work {
	...
	public static DateFormat dateFormat = AceDateFormat.getTableDisplayDateFormat();
   ...
   public Worker(Configuration config, UUID id, String desc)
		...
		workerList.add(this);
		try {
			this.loginContext = (LoginContext) config.getEntry(this.getClass()
					.getName(), "loginContext", LoginContext.class);
			loginContext.login();
			logger.info("Worker " + this.desc + "(" + this.id + ") logged in. "
			...
			logger.info("Worker " + this.desc + "(" + this.id
					+ ") has null loginContext. " + this.getClass().getName()
					+ "\n" + e.toString());
	...
	protected void executeStartupProcesses() {
		try {
			File startupDirectory = (File) config.getEntry(this.getClass()
					.getName(), "startupDirectory", File.class);
			File[] startupFiles = startupDirectory
	 ...
	 * @throws IOException
	 */
	private void initAsSubject(Configuration config)
	...
	public synchronized Condition execute(I_EncodeBusinessProcess process)
		...
		try {
			if (this.loginContext != null) {
				ExecuteAsPriviledgedAction action = new ExecuteAsPriviledgedAction(
						process);
				try {
	 ...
	 * 
	 */
	public void commitTransactionIfActive() throws UnknownTransactionException,
	...
	public Transaction getActiveTransaction() throws LeaseDeniedException,
		...
		this.doAsPrivileged(new PrivilegedExceptionAction<Object>() {
			...
			public Object run() throws LeaseDeniedException, RemoteException,
				...
				Transaction t = Worker.this.jiniManager
						.createTransaction(Worker.this.transactionDuration);
	...
	public void setActiveTransaction(Transaction t) {
		...
		this.propChangeSupport.firePropertyChange("activeTransaction",
				oldValue, this.activeTransaction);
	}

	public void commitActiveTransaction() throws UnknownTransactionException,
			CannotCommitException, RemoteException {
		if (this.activeTransaction == null) {
			throw new UnknownTransactionException("activeTransaction is null");
		}
	...
	public void removePropertyChangeListener(String propertyName,
			PropertyChangeListener listener) {
		this.propChangeSupport.removePropertyChangeListener(propertyName,
				listener);
	}
	...
	public Transaction getNextTransaction() throws LeaseDeniedException,
		...
		try {
			if ((nextTransaction != null)
					&& (nextTransaction.getState() == TransactionConstants.ACTIVE)) {
	...
	public ProxyPreparer getProxyPreparer() throws ConfigurationException {
		return (ProxyPreparer) config.getEntry(this.getClass().getName(),
		...
		return this.doAsPrivileged(new PrivilegedExceptionAction<Object>() {
			public Object run() throws Exception {
				...
				ProxyDoAsSubjectFactory proxyFactory = new ProxyDoAsSubjectFactory(
						Worker.this);
				return proxyFactory.makeProxy(preparer.prepareProxy(proxy),
						interfaceClass);
			}
		}, null);
   ...
   public Object doAsPrivileged(PrivilegedExceptionAction action,
		...
		if (this.loginContext != null) {
			logger.finest("Doing privileged action with loginContext: "
					+ this.loginContext + " " + this.loginContext.getSubject());

			return Subject.doAsPrivileged(this.loginContext.getSubject(),
					action, acc);
		}
		try {
   ...
   public Object doAsPrivileged(PrivilegedAction action,
	...
	public ServiceItem lookup(final ServiceTemplate tmpl,
			final ServiceItemFilter filter, long waitDur)
			throws InterruptedException, RemoteException,
			ConfigurationException {
		logger.info("Looking up: " + tmpl + " filter: " + filter + " worker: "
				+ this.getWorkerDescWithId());
		return Worker.this.jiniManager.lookup(tmpl,
	...
	public ServiceItem[] lookup(ServiceTemplate tmpl, int maxMatches,
		...
		try {
			...
			if (results.length == 0) {
				...
				ServiceItem serviceItem = this.jiniManager.lookup(tmpl,
						addServiceProxyFilter(filter), 1000 * 20);
				if (serviceItem != null) {
					results = new ServiceItem[] { serviceItem };
				}
				logger.info("Looking up with alternate call resulted in: "
						+ results.length + " matches.");
			}
			if (results.length == 0) {
				logger.info("Trying lookup with new ServiceDiscoveryManager");
				ServiceDiscoveryManager serviceDiscovery = (ServiceDiscoveryManager) config
						.getEntry(this.getClass().getName(),
								"serviceDiscovery",
								ServiceDiscoveryManager.class);
				ServiceItem serviceItem = serviceDiscovery.lookup(tmpl,
						addServiceProxyFilter(filter), 1000 * 20);
				if (serviceItem != null) {
					results = new ServiceItem[] { serviceItem };
				}
				logger
						.info("New ServiceDiscoveryManager lookup up resulted in: "
								+ results.length + " matches.");
			}
			...
			if (results.length == 0) {
				...
				while (workerItr.hasNext()) {
					...
					if (worker != this) {
						logger.info("Trying lookup with other worker: "
								+ worker);
						results = worker.lookup(tmpl, maxMatches, filter);
						logger.info("Other worker found results: "
								+ results.length);
					}
				}
    ...
    private void testAccess() {
		...
		try {
			...
			Subject clientSubject = AccessController
					.<Subject> doPrivileged(new PrivilegedAction<Subject>() {
						public Subject run() {
			...
			logger.info("protection domain: "
					+ this.getClass().getProtectionDomain());
			logger.info("permission collection: "
					+ this.getClass().getProtectionDomain().getPermissions());
			logger.info("clientSubject: " + clientSubject);
			Principal clientPrincipal = (Principal) clientSubject
			...
			p = new AuthenticationPermission(clientSubject.getPrincipals(),
					reggiePrincipalSet, "connect");
			logger.info("Permission: " + p);
			sm.checkPermission(p);
			logger.log(Level.INFO, "Test of checkPermission: " + p
					+ " succeeded.");
		} catch (Exception e1) {
			logger.log(Level.INFO, "Test of checkPermission: " + p + " failed."
					+ e1.toString(), e1);
		}
	}

	public ServiceItem[] lookupAllGroups(ServiceTemplate tmpl, int maxMatches,
			ServiceItemFilter filter) throws InterruptedException,
		...
		return Worker.this.jiniManager.getAllGroupsServiceDiscoveryManager()
				.lookup(tmpl, maxMatches, addServiceProxyFilter(filter));
	}
	...
	public ServiceItem[] lookup(ServiceTemplate tmpl, int minMatches,
		...
		logger.info("Looking up (5): " + tmpl + " filter: " + filter
				+ " worker: " + this.getWorkerDescWithId() + "\n ClassLoader: "
		...
		try {
			 ...
			 * ServiceDiscoveryManager lookup up resulted in: " + results.length + "
			 * matches."); }
			 * 
			 * 
	...
	public LookupCache createLookupCache(final ServiceTemplate tmpl,
	...
	public void renewFor(Lease lease, long desiredDuration, long renewDuration,
		...
		this.jiniManager.renewFor(lease, desiredDuration, renewDuration,
				listener);
	}

	public JoinManager createJoinManager(final Object object,
			final Entry[] entries, final ServiceID serviceID,
	...
	private ServiceItemFilter addServiceProxyFilter(
			ServiceItemFilter extraFilter) throws ConfigurationException {
		if ((extraFilter != null)
				&& (ProxyPreparerFilter.class.isAssignableFrom(extraFilter
						.getClass()))) {
			return extraFilter;
		}
