...
public class BpaDragAndDropBean implements PropertyChangeListener, I_SupportDragAndDrop {
	...
	public BpaDragAndDropBean(String prefix, I_DoDragAndDrop dndComponent,
		...
		if (allowDrag) {
			...
			this.gestureListener = new BpaGestureListener(dndComponent,
					this.sourceListener);
            
            recognizer = this.dragSource.createDefaultDragGestureRecognizer(dndComponent
                    .getJComponent(), DnDConstants.ACTION_COPY,
		...
		if (allowDrop) {
			this.dtListener = new BpaTargetListener(dndComponent, prefix, acceptClass);
			this.dropTarget = new DropTarget(dndComponent.getJComponent(),
