...
public class QueueViewerPanel extends JPanel {
    ...
    private class CommitListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            if (tableOfQueues.getSelectedRow() >= 0) {
                qAdaptor =
                        (QueueAdaptor) tableOfQueues.getModel().getValueAt(
                ...
                try {
                    if (lsm.getMinSelectionIndex() < sorter.getRowCount()) {
                        int firstSelectedRow =
                                sorter.modelIndex(lsm.getMinSelectionIndex());
                        ...
                        try {
                            process =
                                    tableOfQueueEntriesModel.getQueue().read(
                                        processMeta.getEntryID(), null);
                            ProcessPanel processPanel =
                                    new ProcessPanel(process, worker);
                            int dividerLoc =
                                    queueContentsSplitPane.getDividerLocation();
    ...
    JSplitPane queueContentsSplitPane =
    ...
    private JButton refresh =
            new JButton("<html><font color='#006400'>refresh");
    ...
    public QueueViewerPanel(Configuration jiniConfig, I_Work worker,
        ...
        this.executeActionListener =
                new ExecuteProcessActionListener(jiniConfig, UuidFactory
                    .generate(), worker);
        this.execute.addActionListener(executeActionListener);
        JPanel statusPanel =
                makeStatusPanel(this.statusMessage, refresh, execute);
        c.weighty = 0;
        c.gridy = 1;
    ...
    private class GetQueuesAsWorker implements PrivilegedExceptionAction {
        ...
        public Object run() throws Exception {
            ...
            ServiceItem[] services =
                    worker.lookup(template, 1, 50, queuefilter, 3000);
            ...
            for (int i = 0; i < services.length; i++) {
                ...
                queue =
                        (I_QueueProcesses) worker.prepareProxy(queue,
                    ...
                    if (services[i].attributeSets[j] != null) {
                        if (Name.class
                            .isAssignableFrom(services[i].attributeSets[j]
                                .getClass())) {
                            Name nameEntry =
                                    (Name) services[i].attributeSets[j];
                ...
                try {
                    if (selectedQueue != queue) {
                        ...
                        tableOfQueueEntriesSortingTable =
                                new TableSorter(tableOfQueueEntriesModel);
                        tableOfQueueEntries =
                                new JTable(tableOfQueueEntriesSortingTable);
                        ...
                        if (selectedRow >= 0
                            && selectedRow < tableOfQueueEntries.getRowCount()) {
                            selectedEntry =
                                    (EntryID) tableOfQueueEntries.getModel()
