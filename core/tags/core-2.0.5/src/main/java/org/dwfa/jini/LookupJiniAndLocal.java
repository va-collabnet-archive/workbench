...
public class LookupJiniAndLocal implements I_LookupServices {
   ...
   public ServiceItem[] lookup(ServiceTemplate tmpl, int minMatches, int maxMatches, ServiceItemFilter filter,
      ...
      if (this.sdm != null) {
         if (I_QueueProcesses.class.isAssignableFrom(tmpl.serviceTypes[0])) {
            ...
            matches.addAll(Arrays.asList(this.sdm
                  .lookup(tmpl, minMatches, maxMatches - matches.size(), filter, waitDur)));
         }
      }
   ...
   /**
    * Returns <code>true</code> if the two arguments are of the same class and
    * for each entry field <em>F</em>, the arguments' values for <em>F</em>
    * on e1 is null (a wildcard), or if either both <code>null</code> or the
    * invocation of <code>equals</code> on one argument's value for <em>F</em>
    * with the other argument's value for <em>F</em> as its parameter returns
    * <code>true</code>. Will also return <code>true</code> if both
    * arguments are <code>null</code>. In all other cases an invocation of
   ...
   private static Field[] fieldInfo(Entry entry) {
      ...
      /*
       * Scan the array to see if we can use it or if we must build up a smaller
       * array because we must skip some fields. If so, we create an ArrayList
       * and add the unskippable fields to it, and then fetch the array back out
       * of it.
       */
