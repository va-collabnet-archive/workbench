...
public class HtmlHandler extends Handler implements I_PublishLogRecord {
    ...
    private SimpleDateFormat fileName =
    ...
    public HtmlHandler(JEditorPane logOut, String loggerName)
            throws IOException {
        super();
        this.logOut = logOut;
        ...
        root.mkdirs();
        FileWriter fw =
                new FileWriter(new File(root, loggerName
                    + fileName.format(new Date()) + ".html"));
        ...
        public void run() {
            try {
                ...
                ((HTMLEditorKit) logOut.getEditorKit()).read(
                    new java.io.StringReader(htmlRecord), logOut.getDocument(),
                    logOut.getDocument().getLength());
    ...
    private String format(LogRecord rec) {
        ...
        if ((params != null) && (params.length > 0)) {
            for (int i = 0; i < params.length; i++) {
                ...
                if (params[i] == null) {
                    buf.append("null");
                } else if (Collection.class.isAssignableFrom(params[i]
                    .getClass())) {
        ...
        if (rec.getThrown() != null) {
            ...
            for (int i = 0; i < elements.length; i++) {
                buf.append("&nbsp;&nbsp;&nbsp;"
                    + elements[i].toString().replaceAll("<", "&lt;"));
                buf.append("<br>");
            }
