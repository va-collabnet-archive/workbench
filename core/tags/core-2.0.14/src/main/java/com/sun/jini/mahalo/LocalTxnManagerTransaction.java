 ...
 * 
 */
class LocalTxnManagerTransaction implements TransactionConstants,
		TimeConstants, LeasedResource {
	 ...
	 * a task.
	 * 
	 * When a transaction is committing, a LocalPrepareJob is created and its tasks
	 * are scheduled. After completion, the LocalPrepareJob's outcome is computed.
	 * Depending on the outcome, either an LocalAbortJob or CommitJob is scheduled.
	 ...
	 */
	LocalTxnManagerTransaction(TransactionManager mgr, LogManager logmgr,
			long id, TaskManager threadpool, WakeupManager wm,
			TxnSettler settler, Uuid uuid) {
		if (logmgr == null)
	...
	void modifyParticipant(LocalParticipantHandle handle, int state) {
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(TxnManagerTransaction.class.getName(),
					"modifyParticipant", new Object[] { handle,
							new Integer(state) });
		}
	 ...
	 */
	public void join(TransactionParticipant preparedPart, long crashCount)
			throws CannotJoinException, CrashCountException, RemoteException {
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger
					.entering(TxnManagerTransaction.class.getName(), "join",
		...
		try {
			LocalParticipantHandle ph = new LocalParticipantHandle(
					preparedPart, crashCount);
			LocalParticipantHandle phtmp = (LocalParticipantHandle) ((parts
					.contains(ph)) ? parts.get(parts.indexOf(ph)) : null);

			if (transactionsLogger.isLoggable(Level.FINEST)) {
				transactionsLogger.log(Level.FINEST,
						"Retrieved LocalParticipantHandle: {0}", phtmp);
			}
			if (phtmp != null) {
	 ...
	 * 
	 */
	void commit(long waitFor) throws CannotCommitException,
		...
		try {
			...
			if (transactionsLogger.isLoggable(Level.FINE)) {
				transactionsLogger.log(Level.FINE,
						"{0} TransactionParticipants have joined", new Integer(
								numparts));
			}
			...
			if (modifyTxnState(VOTING)) {
				...
				synchronized (jobLock) {
					if (job == null) {
						if (phs.length == 1) {
							job = new LocalPrepareAndCommitJob(str, threadpool,
									wm, log, phs[0]);
						} else {
							job = new LocalPrepareJob(str, threadpool, wm, log,
									phs);
						}
						job.scheduleTasks();
					}
				...
				synchronized (jobLock) {
					if ((job instanceof LocalPrepareJob)
			...
			if (transactionsLogger.isLoggable(Level.FINE)) {
				transactionsLogger.log(Level.FINE, "Voting result: {0}",
			...
			case COMMITTED:
				...
				if (getState() != COMMITTED)
					throw new InternalManagerException(
							"TxnManagerTransaction: " + "commit: " + job
									+ " got bad state: "
									+ TxnConstants.getName(result.intValue()));
			...
			if (transactionsLogger.isLoggable(Level.FINEST)) {
				transactionsLogger.log(Level.FINEST,
						"Problem committing transaction", rte);
			}
			throw rte;
		} catch (LogException le) {
			if (transactionsLogger.isLoggable(Level.FINEST)) {
				transactionsLogger.log(Level.FINEST,
						"Problem persisting transaction", le);
			}
	 ...
	 * LocalAbortJob which schedules tasks executed on a thread pool. These tasks
	 ...
	 * @see net.jini.core.transaction.server.TransactionParticipant
	 */
	void abort(long waitFor) throws CannotAbortException,
		...
		try {
			...
			try {
				synchronized (jobLock) {
					if (remainder <= 0 || !job.isCompleted(remainder)) {
						settler.noteUnsettledTxn(str.id);
						throw new TimeoutExpiredException("timeout expired",
								false);
					} else {
						result = (Integer) job.computeResult();
						aborted = true;
			...
			if (!aborted)
				throw new InternalManagerException("TxnManagerTransaction: "
						+ "abort: LocalAbortJob got bad state: "
						+ TxnConstants.getName(result.intValue()));
			...
			if (transactionsLogger.isLoggable(Level.SEVERE)) {
				transactionsLogger.log(Level.SEVERE,
						"Problem aborting transaction", rte);
			}
			throw new InternalManagerException("TxnManagerTransaction: "
					+ "abort: fatal error");
		} catch (LogException le) {
	...
	private Vector<LocalParticipantHandle> parthandles() {
		...
		Vector<LocalParticipantHandle> vect = new Vector<LocalParticipantHandle>(
				parts);

		if (transactionsLogger.isLoggable(Level.FINEST)) {
			transactionsLogger.log(Level.FINEST, "Retrieved {0} participants",
	...
	private String getParticipantInfo() {
		...
		if (transactionsLogger.isLoggable(Level.FINEST)) {
			transactionsLogger.log(Level.FINEST, "{0} participants joined",
					new Integer(parts.size()));
		}
	...
	void restoreTransientState(ProxyPreparer preparer) throws RemoteException {
		...
		LocalParticipantHandle[] handles = (LocalParticipantHandle[]) parts
				.toArray(new LocalParticipantHandle[parts.size()]);
		for (int i = 0; i < handles.length; i++) {
			handles[i].restoreTransientState(preparer);
			if (transactionsLogger.isLoggable(Level.FINEST)) {
				transactionsLogger.log(Level.FINEST,
						"Restored transient state for {0}", handles[i]);
			}
		}
