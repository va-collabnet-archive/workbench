...
@BeanList(specs = 
{ @Spec(directory = "tasks/queue tasks/load", type = BeanType.TASK_BEAN)})
public class LoadFromQueue extends AbstractTask {
    ...
    public void complete(I_EncodeBusinessProcess process, I_Work worker)
        ...
        try {
            ...
            for (PropertyDescriptor d : process.getAllPropertiesBeanInfo()
                    .getPropertyDescriptors()) {
                PropertyDescriptorWithTarget dwt = (PropertyDescriptorWithTarget) d;
                if (dwt.getLabel().equals(this.localPropName)) {
                    qed = (QueueEntryData) dwt.getReadMethod().invoke(
                            dwt.getTarget(), new Object[] {});
                    break;
                }
            }
            ...
            I_EncodeBusinessProcess p = q.take(qed.getEntryID(), worker
