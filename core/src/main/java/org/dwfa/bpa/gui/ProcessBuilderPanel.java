...
public class ProcessBuilderPanel extends JPanel implements ActionListener,
	...
	public class WriteAttachmentActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			String key = (String)JOptionPane.showInputDialog(
			                    ProcessBuilderPanel.this,
			                    "Enter the attachment key:",
			                    "Write Attachment",
	...
	public class SaveProcessForXmlActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				FileDialog f = new FileDialog((Frame) ProcessBuilderPanel.this
    ...
    */
	...
	private static Logger logger = Logger.getLogger(ProcessBuilderPanel.class
			.getName());
	private static LogWithAlerts logWithAlerts = new LogWithAlerts(
			ProcessBuilderPanel.class.getName());
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if (f.getFile() != null) {
					...
					File processBinaryFile = new File(f.getDirectory(),
							fileName);
			...
			try {
				if (f.getFile() != null) {
					...
					if (f.getFile().endsWith(".xml")) {
						...
						I_EncodeBusinessProcess process = (I_EncodeBusinessProcess) d
								.readObject();
						d.close();
						setProcess(process);
					} else {
		...
		public void actionPerformed(ActionEvent e) {
			...
			Runnable r = new Runnable() {
				public void run() {
					...
					if (launchOnly) {
						try {
							MarshalledObject mp = new MarshalledObject(
									executingProcess);
							executingProcess = (I_EncodeBusinessProcess) mp
									.get();
							executingProcess.getLogger().setLevel(
									Worker.WorkerLevel.getInfoPlusLevel());
						} catch (IOException e) {
					...
					try {
						logger.info("Worker: " + worker.getWorkerDesc() + " ("
					...
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							...
							launch
									.setText("<html><font color='#006400'>launch");
							...
							if (exceptionMessage.equals("")) {
								statusMessage
										.setText("<html><font color='blue'>Process complete. Elapsed time: "
												+ elapsedTime + " ms.");
							} else {
								statusMessage
										.setText("<html><font color='blue'>Process complete: Elapsed time: "
	...
	private JLabel statusMessage = new JLabel(
			"<HTML><font color='blue'>Process Builder ready...");

	private I_Work worker;

	private static DateFormat dateFormatter = AceDateFormat.getDisplayDateFormat();
	...
	public ProcessBuilderPanel(Configuration jiniConfig, I_Work worker)
		...
		defaultOrigin = (String) jiniConfig.getEntry(this.getClass().getName(),
		...
		taskSplitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT,
				createTaskTree(), taskScroller);
		taskSplitPane.setDividerLocation(200);

		splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, taskSplitPane,
				processPanel);
		...
		this.statusPanel = makeStatusPanel(this.statusMessage, execute, cancel,
				launch);
		c.weighty = 0;
		c.gridy = 1;
    ...
    */
	public void newProcess() {
		try {
			...
			BusinessProcess newProcess = new BusinessProcess(processName,
					Condition.CONTINUE);
	...
	private class PropertySelectionListener implements ListSelectionListener {
		...
		public PropertySelectionListener(JSplitPane propDocSplit,
				PropertyTableModel propTableModel) {
		...
		public void valueChanged(ListSelectionEvent e) {
			if ((propertiesPane.getSelectedRow() >= 0)
					&& propertiesPane.getSelectedRow() < propertiesPane
							.getRowCount()) {
				PropertyDescriptorWithTarget pdwt = propTableModel
						.getPropertyDescriptor(propertiesPane.getSelectedRow());
				PropertySpec spec = (PropertySpec) pdwt
						.getValue(PropertyDescriptorWithTarget.VALUE.PROPERTY_SPEC
								.name());
				...
				this.propDocSplit.setBottomComponent(new JLabel(
	...
	private void setupPropertiesPane() throws RemoteException, IOException,
		...
		if (panelTypeComboBox != null && (panelTypeComboBox.getSelectedItem() == PanelType.properties)) {
			...
			PropertyTableModel ptm = new PropertyTableModel(processPanel
					.getProcess());
			propertiesPane = new JTable(ptm);
			propertiesPane.setCellSelectionEnabled(true);
			propertiesPane.setDragEnabled(true);
			PropertySelectionListener psl = new PropertySelectionListener(
					propDocSplit, ptm);
			...
			propDocSplit.setBottomComponent(new JLabel(
					"<html>No property selected"));
		...
		if (this.taskDirectory != null) {
			...
			if (tasksAndProcesses != null) {
				...
				for (int i = 0; i < tasksAndProcesses.length; i++) {
					try {
						...
						logger.log(Level.WARNING, "Exception processing "
								+ tasksAndProcesses[i] + " (" + ex.toString()
								+ ")", ex);
					} catch (Error ex) {
				...
				Collections.sort(tasks, new Comparator<I_DefineTask>() {
					...
					public int compare(I_DefineTask t1, I_DefineTask t2) {
						try {
							...
							String nameForT1 = ti1.getBeanDescriptor()
									.getDisplayName();
							BeanInfo ti2 = t2.getBeanInfo();
							String nameForT2 = ti2.getBeanDescriptor()
									.getDisplayName();
							...
							logger
									.log(Level.SEVERE, e.getLocalizedMessage(),
											e);
							return t1.getClass().getName().compareTo(
									t2.getClass().getName());
						}
					}
				});
				...
				for (I_DefineTask task : tasks) {
					try {
						createTaskPanelAndAdd(tasksPanel, c, task, taskFileMap
								.get(task));
					} catch (Exception ex) {
						logger.log(Level.WARNING, "Exception processing "
								+ task + " (" + ex.toString() + ")", ex);
					} catch (Error ex) {
						logger.log(Level.SEVERE, "Error processing " + task
								+ " (" + ex.toString() + ")", ex);
					}
				}
			}
	...
	private JScrollPane createTaskTree() {
		...
		DefaultMutableTreeNode top = new DefaultMutableTreeNode(
				new FileWrapper(new File(System.getProperty("user.dir"),
						"tasks")));
		createNodes(top);
		DefaultMutableTreeNode processesNode = new DefaultMutableTreeNode(
				new FileWrapper(new File(System.getProperty("user.dir"),
						"processes")));
	...
	public void setProcess(I_EncodeBusinessProcess process)
