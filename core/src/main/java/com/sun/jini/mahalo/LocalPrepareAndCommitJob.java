...
public class LocalPrepareAndCommitJob extends LocalJob implements TransactionConstants {
     ...
     * @see net.jini.core.transaction.server.TransactionParticipant
     */
    public LocalPrepareAndCommitJob(Transaction tr, TaskManager pool, WakeupManager wm, ClientLog log,
            LocalParticipantHandle handle) {
        super(pool, wm);
        ...
        if (!(tr instanceof ServerTransaction))
            throw new IllegalArgumentException("PrepareAndCommitJob: " + "PrepareAndCommitJob: "
    ...
    Object doWork(TaskManager.Task who, Object param) {
        ...
        try {
            if (attempt(who) > maxtries) {
                if (operationsLogger.isLoggable(Level.FINE)) {
                    operationsLogger.exiting(PrepareAndCommitJob.class.getName(), "doWork", new Integer(ABORTED));
                }
        ...
        try {
            ...
            reCaught = re;
            if (operationsLogger.isLoggable(Level.FINE)) {
                operationsLogger.log(Level.FINE, "Ignoring remote exception from participant.", re);
            }
        ...
        if (response != null) {
            ...
            try {
                ...
                if (persistenceLogger.isLoggable(Level.FINE)) {
                    persistenceLogger.log(Level.FINE, "Problem writing PrepareAndCommitRecord.", le);
                }
                // TODO - ignore?
            }
            if (operationsLogger.isLoggable(Level.FINE)) {
