...
public class EImage extends EComponent<EImageRevision> {
    ...
    public EImage(I_ImageVersioned imageVer) throws TerminologyException, IOException {
        ...
        time = part.getTime();
        if (partCount > 1) {
            revisions = new ArrayList<EImageRevision>(partCount - 1);
    ...
    public void writeExternal(DataOutput out) throws IOException {
        ...
        if (revisions == null) {
            ...
            for (EImageRevision eiv : revisions) {
                eiv.writeExternal(out);
            }
        }
    }

    public List<EImageRevision> getRevisionList() {
        return revisions;
    }
