...
public class OwlFunctionalSyntaxExportSct extends AbstractTask {
    ...
    public Condition evaluate(I_EncodeBusinessProcess process, I_Work worker)
        ...
        try {
            ...
            if (config.getClassifierInputMode() == CLASSIFIER_INPUT_MODE_PREF.EDIT_PATH) {
                ...
                pcEdit = new SnoPathProcessCondorStated(logger, cEditSnoCons, cEditSnoRels,
                        allowedRoleTypes, statusSet, cViewPosSet, cEditPosSet, null,
            ...
            writeOwl(cEditSnoCons, cEditSnoRels, "condor_sctids_t" + startTime + ".owl");
    ...
    private void writeOwl(List<SnoCon> scl, List<SnoRel> srl, String fName)
        ...
        // BEGIN ONTOLOGY
        bw.append("\n\nOntology(<http://www.ihtsdo.org/wb.sct.owl>\n");
        bw.append("Annotation(rdfs:comment \"IHTSDO Workbench SCT id export.\")\n");
        ...
        for (SnoCon sc : scl) {
            ...
            String rSctIdStr = Long.toString(nid2sctid(roleNid)).replace("-", "n");
            ...
            bw.append("Declaration(ObjectProperty(:SCT_" + rSctIdStr + "))\n");
            bw.append("AnnotationAssertion(rdfs:label :SCT_" + rSctIdStr + " \"" + rText + "\")\n");
            ...
            if (c1Parents.size() == 1) {
                ...
                if (c2Nid != rootRoleNid) {
                    String c2SctIdStr = Long.toString(nid2sctid(c2Nid)).replace("-", "n");
                    if (roleNid == rootRoleNid) {
                        bw.append("SubClassOf(:SCT_" + rSctIdStr + " :SCT_" + c2SctIdStr + ")\n");
                    } else {
                        bw.append("SubObjectPropertyOf(:SCT_" + rSctIdStr + " :SCT_" + c2SctIdStr + ")\n");
                    }
                }
            } else {
            ...
            // :NYI: right identity
            // bw.append("SubObjectPropertyOf(ObjectPropertyChain(:SCT_" + rnid
            //        + " :SCT_" + NYI +") :SCT_" + rnid + ")\n");
        }
        ...
        for (SnoCon sc : scl) {
            ...
            String c1SctIdStr = Long.toString(nid2sctid(sc.id)).replace("-", "n");
            ...
            bw.append("Declaration(Class(:SCT_" + c1SctIdStr + "))\n");
            bw.append("AnnotationAssertion(rdfs:label :SCT_" + c1SctIdStr + " \"" + c1Text + "\")\n");
            ...
            if (c1Parents.size() == 0L && c1Roles.size() == 0L) {
                ...
                String c2SctIdStr = Long.toString(nid2sctid(c1Parents.get(0).c2Id)).replace("-", "n");
                bw.append("SubClassOf(:SCT_" + c1SctIdStr + " :SCT_" + c2SctIdStr + ")\n");
            } else {
                if (sc.isDefined) {
                    bw.append("EquivalentClasses(:SCT_" + c1SctIdStr + " ObjectIntersectionOf(");
                } else {
                    bw.append("SubClassOf(:SCT_" + c1SctIdStr + " ObjectIntersectionOf(");
                }

                // ADD PARENTS
                for (SnoRel snoRel : c1Parents) {
                    String c2SctIdStr = Long.toString(nid2sctid(snoRel.c2Id)).replace("-", "n");
                    bw.append(":SCT_" + c2SctIdStr + " ");
                }
                bw.append("\n");
                ...
                if (c1Roles.size() > 0L) {
                    ...
                    for (SnoGrp rg : groupList) {
                        if (rg.size() > 1) {
                            ...
                            for (SnoRel r : rg) {
                                String typeSct = Long.toString(nid2sctid(r.typeId)).replace("-", "n");
                                String c2Sct = Long.toString(nid2sctid(r.c2Id)).replace("-", "n");
                                if (DescriptionLogic.isNegatedRel(r.relNid, vc)) {
                                    bw.append("                        ObjectSomeValuesFrom(:SCT_" + typeSct
                                            + " ObjectComplementOf(:SCT_" + c2Sct + "))\n");
                                } else {
                                    bw.append("                        ObjectSomeValuesFrom(:SCT_" + typeSct
                                            + " :SCT_" + c2Sct + ")\n");
                                }
                            ...
                            String typeSct = Long.toString(nid2sctid(rg.get(0).typeId)).replace("-", "n");
                            String c2Sct = Long.toString(nid2sctid(rg.get(0).c2Id)).replace("-", "n");
                            if (neverGroupedSet.contains(rg.get(0).typeId)) {
                                ...
                                if (DescriptionLogic.isNegatedRel(rg.get(0).relNid, vc)) {
                                    bw.append("       ObjectSomeValuesFrom(:RoleGroup ObjectSomeValuesFrom(:SCT_"
                                            + typeSct + " ObjectComplementOf(:SCT_" + c2Sct + ")))\n");
                                } else {
                                    bw.append("       ObjectSomeValuesFrom(:RoleGroup ObjectSomeValuesFrom(:SCT_"
                                            + typeSct + " :SCT_" + c2Sct + "))\n");
                                }
        ...
        for (I_GetConceptData disjointCollection : disjointSets) {
            ...
            for (RefexChronicleBI rcbi : members) {
                bw.append(" :SCT_" +
                        Long.toString(nid2sctid(rcbi.getReferencedComponentNid())).replace("-", "n"));
            }
            bw.append(" )\n");
        }
        ...
        for (I_GetConceptData unionCollection : unionsSets) {
            ...
            String unionStr = Long.toString(
                    nid2sctid(union.getReferencedComponentNid())).replace("-", "n");
            bw.append("Declaration(Class(:SCT_" + unionStr + "))\n");
            String unionText = unionCollection.getInitialText().replace("\"", "\'");
            bw.append("AnnotationAssertion(rdfs:label :SCT_" + unionStr
                    + " \"" + unionText + "\")\n");
            bw.append("EquivalentClasses( :SCT_" + unionStr);
    ...
    private long nid2sctid(int nid) {
        // Convert native id to sct id
        long sctid = Long.MAX_VALUE;
        try {
            I_GetConceptData cb = tf.getConcept(nid);
            if (cb != null) {
                I_Identify idfr2 = cb.getIdentifier();
                if (idfr2 != null) {
                    List<? extends I_IdVersion> idv2 = idfr2.getIdVersions();
                    sctid = Long.MAX_VALUE;
                    for (I_IdVersion id : idv2) {
                        if (id.getAuthorityNid() == snomedLongAuthorityNid) {
                            sctid = (Long) id.getDenotation();
                            break;
                        }
                    }
                } else {
                    logger.log(Level.INFO,
                            "\r\n::: [OWLF] no identifier warning nid = {0} {1} {2}",
                            new Object[]{nid, cb.getPrimUuid(), cb.getInitialText()});
                }
            } else {
                logger.log(Level.INFO, "\r\n::: [OWLF] warning nid = {0}", nid);
            }
