...
public class StringExtTupleFileUtil {
    ...
    public static String exportTuple(I_ExtendByRefVersion tuple) throws TerminologyException, IOException {
        ...
        try {
            ...
            UUID tupleUuid = ArchitectonicAuxiliary.Concept.EXT_STRING_TUPLE.getUids().iterator().next();
            ...
            if (!typeUuid.equals(RefsetAuxiliary.Concept.STRING_EXTENSION.getUids().iterator().next())) {
                throw new TerminologyException("Non string ext tuple passed to concept file util.");
            }

            I_ExtendByRefPartStr part = (I_ExtendByRefPartStr) tuple.getMutablePart();
        ...
        try {
            ...
            try {
                ...
                refsetHelper.newStringRefsetExtension(termFactory.uuidToNative(refsetUuid), termFactory
                    .uuidToNative(componentUuid), extString);
            } catch (Exception e) {
