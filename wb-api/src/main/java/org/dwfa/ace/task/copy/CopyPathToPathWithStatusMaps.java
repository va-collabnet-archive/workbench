    ...
    */
    ...
    public Condition evaluate(I_EncodeBusinessProcess process, I_Work worker) throws TaskFailedException {
        try {
            ...
            I_Path toPath = tf.getPath(AceTaskUtil.getConceptFromObject(process.getProperty(toPathPropName)).getUids());
            I_GetConceptData concept = AceTaskUtil.getConceptFromObject(process.getProperty(conceptPropName));
            Map<TermEntry, TermEntry> conceptStatusMap = (Map<TermEntry, TermEntry>) process.getProperty(conceptStatusMapPropName);
            Map<Integer, Integer> conceptStatusNidMap = makeNidMap(conceptStatusMap);
            Map<TermEntry, TermEntry> elementStatusMap = (Map<TermEntry, TermEntry>) process.getProperty(elementStatusMapPropName);
