...
public class CreateRefsetMetaDataTask extends AbstractTask {
   ...
   private String              newRefsetPropName         =
      ProcessAttachmentKeys.WORKING_REFSET.getAttachmentKey();
   private TermEntry           statusTermEntry           =
      new TermEntry(ArchitectonicAuxiliary.Concept.CURRENT.getUids());
   private String              reviewerUuidPropName      =
      ProcessAttachmentKeys.REVIEWER_UUID.getAttachmentKey();
   private transient Condition returnCondition           = Condition.ITEM_COMPLETE;
   private String              ownerUuidPropName         =
      ProcessAttachmentKeys.OWNER_UUID.getAttachmentKey();
   private String              newRefsetUUIDPropName     =
      ProcessAttachmentKeys.REFSET_UUID.getAttachmentKey();
   private String              newRefsetSpecUUIDPropName =
      ProcessAttachmentKeys.REFSET_SPEC_UUID.getAttachmentKey();
   private String              editorUuidPropName        =
      ProcessAttachmentKeys.EDITOR_UUID.getAttachmentKey();
   private String              computeTypeUUIDPropName   =
      ProcessAttachmentKeys.REFSET_COMPUTE_TYPE_UUID.getAttachmentKey();
   ...
   public void doRun(final I_EncodeBusinessProcess process, I_Work worker) {
      try {
         ...
         I_GetConceptData owner             =
            termFactory.getConcept((UUID[]) process.getProperty(ownerUuidPropName));
         I_GetConceptData editor            =
            termFactory.getConcept((UUID[]) process.getProperty(editorUuidPropName));
         ...
         I_GetConceptData parent =
            termFactory.getConcept(new UUID[] {
               (UUID) process.getProperty(ProcessAttachmentKeys.ACTIVE_CONCEPT.getAttachmentKey()) });
         ...
         I_GetConceptData refsetOwnerRel =
            termFactory.getConcept(RefsetAuxiliary.Concept.REFSET_OWNER.getUids());
         I_GetConceptData refsetEditorRel =
            termFactory.getConcept(RefsetAuxiliary.Concept.REFSET_EDITOR.getUids());
         I_GetConceptData promotionRel =
            termFactory.getConcept(RefsetAuxiliary.Concept.PROMOTION_REL.getUids());
         I_GetConceptData commentsRel =
            termFactory.getConcept(RefsetAuxiliary.Concept.COMMENTS_REL.getUids());
         I_GetConceptData editTimeRel =
            termFactory.getConcept(RefsetAuxiliary.Concept.EDIT_TIME_REL.getUids());
         ...
         try {
            ...
            JOptionPane.showMessageDialog(LogWithAlerts.getActiveFrame(null),
                                          "Refset wizard cannot be completed. " + e.getMessage(), "",
         ...
         process.setProperty(ProcessAttachmentKeys.REFSET_SPEC_UUID.getAttachmentKey(),
                             refsetSpec.getUids().iterator().next());
         ...
         JOptionPane.showMessageDialog(LogWithAlerts.getActiveFrame(null),
                                       "Refset wizard cannot be completed. Error creating refset meta data: "
   ...
   public void newDescription(I_GetConceptData concept, I_GetConceptData descriptionType, String description,
               ...
               if (actives.contains(part_search.getStatusNid())
                       && (part_search.getTypeNid()
                           == ArchitectonicAuxiliary.Concept.FULLY_SPECIFIED_DESCRIPTION_TYPE.localize()
                              .getNid()) && part_search.getText().equals(description)) {
      ...
      termFactory.newDescription(descUuid, concept, "en", description, descriptionType,
                                 termFactory.getActiveAceFrameConfig(), status.getNid());
   ...
   public void newRelationship(I_GetConceptData concept, I_GetConceptData relationshipType,
      ...
      try {
         int statusId =
            termFactory.getConcept(ArchitectonicAuxiliary.Concept.CURRENT.getUids()).getConceptNid();
