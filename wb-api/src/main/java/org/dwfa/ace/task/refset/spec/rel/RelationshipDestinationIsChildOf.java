...
/**
 * Task for adding a "relationship destination is child of" structural query to a refset spec.
 * 
 * @author Chrissy Hill
 * 
 */
@BeanList(specs = { @Spec(directory = "tasks/refset/spec/rel", type = BeanType.TASK_BEAN) })
public class RelationshipDestinationIsChildOf extends AddStructuralQueryToRefsetSpec {
    /**
	 * 
	 */
    ...
    protected int getStructuralQueryTokenId() throws IOException, TerminologyException {
        return RefsetAuxiliary.Concept.REL_DESTINATION_IS_CHILD_OF.localize().getNid();
    }
