...
public class TestForEdgCType2Req extends AbstractConceptTest {
    ...
    public List<AlertToDataConstraintFailure> test(I_GetConceptData concept, boolean forCommit)
        ...
        // STEP 1. Find all Descriptions with EDG Clinical Item 2 refset
        // membership.
        ...
        try {
            ...
            for (I_ExtendByRef ext : extList) {
                if (ext.getRefsetId() == nidICD9CodeMappings) {
                    if (extResult == null) {
                        extResult = ext;
                        List<? extends I_ExtendByRefPart> partList = ext.getMutableParts();
                        int lastVersion = Integer.MIN_VALUE;
                        for (I_ExtendByRefPart part : partList) {
