...
public class ComputeRefsetFromSpecTask extends AbstractTask {
	...
	public Condition computeRefset(I_ConfigAceFrame configFrame,
			I_GetConceptData refset, boolean showActivityPanel) {

		if (refset == null) {
			AceLog.getAppLog().info(
					"No refset in refset spec panel to compute.");
		...
		try {
			...
			AceLog.getAppLog().info(
					"Refset: " + refset.getInitialText() + " "
							+ refset.getUids().get(0));
			AceLog.getAppLog().info(
					"Refset spec: " + refsetSpec.getInitialText() + " "
			...
			RefsetSpecQuery query = RefsetQueryFactory.createQuery(configFrame,
					Terms.get(), refsetSpec, refset, computeType);
			...
			if (cancelComputation) {
				...
				if (!DwfaEnv.isHeadless()) {
					JOptionPane.showMessageDialog(LogWithAlerts
							.getActiveFrame(null), "Refset compute cancelled.",
							"", JOptionPane.ERROR_MESSAGE);
				}
				return Condition.ITEM_CANCELED;
			}
	...
	public Condition evaluate(I_EncodeBusinessProcess process, I_Work worker)
		...
		try {
			...
			if (!DwfaEnv.isHeadless()) {
				JOptionPane
						.showMessageDialog(LogWithAlerts.getActiveFrame(null),
								"Unable to complete refset compute: "
										+ ex.getMessage(), "",
								JOptionPane.ERROR_MESSAGE);
			}
			try {
