...
public class PutCompletedConceptsWithNoConflictsInListView extends AbstractTask {
    ...
    */
    ...
    private TermEntry retiredTermEntry = new TermEntry(
        ArchitectonicAuxiliary.Concept.DUPLICATE_PENDING_RETIREMENT.getUids());
    ...
    public Condition evaluate(I_EncodeBusinessProcess process, final I_Work worker) throws TaskFailedException {
        try {
            I_ConfigAceFrame profileForConflictDetection = (I_ConfigAceFrame) process.getProperty(profilePropName);
            ...
            I_IntSet completionStatusNids = Terms.get().newIntSet();
            ...
            CompletedConceptConflictDetector conflictIdentifier = new CompletedConceptConflictDetector(
                Terms.get().newIntSet(), Terms.get().newIntSet(),
                profileForConflictDetection, completionStatusNids);
            Terms.get().iterateConcepts(conflictIdentifier);
            ...
            for (int nid : conflictIdentifier.getNoConflictNids().getSetValues()) {
                noConflicts.add(Terms.get().getConcept(nid));
            }
