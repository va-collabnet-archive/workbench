...
public class GetDataFromAFPCPanel extends AbstractTask {
	...
	public Condition evaluate(final I_EncodeBusinessProcess process, final I_Work worker) throws TaskFailedException {
		...
		try {
			...
			AskForConceptParent panel = null;
			
			for (Component c : workflowDetailsSheet.getComponents()) {
				if (AskForConceptParent.class.isAssignableFrom(c.getClass())) {
					panel = (AskForConceptParent) c;
				}
			}
			...
			if (panel != null) {
				...
				if (listModel.getSize()>0){
					...
					AskForConceptName newPanel = new AskForConceptName(config,listModel);
