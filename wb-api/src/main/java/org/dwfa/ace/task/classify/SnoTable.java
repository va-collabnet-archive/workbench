...
//    }
    ...
    *
    */
   private static ArrayList<SnoRel> findIsaProximal(I_GetConceptData cBean, List<PositionBI> posList,
           boolean isStatedUser) {
      ArrayList<SnoRel> returnSnoRels = new ArrayList<SnoRel>();

      synchronized (posList) {
         posList = new ArrayList<PositionBI>(posList);
      }
            ...
            for (PositionBI pos : posList) {    // FOR EACH PATH POSITION
               ...
               if (rp1 != null) {
                  if ((rp1.getStatusNid() == isCURRENT) && (rp1.getTypeNid() == isaNid)) {
                     returnSnoRels.add(new SnoRel(rel.getC1Id(), rel.getC2Id(), rp1.getTypeNid(),
                                                  rp1.getGroup(), rel.getNid()));
                  }
   ...
   public boolean gatherFormData(I_GetConceptData cBean) {
      ...
      try {
         ...
         // USE: Authoring Form
         roleDiffFromProxList = findRoleGrpDiffFromProx(cBean, isaProxList, inferredPath, USER_SNOROCKET);

         // USE: Short Canonical Form
         roleDiffFromProxPrimList = findRoleGrpDiffFromProxPrim(cBean, isaProxPrimList, inferredPath,
                 USER_SNOROCKET);
      } catch (Exception e) {
         e.printStackTrace();
   ...
   private static boolean prefsNotSet() {
      if ((isaNid == Integer.MIN_VALUE) || (isCURRENT == Integer.MIN_VALUE) || (rootNid == Integer.MIN_VALUE)
              || (roleRootNid == Integer.MIN_VALUE) || (cEditPath == null) || (cViewPath == null)) {
         return true;
      } else {
         return false;
      }
   ...
   public String[][] getStringData() throws TerminologyException, IOException {
      ...
      for (int i = 0; i < totalRows; i++) {
         ...
         // FLAG STRINGS
         tableStrings[i][0] = ((row.flags & stateFlag) > 0)
                              ? xStr
                              : bStr;
         tableStrings[i][1] = ((row.flags & inferFlag) > 0)
                              ? xStr
                              : bStr;
         tableStrings[i][2] = ((row.flags & dnfFlag) > 0)
                              ? xStr
                              : bStr;
         tableStrings[i][3] = ((row.flags & anfFlag) > 0)
                              ? xStr
                              : bStr;
         tableStrings[i][4] = ((row.flags & scfFlag) > 0)
                              ? xStr
                              : bStr;
         tableStrings[i][5] = ((row.flags & lcfFlag) > 0)
                              ? xStr
                              : bStr;
