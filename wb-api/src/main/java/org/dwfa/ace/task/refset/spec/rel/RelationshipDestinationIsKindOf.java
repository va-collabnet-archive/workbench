...
/**
 * Task for adding a "relationship destination is kind of" structural query to a refset spec.
 * 
 * @author Chrissy Hill
 * 
 */
@BeanList(specs = { @Spec(directory = "tasks/refset/spec/rel", type = BeanType.TASK_BEAN) })
public class RelationshipDestinationIsKindOf extends AddStructuralQueryToRefsetSpec {
    /**
	 * 
	 */
    ...
    protected int getStructuralQueryTokenId() throws IOException, TerminologyException {
        return RefsetAuxiliary.Concept.REL_DESTINATION_IS_KIND_OF.localize().getNid();
    }
