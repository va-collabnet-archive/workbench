...
public class RelationshipStatement extends RefsetSpecStatement {
	...
	public I_RepresentIdSet getPossibleConcepts(I_ConfigAceFrame configFrame,
		...
		I_ShowActivity activity = Terms.get().newActivityPanel(true,
				configFrame, "<html>Possible: <br>" + this.toHtmlFragment(),
				true);
		...
		case REL_IS_MEMBER_OF:
			Collection<? extends I_ExtendByRef> refsetExtensions = termFactory
					.getRefsetExtensionMembers(((I_GetConceptData) queryConstraint)
							.getConceptId());
		...
		activity.setProgressInfoLower("Elapsed: " + elapsedStr
				+ ";  Incoming count: " + parentPossibleConcepts.cardinality()
	...
	public I_RepresentIdSet getPossibleDescriptions(I_ConfigAceFrame config,
		...
		throw new TerminologyException(
				"Get possible descriptions in rel statement unsupported operation.");
	}
	...
	public I_RepresentIdSet getPossibleRelationships(
			...
			throws TerminologyException, IOException {
		throw new TerminologyException(
				"Get possible relationships in rel statement unsupported operation.");
	}

	private boolean relRefinabilityIsDescendentOf(I_RelTuple relTuple)
			throws TerminologyException, IOException {
		return relRefinabilityIsDescendentOf(
				(I_GetConceptData) queryConstraint, relTuple);
	}

	private boolean relRefinabilityIsDescendentOf(
			I_GetConceptData requiredRefinability, I_RelTuple relTuple)
			throws TerminologyException, IOException {
		try {
			...
			Set<? extends I_GetConceptData> children = requiredRefinability
	...
	private boolean relRefinabilityIsChildOf(I_RelTuple relTuple)
			throws TerminologyException, IOException {
		try {
			I_IntSet allowedTypes = getIsAIds();
			...
			Set<? extends I_GetConceptData> children = ((I_GetConceptData) queryConstraint)
					.getDestRelOrigins(currentStatuses, allowedTypes,
							termFactory.getActiveAceFrameConfig()
									.getViewPositionSetReadOnly(), config
									.getPrecedence(), config
	...
	private boolean relRefinabilityIsKindOf(I_RelTuple relTuple)
			throws TerminologyException, IOException {
	...
	private boolean relRefinabilityIs(I_GetConceptData requiredRefinability,
			I_RelTuple relTuple) {
		return relTuple.getRefinabilityId() == requiredRefinability
				.getConceptId();
	}

	private boolean relCharIsDescendentOf(I_RelTuple relTuple)
			throws IOException, TerminologyException {
		return relCharIsDescendentOf((I_GetConceptData) queryConstraint,
				relTuple);
	}

	private boolean relCharIsDescendentOf(I_GetConceptData requiredCharType,
			I_RelTuple relTuple) throws IOException, TerminologyException {
		try {
			I_IntSet allowedTypes = getIsAIds();
			// TODO replace with passed in config...
			I_ConfigAceFrame config = Terms.get().getActiveAceFrameConfig();
			...
			Set<? extends I_GetConceptData> children = requiredCharType
					.getDestRelOrigins(currentStatuses, allowedTypes,
							termFactory.getActiveAceFrameConfig()
									.getViewPositionSetReadOnly(), config
									.getPrecedence(), config
	...
	private boolean relCharIsChildOf(I_RelTuple relTuple)
			throws TerminologyException, IOException {
		try {
			I_IntSet allowedTypes = getIsAIds();
			I_HelpSpecRefset helper = Terms.get().getSpecRefsetHelper(
	...
	private boolean relCharIs(I_GetConceptData requiredCharType,
			I_RelTuple relTuple) {
		return relTuple.getCharacteristicId() == requiredCharType
				.getConceptId();
	}
	...
	private boolean relTypeIs(I_GetConceptData requiredRelType,
			I_RelTuple relTuple) {
	...
	private boolean relTypeIsChildOf(I_RelTuple relTuple)
			throws TerminologyException, IOException {
		try {
			I_IntSet allowedTypes = getIsAIds();
			I_HelpSpecRefset helper = Terms.get().getSpecRefsetHelper(
	...
	private boolean relTypeIsDescendentOf(I_RelTuple relTuple)
			throws IOException, TerminologyException {
		return relTypeIsDescendentOf((I_GetConceptData) queryConstraint,
				relTuple);
	}

	private boolean relTypeIsDescendentOf(I_GetConceptData requiredRelType,
			I_RelTuple relTuple) throws IOException, TerminologyException {
		try {
			I_IntSet allowedTypes = getIsAIds();
			I_HelpSpecRefset helper = Terms.get().getSpecRefsetHelper(
			...
			Set<? extends I_GetConceptData> children = requiredRelType
					.getDestRelOrigins(currentStatuses, allowedTypes,
							termFactory.getActiveAceFrameConfig()
									.getViewPositionSetReadOnly(), config
									.getPrecedence(), config
	...
	private boolean relStatusIsDescendentOf(I_RelTuple relTuple)
			throws TerminologyException, IOException {
		return relStatusIsDescendentOf((I_GetConceptData) queryConstraint,
				relTuple);
	}

	private boolean relStatusIsDescendentOf(I_GetConceptData requiredStatus,
			I_RelTuple relTuple) throws TerminologyException, IOException {
		try {
			I_IntSet allowedTypes = getIsAIds();
			I_HelpSpecRefset helper = Terms.get().getSpecRefsetHelper(
					Terms.get().getActiveAceFrameConfig());
			...
			Set<? extends I_GetConceptData> children = requiredStatus
					.getDestRelOrigins(currentStatuses, allowedTypes,
							termFactory.getActiveAceFrameConfig()
									.getViewPositionSetReadOnly(), config
									.getPrecedence(), config
	...
	private boolean relStatusIsKindOf(I_RelTuple relTuple)
			throws TerminologyException, IOException {
	...
	private boolean relStatusIs(I_GetConceptData requiredStatus,
			I_RelTuple relTuple) {
	...
	private boolean relRestrictionIs(I_RelTuple relTuple) throws IOException,
			TerminologyException {
		throw new TerminologyException(
				"Unimplemented query : rel restriction is"); // unimplemented
		// TODO
	}

	private boolean relLogicalQuantifierIsDescendentOf(I_RelTuple relTuple)
			throws TerminologyException {
		throw new TerminologyException(
				"Unimplemented query : rel logical quantifier is descendent");
		// unimplemented TODO
	}

	private boolean relLogicalQuantifierIsChildOf(I_RelTuple relTuple)
			throws TerminologyException {
		throw new TerminologyException(
				"Unimplemented query : rel logical quantifier is child of");
		// unimplemented TODO
	}

	private boolean relLogicalQuantifierIsKindOf(I_RelTuple relTuple)
			throws TerminologyException {
		throw new TerminologyException(
				"Unimplemented query : rel logical quantifier is kind of");
		// unimplemented TODO
	}

	private boolean relLogicalQuantifierIs(I_RelTuple relTuple)
			throws TerminologyException {
		throw new TerminologyException(
				"Unimplemented query : rel logical quantifier is");
		// unimplemented TODO
	}
	...
	private boolean relDestinationIs(I_GetConceptData requiredDestination,
			I_RelTuple relTuple) {
		return relTuple.getC2Id() == requiredDestination.getConceptId();
	}

	private boolean relDestinationIsKindOf(I_RelTuple relTuple)
			throws IOException, TerminologyException {
	...
	private boolean relDestinationIsChildOf(I_RelTuple relTuple)
			throws TerminologyException, IOException {
		try {
			I_IntSet allowedTypes = getIsAIds();
			I_HelpSpecRefset helper = Terms.get().getSpecRefsetHelper(
	...
	private boolean relDestinationIsDescendentOf(I_RelTuple relTuple)
			throws IOException, TerminologyException {
		return relDestinationIsDescendentOf((I_GetConceptData) queryConstraint,
				relTuple);
	}

	private boolean relDestinationIsDescendentOf(
			I_GetConceptData requiredDestination, I_RelTuple relTuple)
			throws IOException, TerminologyException {
		try {
			I_IntSet allowedTypes = getIsAIds();
			I_HelpSpecRefset helper = Terms.get().getSpecRefsetHelper(
			...
			Set<? extends I_GetConceptData> children = requiredDestination
					.getDestRelOrigins(currentStatuses, allowedTypes,
							termFactory.getActiveAceFrameConfig()
									.getViewPositionSetReadOnly(), config
									.getPrecedence(), config
	...
	private I_RelPart getVersion(I_RelVersioned descriptionBeingTested,
		...
		try {
			I_RelPart an = null;
			for (I_RelPart a : descriptionBeingTested.getMutableParts()) {
				// Must be on the path
	...
	private boolean changedRelationshipCharacteristic(I_RelVersioned relBeingTested,
			GROUPING_TYPE version, I_Position v1_is, I_Position v2_is)
			throws TerminologyException, IOException {
		try {
			I_RelPart a1 = getVersion(relBeingTested, v1_is);
			I_RelPart a2 = getVersion(relBeingTested, v2_is);
			return (a1 != null && a2 != null
					&& a1.getVersion() != a2.getVersion() && a1.getCharacteristicId() != a2
					.getCharacteristicId());
		} catch (Exception e) {
			e.printStackTrace();
			throw new TerminologyException(e.getMessage());
	...
	private boolean changedRelationshipRefinability(I_RelVersioned relBeingTested,
			GROUPING_TYPE version, I_Position v1_is, I_Position v2_is)
			throws TerminologyException, IOException {
		try {
			I_RelPart a1 = getVersion(relBeingTested, v1_is);
			I_RelPart a2 = getVersion(relBeingTested, v2_is);
			return (a1 != null && a2 != null
					&& a1.getVersion() != a2.getVersion() && a1.getRefinabilityId() != a2
					.getRefinabilityId());
		} catch (Exception e) {
			e.printStackTrace();
			throw new TerminologyException(e.getMessage());
