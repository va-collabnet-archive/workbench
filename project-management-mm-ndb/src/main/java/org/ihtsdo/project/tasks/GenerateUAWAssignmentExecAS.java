...
public class GenerateUAWAssignmentExecAS extends AbstractTask {
	...
	public Condition evaluate(final I_EncodeBusinessProcess process, final I_Work worker) throws TaskFailedException {
		...
		try {
			...
			String destination="";
			if (selfAssign){
				queueName = config.getUsername().trim() + ".inbox";
				destination=queueName;
				wfProcess.setDestination(destination);
			}else{
				queueName = config.getUsername().trim() + ".outbox";
			...
			HashSet<Integer> conceptIds=(HashSet<Integer>)process.readAttachement("HASHED_CONCEPTS");
