...
public class SpecMarkedParentRefsetHelper extends SpecRefsetHelper implements I_HelpMarkedParentRefset {
    ...
    public SpecMarkedParentRefsetHelper(I_ConfigAceFrame config, int refsetId, int memberTypeId) throws Exception {
        ...
        ConceptVersionBI markedParentRefset = Ts.get().getConceptVersion(config.getViewCoordinate(), parentRefsetId);
        markedParentNids = markedParentRefset.getRefsetMemberNidsActive();
        ConceptVersionBI refset = Ts.get().getConceptVersion(config.getViewCoordinate(), refsetId);
    ...
    public void addParentMembers(Integer... conceptIds) throws Exception {
        for (int conceptId : conceptIds) {
            for (int parentNid : Ts.get().getAncestors(conceptId, config.getViewCoordinate())) {
    ...
    public void removeParentMembers(Integer... conceptIds) throws Exception {
        ...
        for (Integer conceptId : conceptIds) {
            ...
            for (int conceptNid : Ts.get().getAncestors(conceptId, config.getViewCoordinate())) {
        ...
        for (Integer parentId : toBeRetired) {
            for (Integer childId : Ts.get().getChildren(parentId, config.getViewCoordinate())) {
        ...
        for (Integer conceptId : lineageToExclude) {
            for (int conceptNid : Ts.get().getAncestors(conceptId, config.getViewCoordinate())) {
                ancestorIdsToExclude.add(conceptNid);
            }
        }
    ...
    public void removeDescriptionParentMembers(Integer... descriptionIds) throws Exception {
        ...
        Set<Integer> lineageToExclude = new HashSet<Integer>();
        for (Integer parentId : toBeRetired) {
            for (Integer childId : Terms.get().getRefsetHelper(getConfig()).getChildrenOfConcept(parentId, config.getViewCoordinate())) {
    ...
    /**
     * Check for a is_a relationship type defined on the member refset concept otherwise default to just using
