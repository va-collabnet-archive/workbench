...
public class RefsetHelper extends RefsetUtilities implements I_HelpRefsets {
    ...
    @Override
    public List<Integer> getChildrenOfConcept(int conceptId, ViewCoordinate vc) throws IOException, Exception {
        ConceptVersionBI concept = Ts.get().getConceptVersion(vc, conceptId);
        Collection<? extends ConceptVersionBI> children = concept.getRelationshipsIncomingSourceConceptsActiveIsa();
        List<Integer> childNids = new ArrayList<>(children.size());
