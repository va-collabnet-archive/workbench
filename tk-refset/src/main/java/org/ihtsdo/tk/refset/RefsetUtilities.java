...
public abstract class RefsetUtilities extends LineageHelper implements
    ...
    public boolean isValidStatus(I_ConceptAttributeTuple att)
        ...
        return att.getStatusNid() == termFactory.getConcept(
                ...
                || att.getStatusNid() == termFactory.getConcept(
                ArchitectonicAuxiliary.Concept.DO_NOT_EDIT_FOR_RELEASE.getUids()).getConceptNid();
    }
    ...
    @Override
    public List<Integer> getChildrenOfConcept(int conceptId, ViewCoordinate vc)
    ...
    public List<Integer> getSpecificationRefsets() throws Exception {
        ...
        if (this.altIsA == null) {
            is_a.add(termFactory.getConcept(
                    ArchitectonicAuxiliary.Concept.IS_A_REL.getUids()).getConceptNid());
            is_a.add(termFactory.getConcept(
                    ConceptConstants.SNOMED_IS_A.getUuids()).getConceptNid());
        } else {
        ...
        Set<? extends I_GetConceptData> refsetChildren = refsetRoot.getDestRelOrigins(status, is_a, null,
                getConfig().getPrecedence(), getConfig().getConflictResolutionStrategy());
        for (I_GetConceptData refsetConcept : refsetChildren) {
            ...
            for (I_RelVersioned rel : rels) {
                ...
                for (I_RelTuple tuple : tuples) {
                    if (tuple.getStatusNid() == termFactory.getConcept(
                            ...
                            RefsetAuxiliary.Concept.REFSET_PURPOSE_REL.getUids()).getConceptNid()) {
            ...
            if (purposeConcepts.size() == 1) {
                ...
                if (purposeConcepts.iterator().next().getConceptNid() == termFactory.getConcept(
                        RefsetAuxiliary.Concept.INCLUSION_SPECIFICATION.getUids()).getConceptNid()) {
    ...
    public I_IntSet getIntSet(ArchitectonicAuxiliary.Concept... concepts)
        ...
        for (ArchitectonicAuxiliary.Concept concept : concepts) {
            status.add(termFactory.getConcept(concept.getUids()).getConceptNid());
        }
    ...
    protected I_ExtendByRef getExtensionForComponent(int conceptId,
        ...
        List<? extends I_ExtendByRef> exts = termFactory.getAllExtensionsForComponent(conceptId);
