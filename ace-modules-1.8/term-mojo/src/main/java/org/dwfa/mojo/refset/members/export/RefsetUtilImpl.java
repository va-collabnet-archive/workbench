...
public final class RefsetUtilImpl implements RefsetUtil {
    ...
    public I_ThinExtByRefPart getLatestVersion(final I_ThinExtByRefVersioned ext, final I_TermFactory termFactory)
    ...
    public I_ThinExtByRefPart getLatestVersionIfCurrent(final I_ThinExtByRefVersioned ext,
        ...
        if (latest != null && !(latest.getStatusId() == currentStatusId)) {
            latest = null;
        }
    ...
    public I_IntSet createCurrentStatus(final I_TermFactory termFactory) throws Exception {
        return createIntSet(termFactory, 
                ArchitectonicAuxiliary.Concept.CURRENT.localize().getUids());
    }

    public I_IntSet createFullySpecifiedName(final I_TermFactory termFactory) throws Exception {
        return createIntSet(termFactory, 
                ArchitectonicAuxiliary.Concept.FULLY_SPECIFIED_DESCRIPTION_TYPE.localize().getUids());
    }

    public I_IntSet createPreferredTerm(final I_TermFactory termFactory) throws Exception {
        return createIntSet(termFactory, 
                ArchitectonicAuxiliary.Concept.PREFERRED_DESCRIPTION_TYPE.localize().getUids());
    }
