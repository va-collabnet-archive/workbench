...
/**
 * Task which shows or hides the address book.
 * 
 * @author Christine Hill
 * 
 */
@BeanList(specs = { @Spec(directory = "tasks/ide/gui", type = BeanType.TASK_BEAN) })
public class ShowAddressBook extends AbstractTask {
    ...
    private void writeObject(ObjectOutputStream out) throws IOException {
        ...
        out.writeBoolean(showAddressBook);
    }

    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        int objDataVersion = in.readInt();
        if (objDataVersion == dataVersion) {
            showAddressBook = in.readBoolean();
        } else {
    ...
    public boolean getShowAddressBook() {
