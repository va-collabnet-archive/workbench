                ...
                if (sapNids.contains(car.getSapNid())) {
                    for (Rf2File.ConceptsFileFields field : Rf2File.ConceptsFileFields.values()) {
                        switch (field) {
                            ...
                            case EFFECTIVE_TIME:
                                conceptsWriter.write(TimeHelper.formatDateForFile(car.getTime()) + field.seperator);
            ...
            for (DescriptionVersionBI descr : desc.getVersions(vc)) {
                if (sapNids.contains(descr.getSapNid())) {
                    for (Rf2File.DescriptionsFileFields field : Rf2File.DescriptionsFileFields.values()) {
                        switch (field) {
                            case ACTIVE:
                                descriptionsWriter.write(store.getUuidPrimordialForNid(descr.getStatusNid()) + field.seperator);

                                break;

                            case EFFECTIVE_TIME:
                                descriptionsWriter.write(TimeHelper.formatDateForFile(descr.getTime()) + field.seperator);
                            ...
                            case CONCEPT_ID:
                                descriptionsWriter.write(store.getUuidPrimordialForNid(desc.getConceptNid()) + field.seperator);
                            ...
                            case TYPE_ID:
                                descriptionsWriter.write(store.getUuidPrimordialForNid(descr.getTypeNid()) + field.seperator);
                ...
                if (sapNids.contains(rv.getSapNid())) {
                    for (Rf2File.RelationshipsFileFields field : Rf2File.RelationshipsFileFields.values()) {
                        switch (field) {
                            ...
                            case EFFECTIVE_TIME:
                                relationshipsWriter.write(TimeHelper.formatDateForFile(rv.getTime()) + field.seperator);
                            ...
                            case TYPE_ID:
                                relationshipsWriter.write(store.getUuidPrimordialForNid(rv.getTypeNid()) + field.seperator);
