...
public class ProcessMemberTaskLanguageScoped extends ProcessMemberTask {
    ...
    protected void reset(UUID refsetUuid, UUID statusUuid, UUID componentUuid, UUID pathUuid, int version,
            int memberId, UUID acceptabilityUuid, UUID correctnessUuid, UUID degreeOfSynonymyUuid, UUID scopeUuid,
            int priority, UUID tagUuid) {
    ...
    protected ThinExtByRefPartLanguageScoped makeNewPart() throws Exception {
        ...
        int degreeOfSynonymyId = ProcessAceFormatSourcesBerkeley.map.getIntId((UUID) degreeOfSynonymyUuid,
            ProcessAceFormatSourcesBerkeley.aceAuxPath, version);
        int scopeId = ProcessAceFormatSourcesBerkeley.map.getIntId((UUID) scopeUuid,
            ProcessAceFormatSourcesBerkeley.aceAuxPath, version);
        int tagId = ProcessAceFormatSourcesBerkeley.map.getIntId((UUID) tagUuid,
            ProcessAceFormatSourcesBerkeley.aceAuxPath, version);
    ...
    public static void acquire(UUID refsetUuid, UUID statusUuid, UUID componentUuid, UUID pathUuid, int version,
            int memberId, UUID acceptabilityUuid, UUID correctnessUuid, UUID degreeOfSynonymyUuid, UUID scopeUuid,
            int priority, UUID tagUuid) throws Exception {
        check();
        semaphore.acquire();
        ...
        for (ProcessMemberTaskLanguageScoped task : taskArray) {
            if (task.isUsable()) {
                task.reset(refsetUuid, statusUuid, componentUuid, pathUuid, version, memberId, acceptabilityUuid,
                    correctnessUuid, degreeOfSynonymyUuid, scopeUuid, priority, tagUuid);
