...
public class IsaCoordinate implements Serializable {
    ...
    /**
     * cached value so that viewPositionSet does not have to be recreated each
    ...
    public IsaCoordinate(PositionBI viewPosition, NidSetBI allowedStatus, NidSetBI relTypeNids,
        ...
        try {
            if (allowedStatus.contains(SnomedMetadataRfx.getSTATUS_RETIRED_NID())) {
                allowedStatus.remove(SnomedMetadataRfx.getSTATUS_RETIRED_NID());
                System.out.println("Warning: Profile allowed status list contained the retired status, it was self healed.");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
