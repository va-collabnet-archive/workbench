...
public class TkRefsetCidCidCidMember extends TkRefsetAbstractMember<TkRefsetCidCidCidRevision> {
    ...
    public TkRefsetCidCidCidMember(RefexChronicleBI another) throws IOException {
        this((RefexCnidCnidCnidVersionBI) another.getPrimordialVersion(), RevisionHandling.INCLUDE_REVISIONS);
    }

    public TkRefsetCidCidCidMember(RefexCnidCnidCnidVersionBI another,
        ...
        if (revisionHandling == RevisionHandling.EXCLUDE_REVISIONS) {
            ...
            this.c3Uuid = ts.getUuidPrimordialForNid(another.getCnid3());
        } else {
            ...
            if (partCount > 1) {
                ...
                while (itr.hasNext()) {
                    rv = itr.next();
                    TkRefsetCidCidCidRevision rev = new TkRefsetCidCidCidRevision(rv);
                    if (rev.getTime() == this.time) {
                        ...
                        this.c2Uuid = rev.c2Uuid;
                        this.c3Uuid = rev.c3Uuid;
                    } else {
                        revisions.add(rev);
                    }
                }
    ...
    public void readExternal(DataInput in, int dataVersion) throws IOException, ClassNotFoundException {
        ...
        if (versionSize > 0) {
            ...
            for (int i = 0; i < versionSize; i++) {
                TkRefsetCidCidCidRevision rev = new TkRefsetCidCidCidRevision(in, dataVersion);
                if (rev.getTime() == this.time) {
                    // TODO this check can be removed after trek-95 change sets are no longer in production. 
                    c1Uuid = rev.c1Uuid;
                    c2Uuid = rev.c2Uuid;
