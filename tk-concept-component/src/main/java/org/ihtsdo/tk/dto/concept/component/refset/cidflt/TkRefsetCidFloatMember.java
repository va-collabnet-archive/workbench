...
public class TkRefsetCidFloatMember extends TkRefsetAbstractMember<TkRefsetCidFloatRevision> {
    ...
    public TkRefsetCidFloatMember(RefexChronicleBI another) throws IOException {
         this((RefexCnidFloatVersionBI) another.getPrimordialVersion(), RevisionHandling.INCLUDE_REVISIONS);
    }

    public TkRefsetCidFloatMember(RefexCnidFloatVersionBI another,
        ...
        if (revisionHandling == RevisionHandling.EXCLUDE_REVISIONS) {
            ...
            if (partCount > 1) {
                ...
                while (itr.hasNext()) {
                    rv = itr.next();
                    TkRefsetCidFloatRevision rev = new TkRefsetCidFloatRevision(rv);
                    if (rev.getTime() == this.time) {
                        ...
                        this.floatValue = rev.floatValue;
                    } else {
                        revisions.add(rev);
                    }
                }
    ...
    public void readExternal(DataInput in, int dataVersion) throws IOException, ClassNotFoundException {
        ...
        if (versionSize > 0) {
            ...
            for (int i = 0; i < versionSize; i++) {
                TkRefsetCidFloatRevision rev = new TkRefsetCidFloatRevision(in, dataVersion);
                if (rev.getTime() == this.time) {
                    // TODO this check can be removed after trek-95 change sets are no longer in production. 
                    c1Uuid = rev.c1Uuid;
                    floatValue = rev.floatValue;
                } else {
                    revisions.add(rev);
                }
            }
