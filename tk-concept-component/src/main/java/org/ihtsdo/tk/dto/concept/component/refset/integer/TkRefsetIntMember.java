...
public class TkRefsetIntMember extends TkRefsetAbstractMember<TkRefsetIntRevision> {
    ...
    public TkRefsetIntMember(RefexChronicleBI another) throws IOException {
        this((RefexIntVersionBI) another.getPrimordialVersion(), RevisionHandling.INCLUDE_REVISIONS);
    }

    public TkRefsetIntMember(RefexIntVersionBI another,
        ...
        if (revisionHandling == RevisionHandling.EXCLUDE_REVISIONS) {
            ...
            if (partCount > 1) {
                ...
                while (itr.hasNext()) {
                    rv = itr.next();
                    TkRefsetIntRevision rev = new TkRefsetIntRevision(rv);
    ...
    public void readExternal(DataInput in, int dataVersion) throws IOException, ClassNotFoundException {
        ...
        if (versionSize > 0) {
            ...
            for (int i = 0; i < versionSize; i++) {
                TkRefsetIntRevision rev = new TkRefsetIntRevision(in, dataVersion);
                if (rev.getTime() == this.time) {
                    // TODO this check can be removed after trek-95 change sets are no longer in production. 
