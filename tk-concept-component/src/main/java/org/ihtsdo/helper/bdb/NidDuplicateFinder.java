...
/**
 * The Class NidDuplicateFinder processes every concept in the database to determine
 * if the same nid is used to identify more than one concept or component. This
 * class implements
 ...
 *
 */
public class NidDuplicateFinder implements ProcessUnfetchedConceptDataBI {

    private AtomicInteger count = new AtomicInteger(0);
    private AtomicInteger dots = new AtomicInteger(0);
    ConcurrentSkipListSet<Integer> allNids = new ConcurrentSkipListSet<Integer>();
    ConcurrentSkipListSet<Integer> dupNids = new ConcurrentSkipListSet<Integer>();
    File dupsFile = new File("nid-dups.oos");
     ...
     */
    public NidDuplicateFinder() throws IOException, ClassNotFoundException {
     ...
     * @throws IOException signals that an I/O exception has occurred
     */
    private void addToNidList(ComponentChronicleBI component) throws IOException {
        if (component != null) {
            int nid = component.getNid();

            if (allNids.contains(nid)) {
                dupNids.add(nid);
            } else {
                allNids.add(nid);
            }
    ...
    private void processConcept(ConceptChronicleBI concept) throws IOException {
        ...
        // concept attributes
        addToNidList(concept.getConceptAttributes());
     ...
     */
    public ConcurrentSkipListSet<Integer> getDupNids() {
        return dupNids;
    }
