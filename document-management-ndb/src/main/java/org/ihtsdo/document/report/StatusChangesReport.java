...
public class StatusChangesReport implements I_Report {

	private SimpleDateFormat formatter;
	private I_TermFactory tf;

	public StatusChangesReport() {
		super();
	...
	public File getCsv() throws Exception {
		...
		try {
			...
			pw.append("project|workset|worklist|term|date|status|author");
			pw.println();
								...
								try {
									...
									if (history != null) {
										...
										for (I_ExtendByRefPartCid part : parts) {
											I_GetConceptData cid1 = tf.getConcept(part.getC1id());
											System.out.println(formatter.format(new Date(part.getTime())));
										}
									}
							...
							for (WorkListMember workListMember : keys) {
								...
								for (I_ExtendByRefPartCid part : parts) {
									...
									if(!concept.getUids().iterator().next().equals(ArchitectonicAuxiliary.Concept.WORKLIST_ITEM_ASSIGNED_STATUS.getUids().iterator().next())
										...
										pw.append(formatter.format(new Date(part.getTime())) + "|");
										pw.append(concept + "|");
										pw.append(tf.getConcept(part.getAuthorNid()) + "");
										pw.println();
									}
								}
							}
	...
	public String toString() {
		return "Deprecated Accumulated status canges report";
	}
}
