...
public class SnoPathProcessCondorStated implements I_ProcessConcepts {
    ...
    public SnoPathProcessCondorStated(
            Logger logger,
            List<SnoCon> snocons,
            List<SnoRel> snorels,
            I_IntSet roleSet,
...
//        }
    ...
    private List<SnoRel> tupleToSnoRel(List<? extends I_RelTuple> relTupList) throws Exception {
        ...
        for (I_RelTuple rt : relTupList) {
            c1 = rt.getC1Id();
            if (rt.getAuthorNid() == snorocketAuthorNid ||
                    rt.getCharacteristicNid() != SnomedMetadataRfx.getREL_CH_STATED_RELATIONSHIP_NID()
                    ) // filter out classifier as user. must also be stated relationship
            {
                continue;
            }
