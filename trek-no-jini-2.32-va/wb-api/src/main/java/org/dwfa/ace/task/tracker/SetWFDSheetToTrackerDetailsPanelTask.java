...
/**
 * This task prepares the Workflow Details Sheet to display the TrackerDetailsPanel
 * where the user will be asked to enter a number of fields required to start the
 * Download Assignment from Tracker process.
 */
@BeanList(specs = { @Spec(directory = "tasks/refset/spec/wf/sme", type = BeanType.TASK_BEAN) })
public class SetWFDSheetToTrackerDetailsPanelTask extends AbstractTask {
    ...
    private void doRun(final I_EncodeBusinessProcess process, final I_Work worker) {
        ...
        try {
            ...
            TrackerDetailsPanel newPanel = new TrackerDetailsPanel();
