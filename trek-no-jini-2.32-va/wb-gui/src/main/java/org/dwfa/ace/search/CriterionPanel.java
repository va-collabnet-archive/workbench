...
public class CriterionPanel extends JPanel {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				for (PropertyDescriptor pd : searchInfo.getPropertyDescriptors()) {
					JLabel editorLabel = new JLabel(getName(pd.getDisplayName()));
					editorLabel.setForeground(new Color(9, 100, 10));
					...
					editorComponent.setBorder(BorderFactory.createEmptyBorder(2, 2, 2, 2));
				...
				CriterionPanel.this.revalidate();
			} catch (Exception ex) {
	...
	public CriterionPanel(I_MakeCriterionPanel searchPanel, I_TestSearchResults beanToSet) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
	...
	public CriterionPanel(I_MakeCriterionPanel searchPanel, I_TestSearchResults beanToSet, List<I_TestSearchResults> criterionOptions) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
		super(new GridBagLayout());
		GridBagConstraints gbc = new GridBagConstraints();
	...
	public CriterionPanel(I_MakeCriterionPanel searchPanel) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
		this(searchPanel, null);
	}

	@SuppressWarnings("unchecked")
	private void setupCriterionOptions(List<I_TestSearchResults> criterionOptions) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
		...
		if (criterionOptions == null || criterionOptions.isEmpty()) {
			...
			if (searchPlugins != null) {
				...
				AceLog.getAppLog().alertAndLogException(this, new Exception("No search plugins in folder: " + searchPluginFolder.getAbsolutePath()));
			}
		}
		for (I_TestSearchResults optionBean : criterionOptions) {
			try {
				String searchInfoClassName = optionBean.getClass().getName() + "SearchInfo";
				Class<BeanInfo> searchInfoClass = (Class<BeanInfo>) optionBean.getClass().getClassLoader().loadClass(searchInfoClassName);
