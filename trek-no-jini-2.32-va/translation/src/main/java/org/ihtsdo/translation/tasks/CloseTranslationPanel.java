			...
			if (tp!=null){
				...
				for (int i=0;i<tabCount;i++){
					if (tp.getTitleAt(i).equals(TranslationHelperPanel.TRANSLATION_LEFT_MENU)){
						WfInboxPanel inbox=(WfInboxPanel) tp.getComponentAt(i);
						inbox.refresh();
						openInbox=true;
						break;
					}
					...
					for (int i=0;i<tabCount;i++){
						if (tpc.getTitleAt(i).equals(TranslationHelperPanel.TRANSLATION_TAB_NAME)){
							if (tpc.getComponentAt(i) instanceof TranslationView){
								...
								TranslationView uiPanel=(TranslationView)tpc.getComponentAt(i);
								if ((workListMember==null) || (workListMember!=null)){
									tpc.remove(i);
									tpc.repaint();
									tpc.revalidate();
									break;
