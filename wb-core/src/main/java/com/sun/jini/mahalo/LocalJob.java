...
public abstract class LocalJob {
    ...
    /**
     * Create the <code>Job</code> object giving it the <code>TaskManager</code>
     ...
     */
    public LocalJob(TaskManager pool, WakeupManager wm) {
        this.wm = wm;
        this.pool = pool;
    }
    ...
    public void scheduleTasks() {
        ...
        if (tmp != null) {
            if (logger.isLoggable(Level.INFO)) {
                logger.log(Level.INFO, "Job:scheduleTasks with {0} tasks", new Integer(tmp.length));
            }
            ...
            for (int i = 0; i < tmp.length; i++) {
                ...
                synchronized (tasks) {
                    ...
                    pool.add(tmp[i]);
                    if (logger.isLoggable(Level.INFO)) {
                        logger.log(Level.INFO, "Job:scheduleTasks added {0} to thread pool", tmp[i]);
                    }
                    attempts[i] = 0;
                }
            }
    ...
    private synchronized void awaitPending(long waitFor) {
        ...
        try {
            if (logger.isLoggable(Level.INFO)) {
                logger.log(Level.INFO, "Job:awaitPending waiting for {0} items", new Integer(pending));
            }
            ...
            if (waitFor == Long.MAX_VALUE) {
                while (pending > 0) {
                    wait();
                    if (logger.isLoggable(Level.INFO)) {
                        logger.log(Level.INFO, "Job:awaitPending awoken");
                    }
                }
            } else {
    ...
    private synchronized void setPending(int num) {
        ...
        if (pending <= 0) {
            if (logger.isLoggable(Level.INFO)) {
                logger.log(Level.INFO, "Job:setPending notifying, pending = {0}", new Integer(pending));
            }
            notifyAll();
        }
    }
    ...
    private synchronized void decrementPending() {
        ...
        if (pending <= 0) {
            if (logger.isLoggable(Level.INFO)) {
                logger.log(Level.INFO, "Job:decrementPending notifying, pending = {0}", new Integer(pending));
            }
            notifyAll();
        }
    }
    ...
    private void reportDone(TaskManager.Task who, Object param) throws JobException {
        ...
        synchronized (results) {
            if (results[position.intValue()] == null) {
                if (logger.isLoggable(Level.INFO)) {
                    logger.log(Level.INFO, "Job:reportDone who = {0}, param = {1}", new Object[] { who, param });
                }
