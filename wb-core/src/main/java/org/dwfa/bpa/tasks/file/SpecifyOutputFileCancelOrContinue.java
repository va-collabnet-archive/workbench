...
/**
 * Added new version of Specifiy Output File that allows two return conditions - continue (normal execution) and
 * item_cancelled, which occurs if the user didn't select a file and pressed the cancel button instead. This can be used
 * in cases where it is valid for the user to not select a file and instead opt to cancel the file chooser (and the
 * desired behaviour is that the BP continues and doesn't throw a task failed exception).
 * 
 * @author Christine Hill
 * 
 */
@BeanList(specs = { @Spec(directory = "tasks/file", type = BeanType.TASK_BEAN) })
public class SpecifyOutputFileCancelOrContinue extends AbstractTask {
        ...
        try {
            SwingUtilities.invokeAndWait(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        String defaultFileName = (String) process.getProperty(outputFilePropName);
