...
public class ImageTableRenderer extends AceTableRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value,
        ...
        if (StringWithImageTuple.class.isAssignableFrom(value.getClass())) {
            setIcon(null);
            setupStringWithTuple(table, value, isSelected, row, column,
                renderComponent, same);
        } else if (ImageWithImageTuple.class.isAssignableFrom(value.getClass())) {
            ImageWithImageTuple icon = (ImageWithImageTuple) value;
            boolean uncommitted =
                    icon.getTuple().getVersion() == Integer.MAX_VALUE;
            renderComponent.setIcon(icon.getImage());
            renderComponent.setText("");
            if (row > 0) {
                ImageWithImageTuple prevImage =
                        (ImageWithImageTuple) table.getValueAt(row - 1, column);
                same =
                        icon.getTuple().getImageId() == prevImage.getTuple()
                            .getImageId();
                if (same) {
            ...
            if ((!same)
                && (icon.getImage().getIconHeight() > table.getRowHeight(row))) {
    ...
    private void setupStringWithTuple(JTable table, Object value,
            boolean isSelected, int row, int column, JLabel renderComponent,
            boolean same) {
        StringWithImageTuple swt = (StringWithImageTuple) value;
        boolean uncommitted = swt.getTuple().getVersion() == Integer.MAX_VALUE;
        if (row > 0) {
            StringWithImageTuple prevSwt =
                    (StringWithImageTuple) table.getValueAt(row - 1, column);
            same =
                    swt.getTuple().getImageId() == prevSwt.getTuple()
                        .getImageId();
        ...
        if (BasicHTML.isHTMLString(renderComponent.getText())) {
            View v =
                    BasicHTML
                        .createHTMLView(renderComponent, swt.getCellText());
