...
public abstract class ReflexiveTableModel extends AbstractTableModel implements
	...
	public static class ConceptFieldEditor extends DefaultCellEditor {
		...
		public ConceptBean getSelectedItem(Object value) {
			...
			if (field.getType() == REFSET_FIELD_TYPE.CONCEPT_IDENTIFIER) {
				try {
					return ConceptBean.get((Integer) field.getReadMethod()
							.invoke(swet.getTuple()));
				} catch (Exception e) {
					throw new RuntimeException(e);
				}
			}
	...
	protected class ReferencedConceptsSwingWorker extends
	...
	public ReflexiveTableModel(I_HostConceptPlugins host,
		...
		try {
			...
			case CONCEPT_COMPONENT:
				...
				if (columns[columnIndex].readParamaters != null) {
					value = columns[columnIndex].getReadMethod().invoke(
							ConceptBean.get(tuple.getComponentId()),
			...
			case CONCEPT_IDENTIFIER:
				if (Integer.class.isAssignableFrom(value.getClass())) {
					...
					return new StringWithExtTuple(Integer.toString(conceptId),
							tuple, id);
				} else if (ThinDescTuple.class.isAssignableFrom(value.getClass())) {
					ThinDescTuple descTuple = (ThinDescTuple) value;
					return new StringWithExtTuple(descTuple.getText(),
							tuple, descTuple.getConceptId());
				}
	...
	public void setValueAt(Object value, int row, int col) {
		if (columns[col].isCreationEditable()
				|| columns[col].isUpdateEditable()) {
			...
			if (extTuple.getVersion() == Integer.MAX_VALUE) {
				...
				case CONCEPT_IDENTIFIER:
					...
					referencedConcepts.put(identifier, ConceptBean
							.get(identifier));
				default:
					try {
						...
						case CONCEPT_COMPONENT:
							break;
						case IMMUTABLE:
							columns[col].getWriteMethod().invoke(extTuple,
									value);
							changed = true;
							break;
						case PART:
							columns[col].getWriteMethod().invoke(
									extTuple.getPart(), value);
							changed = true;
							break;
						default:
							throw new UnsupportedOperationException(
	...
	protected abstract I_ChangeTableInSwing getTableChangedSwingWorker(
