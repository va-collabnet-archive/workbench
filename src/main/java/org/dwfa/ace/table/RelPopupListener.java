...
public class RelPopupListener extends MouseAdapter {
        ...
        for (I_Path p : config.getEditingPathSet()) {
          ...
          if ((srcRel != null) && (destRel != null)) {
            ...
            AceLog.getAppLog().alertAndLogException(
                new Exception("srcRel: " + srcRel + " destRel: " + destRel
                    + " cannot be null"));
          }
        }
  ...
  public RelPopupListener(JTable table, I_ConfigAceFrame config,
      RelTableModel model) {
    super();
    this.table = table;
    this.config = config;
  ...
  private void makePopup(MouseEvent e) {
    try {
      ...
      if ((row != -1) && (column != -1)) {
        ...
        addSubmenuItems(changeType, FieldToChange.TYPE,
            model.host.getConfig().getEditRelTypePopup());
  ...
  private void addSubmenuItems(JMenu menu, FieldToChange field,
    ...
    for (int id : possibleValues.getListValues()) {
      I_GetConceptData possibleValue = LocalVersionedTerminology.get().getConcept(
          id);
