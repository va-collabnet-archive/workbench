...
public abstract class TupleAdder<V, W> {
	private class AdmitAndSortParts implements
			Comparator<I_AmPart> {
		...
		boolean admit(I_AmPart part) {
			for (PathSortInfo psi : pathInfo) {
				if (part.getPathId() == psi.pathId
						&& part.getVersion() <= psi.maxVersion) {
					return true;
				}
			}
			return false;
	...
	private class SortPartsByVersion implements
			Comparator<I_AmPart> {
	...
	public void addPaths(ArrayList<PathSortInfo> pathInfo, int depth,
			int maxVersion, I_Path p) {
	...
	public void addTuples(I_IntSet allowedStatus, I_IntSet allowedTypes,
		...
		SortedSet<I_AmPart> sortedPartsToAdd = new TreeSet<I_AmPart>(
				new SortPartsByVersion());
