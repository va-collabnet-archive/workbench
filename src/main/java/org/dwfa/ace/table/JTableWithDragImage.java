    ...
    private class DragGestureListenerWithImage implements DragGestureListener {
        ...
        public void dragGestureRecognized(DragGestureEvent dge) {
            ...
            if (row > -1) {
                ...
                Point imageOffset =
                        new Point(-10, -(dragImage
                            .getHeight(JTableWithDragImage.this) + 1));
                try {
        ...
        private Transferable getTransferable(Object obj, int column)
            ...
            if (I_CellTextWithTuple.class.isAssignableFrom(obj.getClass())) {
                ...
                } else if (StringWithExtTuple.class.isAssignableFrom(obj
                    .getClass())) {
            ...
            AceLog.getAppLog().info(
                "Can't create transferable for: " + obj.getClass());
            return null;
        }
        ...
        private Transferable transferableFromSWExtT(Object obj, int column) {
            ...
            if (ReflexiveRefsetFieldData.class
                .isAssignableFrom(columnIdentifier.getClass())) {
                ReflexiveRefsetFieldData fieldData =
                        (ReflexiveRefsetFieldData) columnIdentifier;
                switch (fieldData.getType()) {
                case COMPONENT_IDENTIFIER:
                    throw new UnsupportedOperationException();
                case CONCEPT_IDENTIFIER:
                    return new ConceptTransferable(ConceptBean.get(swextt
                        .getId()));
                case STRING:
                ...
                default:
                    throw new UnsupportedOperationException(
                        "Can't handle fieldDataType: " + fieldData.getType());
                }

            } else if (REFSET_FIELDS.class.isAssignableFrom(columnIdentifier
                .getClass())) {
                ...
                case BOOLEAN_VALUE:
                    return new StringSelection(swextt.getCellText());
                case CONCEPT_ID:
                    return new ConceptTransferable(ConceptBean
                        .get(((I_ThinExtByRefPartConcept) swextt.getTuple()
                            .getPart()).getConceptId()));
                case INTEGER_VALUE:
                    return new StringSelection(swextt.getCellText());
                case ACCEPTABILITY:
                    return new ConceptTransferable(ConceptBean
                        .get(((I_ThinExtByRefPartLanguage) swextt.getTuple()
                ...
                case TAG:
                    return new ConceptTransferable(ConceptBean
                        .get(((I_ThinExtByRefPartLanguageScoped) swextt
                ...
                case MEASUREMENT_UNITS_ID:
                    return new ConceptTransferable(ConceptBean
                        .get(((I_ThinExtByRefPartMeasurement) swextt.getTuple()
                            .getPart()).getUnitsOfMeasureId()));
                case STRING_VALUE:
                    return new StringSelection(swextt.getCellText());
                default:
                    throw new UnsupportedOperationException(
                        "Can't handle field: " + field);
                }
            }
            throw new UnsupportedOperationException(
                "Can't handle columnIdentifier: " + columnIdentifier);
        ...
        private Transferable TransferableFromSWIdT(Object obj, int column) {
            ...
            ID_FIELD field =
                    (ID_FIELD) getColumnModel().getColumn(column)
                        .getIdentifier();
            switch (field) {
            case LOCAL_ID:
                throw new UnsupportedOperationException();
            case EXT_ID:
                return new StringSelection(seidt.getTuple().getSourceId()
                    .toString());
            case STATUS:
                return new ConceptTransferable(ConceptBean.get(seidt.getTuple()
                    .getStatusId()));
            case VERSION:
                return new StringSelection(ThinVersionHelper.format(seidt
                    .getTuple().getVersion()));
            case PATH:
                return new ConceptTransferable(ConceptBean.get(seidt.getTuple()
                    .getPathId()));
            default:
                throw new UnsupportedOperationException("Cana't handle field: "
                    + field);
            }
        }
        ...
        private Transferable TransferableFromSWRT(Object obj, int column) {
            ...
            REL_FIELD field =
                    (REL_FIELD) getColumnModel().getColumn(column)
                        .getIdentifier();
            switch (field) {
            case REL_ID:
                throw new UnsupportedOperationException();
            case SOURCE_ID:
                return new ConceptTransferable(ConceptBean.get(swrt.getTuple()
                    .getC1Id()));
            case REL_TYPE:
                return new ConceptTransferable(ConceptBean.get(swrt.getTuple()
                    .getTypeId()));
            case DEST_ID:
                return new ConceptTransferable(ConceptBean.get(swrt.getTuple()
                    .getC2Id()));
            case GROUP:
                return new StringSelection(Integer.toString(swrt.tuple
                    .getGroup()));
            case REFINABILITY:
                return new ConceptTransferable(ConceptBean.get(swrt.getTuple()
                    .getRefinabilityId()));
            ...
            case STATUS:
                return new ConceptTransferable(ConceptBean.get(swrt.getTuple()
                    .getStatusId()));
            case VERSION:
                return new StringSelection(ThinVersionHelper.format(swrt
                    .getTuple().getVersion()));
            case PATH:
                return new ConceptTransferable(ConceptBean.get(swrt.getTuple()
                    .getPathId()));
        ...
        private Transferable TransferableFromSWDT(Object obj, int column) {
            ...
            DESC_FIELD field =
                    (DESC_FIELD) getColumnModel().getColumn(column)
                        .getIdentifier();
            switch (field) {
            case DESC_ID:
                throw new UnsupportedOperationException();
            case CON_ID:
                return new ConceptTransferable(ConceptBean.get(swdt.getTuple()
                    .getConceptId()));
            case TEXT:
            ...
            case TYPE:
                return new ConceptTransferable(ConceptBean.get(swdt.getTuple()
                    .getTypeId()));
            case VERSION:
                return new StringSelection(ThinVersionHelper.format(swdt
                    .getTuple().getVersion()));
            case PATH:
                return new ConceptTransferable(ConceptBean.get(swdt.getTuple()
                    .getPathId()));
        ...
        private Transferable TransferableFromSWCT(Object obj, int column) {
            ...
            CONCEPT_FIELD field =
                    (CONCEPT_FIELD) getColumnModel().getColumn(column)
                        .getIdentifier();
            switch (field) {
            case CON_ID:
                return new ConceptTransferable(ConceptBean.get(swct.getTuple()
                    .getConId()));
            case STATUS:
                return new ConceptTransferable(ConceptBean.get(swct.getTuple()
                    .getConceptStatus()));
            ...
            case VERSION:
                return new StringSelection(ThinVersionHelper.format(swct
                    .getTuple().getVersion()));
            case PATH:
                return new ConceptTransferable(ConceptBean.get(swct.getTuple()
                    .getPathId()));
        ...
        private Transferable transferableFromIWImgT(Object obj)
                throws DatabaseException {
        ...
        private Transferable transferableFromSWImgT(Object obj, int column)
            ...
            IMAGE_FIELD field =
                    (IMAGE_FIELD) getColumnModel().getColumn(column)
                        .getIdentifier();
            switch (field) {
            case IMAGE_ID:
                return new StringSelection("<img src='ace:"
                    + AceConfig.getVodb().nativeToUuid(
                        swit.getTuple().getImageId())
                    + "$"
                    + AceConfig.getVodb().nativeToUuid(
                        swit.getTuple().getConceptId()) + "'>");
            case CON_ID:
                return new ConceptTransferable(ConceptBean.get(swit.getTuple()
                    .getConceptId()));
            case DESC:
            ...
            case TYPE:
                return new ConceptTransferable(ConceptBean.get(swit.getTuple()
                    .getTypeId()));
            case VERSION:
                return new StringSelection(ThinVersionHelper.format(swit
                    .getTuple().getVersion()));
            case PATH:
                return new ConceptTransferable(ConceptBean.get(swit.getTuple()
                    .getPathId()));
            default:
                throw new UnsupportedOperationException("Can't handle field: "
                    + field);
            }
        }
        ...
        public Image getDragImage(Object obj) {
            ...
            if (I_CellTextWithTuple.class.isAssignableFrom(obj.getClass())) {
                ...
                Dimension size =
                        new Dimension(iwit.getImage().getIconWidth(), iwit
            ...
            FilteredImageSource fis =
                    new FilteredImageSource(dragImage.getSource(),
