...
/**
 * <h1>VerifyValidIsADups</h1>
 * <br>
 * <p>The <code>VerifyValidIsNotADups</code> class checks the users view paths for any source </p>
 * <p>concepts that have a status of <code>IS_NOT_A_DUP_REL</code>.</p>
 * <p>If the concept has a <code>IS_NOT_A_DUP_REL</code> relationship, then check to ensure that</p>
 * <p>either the source or destination concept does NOT have a status of <code>DUPLICATE_PENDING_RETIREMENT</code>.</p>
...
public class VerifyValidIsNotADups extends AbstractTask {
    ...
    public Condition evaluate(I_EncodeBusinessProcess process, final I_Work worker) throws TaskFailedException {
        try {
            config = (I_ConfigAceFrame) worker.readAttachement(WorkerAttachmentKeys.ACE_FRAME_CONFIG
                    .name());
...
//            }
//	 		else{
//				selectedConcept = LocalVersionedTerminology.get().getConcept( config.getHierarchySelection().getConceptId() );
//        	}
            ...
            for (I_GetConceptData notDupConcept: selectedConcept.getSourceRelTargets( pendingRetirementStatusSet, 
            	...
            	List<I_ConceptAttributeTuple> statusTuples = notDupConcept.getConceptAttributeTuples(statusTypeSet, config.getViewPositionSet() );
            	
            	for( I_ConceptAttributeTuple tuple : statusTuples )	return Condition.FALSE;
...
//            for (I_GetConceptData child: selectedConcept.getSourceRelTargets( pendingRetirementStatusSet, 
//					isNotDupTypeSet, config.getViewPositionSet(), true ) ) {
//            	return Condition.FALSE;
//            }
//            
//            for (I_GetConceptData child: selectedConcept.getDestRelOrigins( pendingRetirementStatusSet, 
//					isNotDupTypeSet, config.getViewPositionSet(), true ) ) {
//            	return Condition.FALSE;
//            }
            
//            for (I_GetConceptData child: selectedConcept.getSourceRelTargets(config.getAllowedStatus(), 
//					isNotDupTypeSet, config.getViewPositionSet(), true)) {
//            	for(I_GetConceptData grandChild: child.getSourceRelTargets(pendingRetirementStatusSet,null, config.getViewPositionSet(), true)){
//            		return Condition.FALSE;
//            	}
//            	for(I_GetConceptData grandChild: child.getDestRelOrigins(pendingRetirementStatusSet,null, config.getViewPositionSet(), true)){
//            		return Condition.FALSE;
//            	}
