...
public class ClassifyCurrent extends AbstractTask {
	...
	public Condition evaluate(final I_EncodeBusinessProcess process,
		...
		try {
			...
			definingCharacteristic = LocalVersionedTerminology
					.get()
					.getConcept(
			...
			statedCharacteristic = LocalVersionedTerminology.get().getConcept(
					ArchitectonicAuxiliary.Concept.STATED_RELATIONSHIP
							.getUids()).getConceptId();
			currentStatus = LocalVersionedTerminology.get().getConcept(
					ArchitectonicAuxiliary.Concept.CURRENT.getUids())
					.getConceptId();
			limitedStatus = LocalVersionedTerminology.get().getConcept(
	...
	private void processConcept(final I_GetConceptData concept,
			final I_SnorocketFactory rocket) throws IOException,
		...
		I_ConceptAttributeVersioned conceptAttributes = concept
				.getUncommittedConceptAttributes();
		if (null == conceptAttributes) {
			conceptAttributes = concept.getConceptAttributes();
		}

		final List<I_ConceptAttributeTuple> tuples = conceptAttributes
				.getTuples();
		boolean isDefined = false;

		I_ConceptAttributeTuple tu = tuples.get(tuples.size() - 1);
		if (tu.getConceptStatus() == currentStatus
				|| tu.getConceptStatus() == limitedStatus) {
		...
		logger.info("Add concept: " + concept + " : " + concept.getId() + ": "
				+ isDefined);
			...
			for (I_RelVersioned rel : sourceRels) {
				...
				if (definingCharacteristic == lastTuple.getCharacteristicId()
						|| statedCharacteristic == lastTuple
								.getCharacteristicId()) {
					if (lastTuple.getStatusId() == currentStatus
							|| lastTuple.getStatusId() == limitedStatus) {
						logger.info("Add relationship: " + lastTuple.getC1Id()
								+ " " + lastTuple.getRelTypeId() + " "
								+ lastTuple.getC2Id());
						rocket.addRelationship(lastTuple.getC1Id(), lastTuple
								.getRelTypeId(), lastTuple.getC2Id(), lastTuple
								.getGroup());
					}
