...
public class BuildClassificationResultString extends AbstractTask {
	...
	public Condition evaluate(I_EncodeBusinessProcess process, I_Work worker)
		...
		try {
			...
			for (ClassifyResult c: CR)
			{
				...
				{
					if(c.concept1 == d.concept2 && c.reltype == isa && d.reltype==isa
							&& c.concept2  == d.concept1)
					
					{
						equalSting += tf.getConcept(c.concept1).toString() + "---equal--- " +
						 				tf.getConcept(c.concept2) + "<p>";
					}
				}
			}
	...
	private class ProcessResults implements I_SnorocketFactory.I_Callback {
		...
		public ProcessResults(final I_Work worker, StringBuilder sb, ArrayList<ClassifyResult> cr)
				throws Exception {
			this.worker = worker;
			this.sb = sb;
			this.cr = cr;
