...
@BeanList(specs = {
		@Spec(directory = "tasks/ace/search", type = BeanType.TASK_BEAN),
...
public class RelSubsumptionMatch extends AbstractSearchTest {
	 ...
	 */
	private TermEntry relTypeTerm = new TermEntry(
	...
	public boolean test(I_AmTermComponent component,
		...
		try {
			...
			if (relRestrictionTerm != null) {
				relRestrictionToMatch = AceTaskUtil
						.getConceptFromObject(relRestrictionTerm);
			}
			...
			if (I_GetConceptData.class.isAssignableFrom(component.getClass())) {
				...
				testConcept = LocalVersionedTerminology.get().getConcept(
						desc.getConceptId());
			} else {
			...
			if (AceLog.getAppLog().isLoggable(Level.FINE)) {
				AceLog.getAppLog().fine(
						"### testing rel subsumption: " + testConcept);
			}
			...
			for (I_RelTuple tuple : tuples) {
				I_GetConceptData relType = LocalVersionedTerminology.get()
						.getConcept(tuple.getRelTypeId());
				I_GetConceptData relRestriction = LocalVersionedTerminology
						.get().getConcept(tuple.getC2Id());
				if (AceLog.getAppLog().isLoggable(Level.FINE)) {
					AceLog.getAppLog().fine(
							"     relType: " + relType + " relRestriction: "
									+ relRestriction + " relTypeToMatch: "
									+ relTypeToMatch
