...
public class ImportRefsetFromLanguageSubsetFile extends AbstractTask {
    ...
    public Condition evaluate(I_EncodeBusinessProcess process, I_Work worker) throws TaskFailedException {
        ...
        try {
            ...
            memberRefsetHelper = new SpecMemberRefsetHelper(refsetId,
    ...
    private void updateResetExtentions(BatchMonitor monitor, int refsetId, LanguageSubsetMemberLine conceptDescription)
        ...
        int extensionTypeId = ArchitectonicAuxiliary.getSnomedDescriptionType(
            conceptDescription.getDescriptionStatusId()).localize().getNid();
        ...
        I_ThinExtByRefPartConcept currentRefsetExtension = memberRefsetHelper.getCurrentRefsetExtension(refsetId,
            conceptDescription.getDescriptionVersioned().getDescId());
        if (currentRefsetExtension != null) {
            int currentRefsetExtensionType = currentRefsetExtension.getConceptId();
            if (currentRefsetExtensionType != extensionTypeId) {
                // retire the old type.
                memberRefsetHelper.retireRefsetExtension(refsetId, conceptDescription.getDescriptionVersioned()
                    .getDescId(), currentRefsetExtensionType);
                addRetiredExtensionMessage(monitor, currentRefsetExtension);
            }
        }
        // create a new extension if not currently set.
        memberRefsetHelper.newRefsetExtension(refsetId, conceptDescription.getDescriptionVersioned().getDescId(),
            extensionTypeId);
    }
     ...
     */
    private void addDescriptionTypeMismatchMessage(BatchMonitor monitor, I_DescriptionTuple descriptionTuple,
            int extensionTypeId) throws TerminologyException, IOException {
        ...
        processMessages.append(descriptionTuple.getText() + " - "
            + termFactory.getConcept(descriptionTuple.getDescId()).getUids().get(0) + " - " + fileDescriptionType
            + "<br>");

        monitor.setText("<html>" + processMessages + "</html>");

        getLogger().info(
            "Description type missmatch - End - " + descriptionTuple.getText() + " - "
    ...
    private void addRetiredExtensionMessage(BatchMonitor monitor, I_ThinExtByRefPartConcept retiredRefsetExtension)
        ...
        processMessages.append(LocalVersionedTerminology.get()
            .getConcept(retiredRefsetExtension.getConceptId())
            .getUids()
            .get(0)
            + "<br/>");
