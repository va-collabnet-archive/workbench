...
public class RelationshipStatement extends RefsetSpecStatement {
	...
	public boolean getStatementResult(I_AmTermComponent component) 
		...
		I_RelVersioned relVersioned = 
			(I_RelVersioned) component;
		I_RelTuple relTuple = 
			relVersioned.getLastTuple();
		...
		if (queryToken.equals(termFactory.getConcept(
			...
			return relationshipIsMemberOf(relTuple);
				...
				RefsetAuxiliary.Concept.REL_STATUS_IS.getUids()))) {
			return componentStatusIs(relTuple);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.REL_STATUS_IS_KIND_OF.getUids()))) { 
			return componentStatusIsKindOf(relTuple);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.REL_TYPE_IS.getUids()))) {
			return relationshipTypeIs(relTuple);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.REL_TYPE_IS_KIND_OF.getUids()))) {
			return relationshipTypeIsKindOf(relTuple);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.REL_LOGICAL_QUANTIFIER_IS.getUids()))) {
			return relationshipLogicalQuantifierIs(relTuple);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.REL_LOGICAL_QUANTIFIER_IS_KIND_OF.getUids()))) {
			return relationshipLogicalQuantifierIsKindOf(relTuple);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.REL_CHARACTERISTIC_IS.getUids()))) {
			return relationshipCharacteristicIs(relTuple);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.REL_CHARACTERISTIC_IS_KIND_OF.getUids()))) {
			return relationshipCharacteristicIsKindOf(relTuple);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.REL_REFINABILITY_IS.getUids()))) {
			return relationshipRefinabilityIs(relTuple);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.REL_REFINABILITY_IS_KIND_OF.getUids()))) {
	...
	private boolean relationshipIsMemberOf(I_RelTuple rel) throws 
	...
	private boolean relationshipTypeIs(I_GetConceptData relType, 
			I_RelTuple rel) {
