...
public class ConceptStatement extends RefsetSpecStatement {
	...
	public boolean getStatementResult(I_AmTermComponent component) throws TerminologyException, IOException {
		...
		if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.CONCEPT_IS.getUids()))) {
			...
			return conceptIsMemberOf(concept);
				...
				RefsetAuxiliary.Concept.CONCEPT_IS_KIND_OF.getUids()))) { 
			return conceptIsKindOf(concept);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.CONCEPT_STATUS_IS.getUids()))) {
			return conceptStatusIs(concept);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.CONCEPT_STATUS_IS_KIND_OF.getUids()))) {
			return conceptStatusIsKindOf(concept);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.CONCEPT_IS_CHILD_OF.getUids()))) {
			return conceptIsChildOf(concept);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.CONCEPT_CONTAINS_REL_GROUPING.getUids()))) {
			return conceptContainsRelGrouping(concept);
		} else if (queryToken.equals(termFactory.getConcept(
				RefsetAuxiliary.Concept.CONCEPT_CONTAINS_DESC_GROUPING.getUids()))) {
	 ...
	 * @throws TerminologyException
	 */
	private boolean conceptStatusIs(I_GetConceptData concept, I_GetConceptData statusConcept) throws IOException, TerminologyException {
		List<I_ConceptAttributeTuple> tuples = concept.getConceptAttributeTuples(
				termFactory.getActiveAceFrameConfig().getAllowedStatus(), null, true, true); 
	...
	private boolean conceptStatusIsKindOf(I_GetConceptData concept) throws IOException, TerminologyException {
		...
		Set<I_GetConceptData> childStatuses = concept.getDestRelOrigins(
