...
public class RefsetSpecQuery {
	...
	private int getGroupingTypeFromConcept(I_GetConceptData concept) throws TerminologyException, IOException {
	...
	public RefsetSpecStatement addConceptStatement(boolean useNotQualifier, 
			I_GetConceptData groupingToken, 
			I_GetConceptData constraint) {
		RefsetSpecStatement statement = new ConceptStatement(
				useNotQualifier, groupingToken, constraint);
		statements.add(statement);
		return statement;
	}
	...
	public RefsetSpecStatement addDescStatement(boolean useNotQualifier, 
			I_GetConceptData groupingToken, 
			I_GetConceptData constraint) {
		RefsetSpecStatement statement = new DescStatement(
				useNotQualifier, groupingToken, constraint);
		statements.add(statement);
		return statement;
	}
	...
	public boolean execute(I_AmTermComponent component) throws IOException, 
		...
		switch(groupingType) {
			case (AND_GROUPING):
				...
				for (RefsetSpecQuery subquery : subqueries) {
					if (!subquery.execute(component)) {
						// can exit the AND early, as at least one query is returning false
						return false; 
					}
				}
			...
			case (OR_GROUPING):
				...
				for (RefsetSpecQuery subquery : subqueries) {
					if (subquery.execute(component)) {
						// exit the OR statement early, as at least one query has returned true
						return true; 
					}
				}
			...
			case (CONCEPT_CONTAINS_DESC_GROUPING):
				...
				List<I_DescriptionTuple> descriptionTuples = concept.getDescriptionTuples(
						termFactory.getActiveAceFrameConfig().getAllowedStatus(), 
						null, termFactory.getActiveAceFrameConfig().getViewPositionSet(), true);
