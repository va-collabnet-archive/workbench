...
/**
 * Task for adding a "description status is descendent of" structural query to a refset spec.
 * 
 * @author Chrissy Hill
 * 
 */
@BeanList(specs = { @Spec(directory = "tasks/refset/spec/desc", type = BeanType.TASK_BEAN) })
public class DescriptionStatusIsDescendentOf extends AddStructuralQueryToRefsetSpec {
    /**
	 * 
	 */
    ...
    protected int getStructuralQueryTokenId() throws IOException, TerminologyException {
        return RefsetAuxiliary.Concept.DESC_STATUS_IS_DESCENDENT_OF.localize().getNid();
    }
