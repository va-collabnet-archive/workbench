...
/**
 * Task for adding a "relationship characteristic is" structural query to a refset spec.
 * @author Chrissy Hill
 *
 */
@BeanList(specs = { @Spec(directory = "tasks/refset/spec", type = BeanType.TASK_BEAN) })
public class RelationshipCharacteristicIs extends AddStructuralQueryToRefsetSpec {
	/**
	 * 
	 */
	...
	protected int getStructuralQueryTokenId() throws IOException, TerminologyException {
		return RefsetAuxiliary.Concept.REL_CHARACTERISTIC_IS.localize().getNid();
	}
