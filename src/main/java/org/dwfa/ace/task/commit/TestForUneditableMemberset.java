...
public class TestForUneditableMemberset extends AbstractConceptTest {
    ...
    public List<AlertToDataConstraintFailure> test(I_GetConceptData concept, 
    	...
    	allowedTypes.add(ConceptConstants.GENERATES_REL.localize().getNid());
    	
        try {
            
        	for(I_GetConceptData dest : concept.getDestRelOrigins(null, allowedTypes, null, true)){
    ...
    private List<AlertToDataConstraintFailure> testConcept(I_GetConceptData concept, boolean forCommit) throws TaskFailedException {
    	try{
    		UUID[] ids = new UUID[1];
	    	...
	    	for(I_GetConceptData src : concept.getSourceRelTargets(null, allowedTypes, null, true)){
        		if(src.getConceptId() == refsetConcept.getConceptId()){
        			
        			String alertString = "<html>Uneditable concept:<br> <font color='blue'>" + concept.getInitialText()
