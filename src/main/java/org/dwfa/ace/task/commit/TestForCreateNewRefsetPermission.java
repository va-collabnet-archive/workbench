...
public class TestForCreateNewRefsetPermission extends AbstractConceptTest {
    ...
    public List<AlertToDataConstraintFailure> test(I_GetConceptData concept, boolean forCommit)
        ...
        try {
            ...
            I_ConfigAceDb configDb = termFactory.getActiveAceFrameConfig().getDbConfig();

            I_GetConceptData activeUser = configDb.getUserConcept();

            if (activeUser == null) {
                // alertList.add(new
                // AlertToDataConstraintFailure(AlertToDataConstraintFailure.ALERT_TYPE.WARNING,
                // "<html>Unable to find active user.", concept));
                return alertList;
            }
