...
public class SetSignpostDetailAndInstruction extends AbstractTask {
	...
	private String detailHtmlPropName = ProcessAttachmentKeys.HTML_DETAIL
			.getAttachmentKey();

	private String instructionHtmlPropName = ProcessAttachmentKeys.HTML_INSTRUCTION
			.getAttachmentKey();
		...
		try {
			I_ConfigAceFrame config = (I_ConfigAceFrame) worker
					.readAttachement(WorkerAttachmentKeys.ACE_FRAME_CONFIG
							.name());
			final String detailHtmlStr = (String) process
					.readProperty(detailHtmlPropName);
			final String instructionHtmlStr = (String) process
					.readProperty(instructionHtmlPropName);
			...
			SwingUtilities.invokeAndWait(new Runnable() {
				...
				public void run() {
					...
					tabs.addTab("instruction", new JScrollPane(new JLabel(
							instructionHtmlStr),
							JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
							JScrollPane.HORIZONTAL_SCROLLBAR_NEVER));
					tabs.addTab("detail", new JScrollPane(new JLabel(
							detailHtmlStr),
							JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
							JScrollPane.HORIZONTAL_SCROLLBAR_NEVER));
