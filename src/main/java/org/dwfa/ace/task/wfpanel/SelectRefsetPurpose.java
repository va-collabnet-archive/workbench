...
/**
 * This task enables a user to select the "purpose" of a refset using a ComboBox. 
	...
	public Condition evaluate(I_EncodeBusinessProcess process, final I_Work worker) 
		...
		try {
			...
			if (returnCondition == Condition.CONTINUE) {
				...
				/*  -----------------------------------------------------------------------
				 *  Add relationship to the concept that identifies the current Refset Spec
				 *  -----------------------------------------------------------------------
				 */
...
//				 */
	...
	/**
     * This method overrides a method by the same name in the parent class. It is used
     * to tell the parent class (PreviousNextOrCancel) whether to show
     * the previous button or not.  Since we only want the Next and Cancel buttons, 
	...
	/**
     * This subclass allows the task to perform GUI-related work in a dedicated thread
     * using a SwingWorker.
     * 
     * @author  Perry Reid
     * @version 1.0, October 2009 
...
//				purposeList[0] = LocalVersionedTerminology.get().getConcept(RefsetAuxiliary.Concept.ANNOTATION_PURPOSE.getUids());
//				purposeList[1] = LocalVersionedTerminology.get().getConcept(RefsetAuxiliary.Concept.REFSET_PURPOSE.getUids());
//				purposeList[2] = LocalVersionedTerminology.get().getConcept(RefsetAuxiliary.Concept.ENUMERATED_ANNOTATION_PURPOSE.getUids());

				// Fetch purpose concepts... and place them in an array 
				purposeList[0] = LocalVersionedTerminology.get().getConcept(RefsetAuxiliary.Concept.SIMPLE_COMPONENT.getUids());
				purposeList[1] = LocalVersionedTerminology.get().getConcept(RefsetAuxiliary.Concept.NAVIGATION.getUids());
				// Missing: Reference set descriptor
				purposeList[2] = LocalVersionedTerminology.get().getConcept(RefsetAuxiliary.Concept.ATTRIBUTE_VALUE.getUids());
				// Missing: Simple Map
				purposeList[3] = LocalVersionedTerminology.get().getConcept(RefsetAuxiliary.Concept.ALTERNATIVE_MAP_STATUS.getUids());
				// Missing: Language dialect
				purposeList[4] = LocalVersionedTerminology.get().getConcept(RefsetAuxiliary.Concept.QUERY_SPECIFICATION.getUids());
				purposeList[5] = LocalVersionedTerminology.get().getConcept(RefsetAuxiliary.Concept.ANNOTATION_PURPOSE.getUids());
				// Missing: Association 
				purposeList[6] = LocalVersionedTerminology.get().getConcept(RefsetAuxiliary.Concept.MODULE_DEPENDENCY.getUids());
				purposeList[7] = LocalVersionedTerminology.get().getConcept(RefsetAuxiliary.Concept.DESC_TYPE_IS.getUids());				
			} catch (TerminologyException e) {
				e.printStackTrace();
			} catch (IOException e) {
				e.printStackTrace();
