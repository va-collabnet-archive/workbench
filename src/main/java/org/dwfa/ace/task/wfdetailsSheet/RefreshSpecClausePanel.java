...
public class RefreshSpecClausePanel extends JPanel implements ActionListener {
    ...
    public RefreshSpecClausePanel(I_GetConceptData refsetIdentityConcept, PositionSetReadOnly refsetSpecVersionSet,
        ...
        this.refsetSpec =
                RefsetHelper.getSpecificationRefsetForRefset(refsetIdentityConcept, frameConfig).iterator().next();
        ...
        List<I_ThinExtByRefTuple> tuples =
                member.getTuples(frameConfig.getAllowedStatus(), refsetSpecVersionSet, false, false);
        for (I_ThinExtByRefTuple tuple : tuples) {
            if (tuple.getTypeId() == RefsetAuxiliary.Concept.CONCEPT_CONCEPT_EXTENSION.localize().getNid()) {
                ...
                I_ThinExtByRefPartConceptConceptConcept cccPart =
                        (I_ThinExtByRefPartConceptConceptConcept) tuple.getMutablePart();
    ...
    private void layoutRefreshSpecClausePanel() throws IOException, TerminologyException {
        ...
        List<I_ThinExtByRefTuple> tuples =
                member.getTuples(frameConfig.getAllowedStatus(), frameConfig.getViewPositionSet(), false, false);
        ...
        if (updateOptions.getSelectedItem().equals(REPLACE_OPTION)) {
            ...
            if (replacementConceptLabel == null || replacementConceptLabel.getTermComponent() == null) {
                ...
                int newMemberNid =
                        tf.uuidToNativeWithGeneration(UUID.randomUUID(),
                            ArchitectonicAuxiliary.Concept.UNSPECIFIED_UUID.localize().getNid(), config
                                .getEditingPathSet().iterator().next(), Integer.MAX_VALUE);
                I_Identify newMemberId = tf.getId(newMemberNid);
                I_ThinExtByRefVersioned newMember =
                ...
                for (I_Path p : config.getEditingPathSet()) {
                    ...
                    for (I_ThinExtByRefTuple tuple : tuples) {
                        ...
                        I_ThinExtByRefPart newRetiredPart =
                                (I_ThinExtByRefPart) tuple.getMutablePart().makeAnalog(retiredNid, p.getConceptId(),
                                    Long.MAX_VALUE);
                        ...
                        if (tuple.getTypeId() == RefsetAuxiliary.Concept.CONCEPT_CONCEPT_EXTENSION.localize().getNid()) {
                            ...
                            I_ThinExtByRefPartConceptConcept newCCPart =
                                    (I_ThinExtByRefPartConceptConcept) tuple.getMutablePart().makeAnalog(currentNid,
                            ...
                            .localize().getNid()) {
                            I_ThinExtByRefPartConceptConceptConcept newCCCPart =
                                    (I_ThinExtByRefPartConceptConceptConcept) tuple.getMutablePart().makeAnalog(
                    ...
                    if (writeComment) {
                        ...
                        int commentNid =
                                tf.uuidToNativeWithGeneration(memberUuids,
                                    ArchitectonicAuxiliary.Concept.UNSPECIFIED_UUID.localize().getNid(), p,
                                    Integer.MAX_VALUE);
                        ...
                        if (commentRefset != null && commentId != null) {
                            ...
                            I_ThinExtByRefPartString commentExtPart =
                                    tf.newExtensionPart(I_ThinExtByRefPartString.class);
            ...
            for (I_Path p : config.getEditingPathSet()) {
                ...
                if (writeComment) {
                    ...
                    memberUuids.add(memberUuid);
                    int commentNid =
                            tf.uuidToNativeWithGeneration(memberUuids, ArchitectonicAuxiliary.Concept.UNSPECIFIED_UUID
                                .localize().getNid(), p, Integer.MAX_VALUE);
                    commentId = tf.getId(commentNid);
                    commentRefset.getUncommittedIdVersioned().add(commentId);
                    comment =
