		...
		try {
			if (LocalVersionedTerminology.get().getProperty(jarFile.getName()) != null) {
				AceLog.getAppLog().alertAndLogException(
						new Exception(jarFile.getName()
								+ " has already been processed on: "
								+ LocalVersionedTerminology.get().getProperty(
										jarFile.getName())));
			...
			if (profilesDir.exists()) {
				...
				if (profiles != null) {
					...
					for (File profile : profiles) {
						...
						for (File profileFile : FileIO.recursiveGetFiles(
								profile, prefix, suffix, excludeHidden)) {
							ObjectInputStream ois = new ObjectInputStream(
									new BufferedInputStream(
											new FileInputStream(profileFile)));
							I_ConfigAceDb remoteDbConfig = (I_ConfigAceDb) ois
									.readObject();
							SvnPrompter prompter = new SvnPrompter();
							prompter.prompt("Please authenticate for: "
									+ jarFile.getName(), remoteDbConfig
									.getUsername());

							for (final I_ConfigAceFrame ace : remoteDbConfig
									.getAceFrames()) {
								if (ace.isActive()) {
			...
			if (authenticated) {
				...
				if (inboxesDir.exists()) {
					...
					if (inboxes != null) {
						for (File inbox : inboxes) {
							...
							List<File> bpList = FileIO.recursiveGetFiles(inbox,
									prefix, suffix, excludeHidden);
							for (File bpFile : bpList) {
								if (bpFile.getParentFile().getName().equals(
										".dlog")) {
									File dlogFileRelative = new File(bpFile
											.getAbsolutePath().substring(
													absolutePathPortion));
									File llogFile = new File(new File(
											dlogFileRelative.getParentFile()
													.getParentFile(), ".llog"),
											dlogFileRelative.getName());
									if (llogFile.exists()) {
										llogFile.delete();
									}
								} else if (bpFile.getParentFile().getName()
										.startsWith(".") == false) {
									File bpFileRelative = new File(bpFile
											.getAbsolutePath().substring(
													absolutePathPortion));
									File llogFile = new File(new File(
											bpFileRelative.getParentFile(),
											".llog"), bpFileRelative.getName());
									if ((bpFileRelative.exists() == false)
											&& (llogFile.exists() == false)) {
				...
				if (outboxesDir.exists()) {
					...
					if (outboxes != null) {
						...
						for (File outbox : outboxes) {
							...
							List<File> cFileList = FileIO.recursiveGetFiles(
									outbox, prefix, suffix, excludeHidden);
							for (File cFile : cFileList) {
								if (cFile.getParentFile().getName().equals(
										".clog")) {
									File cFileRelative = new File(cFile
											.getAbsolutePath().substring(
													absolutePathPortion));
									File dlogFileRelative = new File(
											new File(cFileRelative.getParent(),
													".dlog"), cFileRelative
													.getName());
									File llogFileRelative = new File(
											new File(cFileRelative.getParent(),
													".llog"), cFileRelative
													.getName());
									File bpFileRelative = new File(
											cFileRelative.getParent(),
											cFileRelative.getName());
									if (bpFileRelative.exists() == false
											&& dlogFileRelative.exists() == false) {
					...
					if (changeSets != null) {
						...
						for (File changeSet : changeSets) {
							...
							for (String suffix : suffixes) {
								changeSetFiles.addAll(FileIO.recursiveGetFiles(
										changeSet, prefix, suffix,
										excludeHidden));
							}

							for (File csFile : changeSetFiles) {

								File csFileRelative = new File(csFile
										.getAbsolutePath().substring(
												absolutePathPortion));
								if (csFileRelative.exists() == false) {
									FileIO.copyFile(csFile, csFileRelative);
								} else {
									if (csFile.length() > csFileRelative
											.length()) {
										FileIO.copyFile(csFile, csFileRelative);
									}
								}
				...
				LocalVersionedTerminology.get().setProperty(jarFile.getName(),
						new Date().toString());
			} else {
				AceLog.getAppLog().alertAndLogException(
