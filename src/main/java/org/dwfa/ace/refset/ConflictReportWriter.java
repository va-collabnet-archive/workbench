...
public class ConflictReportWriter implements Runnable {
	...
	public static enum RESOLUTION { INCLUDE, EXCLUDE };
	...
	public void addConflict(int subjectId, int conceptAId, int conceptAInclTypeId, int conceptBId, int conceptBInclTypeId, RESOLUTION resolution) {
				...
				for (Integer conceptBId : conflicts.keySet()) {
					...
					for (Integer subjectId : subjectMap.keySet()) {
						...
						for (FieldMap fieldMap : fields) {
							conflictReport.append("\t\t* Resolved to " + ((fieldMap.get(FIELD_NAME.RESOLUTION) == RESOLUTION.INCLUDE) ? "include: " : "exclude: "));
							conflictReport.append("\"" + getConcept(subjectId) + "\": [" + getConcept((Integer)fieldMap.get(FIELD_NAME.INCL_TYPE_A)) + "]:\"" + getConcept(conceptAId));
							conflictReport.append("\" conflicts with [" + getConcept((Integer)fieldMap.get(FIELD_NAME.INCL_TYPE_B)) + "]:\"" + getConcept(conceptBId) + "\"\n");
						}
					}
				}			
