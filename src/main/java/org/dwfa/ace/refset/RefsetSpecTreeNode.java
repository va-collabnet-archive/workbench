...
public class RefsetSpecTreeNode extends DefaultMutableTreeNode implements Comparable<RefsetSpecTreeNode> {
	...
	public String getConstraintDesc() {
		if (constraintDesc == null) {
			...
			try {
				I_DescriptionTuple thisConstraintDesc = thisConstraint.getDescTuple(aceConfig.getTreeDescPreferenceList(), aceConfig);
	...
	public I_ThinExtByRefTuple getExtension() {
		if (extension == null) {
			List<I_ThinExtByRefTuple> tupleList = ((I_ThinExtByRefVersioned) this.userObject).getTuples(aceConfig.getAllowedStatus(), 
					aceConfig.getViewPositionSet(), true);
			if (tupleList.size() > 0) {
				extension = tupleList.get(tupleList.size() -1);
			} else {
				tupleList = ((I_ThinExtByRefVersioned) this.userObject).getTuples(null, 
						aceConfig.getViewPositionSet(), true);
				if (tupleList.size() > 0) {
					extension = tupleList.get(tupleList.size() -1);
				}				
