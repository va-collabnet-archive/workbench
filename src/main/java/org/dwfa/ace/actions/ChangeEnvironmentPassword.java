...
public class ChangeEnvironmentPassword implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			prompter.prompt("Current username/password", AceConfig.config.getUsername());
			if (prompter.getUsername().equals(AceConfig.config.getUsername()) == false) {
				throw new AuthenticationFailedException("username does not match");
			}
			if (prompter.getPassword().equals(AceConfig.config.getPassword()) == false) {
				throw new AuthenticationFailedException("password does not match");
			}
			prompter.prompt("New username/password", AceConfig.config.getUsername());
