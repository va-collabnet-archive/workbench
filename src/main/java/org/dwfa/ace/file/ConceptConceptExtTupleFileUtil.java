...
public class ConceptConceptExtTupleFileUtil {
    ...
    public static void importTuple(String inputLine) throws TerminologyException {
        ...
        try {
            ...
            if (!typeUuid.equals(RefsetAuxiliary.Concept.CONCEPT_CONCEPT_EXTENSION.getUids().iterator().next())) {
                throw new TerminologyException("Non concept-concept ext string passed to concept-concept file util.");
            }
            ...
            refsetHelper.newConceptConceptRefsetExtension(termFactory.getId(refsetUuid).getNativeId(), termFactory
                .getId(componentUuid).getNativeId(), termFactory.getId(c1Uuid).getNativeId(), termFactory.getId(c2Uuid)
                .getNativeId(), memberUuid, pathUuid, statusUuid, effectiveDate);
