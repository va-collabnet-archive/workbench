...
public class LookupJiniAndLocal implements I_LookupServices {
	...
	private static class TemplateFilter implements ServiceItemFilter {
		...
		public boolean check(ServiceItem item) {
			if (logger.isLoggable(Level.FINE))
				logger.fine("Checking: " + item + " attributes: "
			...
			if (tmpl.serviceTypes != null) {
				for (Class serviceClass : tmpl.serviceTypes) {
					if (serviceClass.isAssignableFrom(item.service.getClass()) == false) {
						if (logger.isLoggable(Level.FINE))
							logger.fine("Failed on service class: "
									+ serviceClass.getName() + " proxy class: "
			...
			if (tmpl.attributeSetTemplates != null) {
				Set<Entry> entrySet = new HashSet<Entry>(Arrays
						.asList(item.attributeSets));
				if (item.attributeSets == null) {
					if (logger.isLoggable(Level.FINE))
						logger.fine("Failed on attributeSets == null: "
								+ " template attributes: "
								+ Arrays.asList(item.attributeSets));
					return false;
				} else {
					for (Entry e : tmpl.attributeSetTemplates) {
						if (entrySet.contains(e) == false) {
							if (logger.isLoggable(Level.FINE))
								logger.fine("entrySet.contains(e): " + e
										+ " entrySet: " + entrySet);
							return false;
						}
					}
	...
	public ServiceItem[] lookup(ServiceTemplate tmpl, int minMatches,
		...
		TreeSet<ServiceItem> matches = new TreeSet<ServiceItem>(
				new ServiceItemComparator());
		matches.addAll(this.lookupLocal(tmpl, filter));
		if (this.sdm != null) {
			if (I_QueueProcesses.class.isAssignableFrom(tmpl.serviceTypes[0])) {
				matches.addAll(Arrays.asList(this.queueCache.lookup(
						new TemplateFilter(tmpl, filter), maxMatches
								- matches.size())));
			} else {
	...
	public ServiceItem lookup(ServiceTemplate tmpl, ServiceItemFilter filter) {
		...
		if (this.sdm != null) {
			if (I_QueueProcesses.class.isAssignableFrom(tmpl.serviceTypes[0])) {
				matches.addAll(Arrays.asList(this.queueCache
						.lookup(new TemplateFilter(tmpl, filter))));
				if (matches.size() > 0) {
					return matches.first();
				}
			} else {
						...
						if (checkEntries(tmpl, serviceItem)) {
							if (logger.isLoggable(Level.FINEST)) {
								logger.log(Level.FINEST,
										"checkEntries matched tmpl: " + tmpl
												+ " si: " + serviceItem);
							}
	...
	private boolean checkEntries(ServiceTemplate tmpl, ServiceItem serviceItem) {
		...
		for (Entry tmplEntry : tmpl.attributeSetTemplates) {
			...
			for (Entry serviceEntry : serviceItem.attributeSets) {
				if (logger.isLoggable(Level.FINE)) {
					logger.fine("comparing: " + tmplEntry + " and "
							+ serviceEntry);
				}
	 ...
	 * and for each entry field <em>F</em>, the arguments' values for
	 * <em>F</em> on e1 is null (a wildcard), or if either both
	 * <code>null</code> or the invocation of <code>equals</code> on one
