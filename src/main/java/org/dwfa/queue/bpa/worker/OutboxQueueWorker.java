...
public class OutboxQueueWorker extends Worker implements I_GetWorkFromQueue,
		...
		while (true) {
			try {
				...
				if (logger.isLoggable(Level.FINE)) {
					logger.fine(this.getWorkerDesc() + " (" + this.getId()
							+ ") started sleep.");
				}
				this.sleep();
				if (logger.isLoggable(Level.FINE)) {
	...
	private boolean smtpDelivery(I_EncodeBusinessProcess process, Transaction t) {
		...
		try {
			...
			processPart.setContent(Base64.encodeObject(process),
					PROCESS_ATTACHMENT_TYPE);
	 ...
	 */
	private boolean jiniDelivery(I_EncodeBusinessProcess process, Transaction t)
			throws IOException, TransactionException, TaskFailedException {

		ElectronicAddress eAddress = new ElectronicAddress(process
				.getDestination());
		...
		try {
			...
			logger.info(this.getWorkerDesc() + " process: "
					+ process.getProcessID() + " delivered to: "
					+ process.getDestination()
					+ " via Jini discovered queue service. ");
			return true;
		} catch (InterruptedException e) {
			logger.info(this.getWorkerDesc() + " Jini delivery failed: "
					+ e.toString());
		} catch (IOException e) {
			logger.info(this.getWorkerDesc() + " Jini delivery failed: "
					+ e.toString());
