    ...
    while (true) {
        try {
             while (true) {
                try {
                    ...
                    if (worker.getLogger().isLoggable(Level.INFO)) {
                      worker.getLogger().info(desc + " TAKE: " + process.getName() + " (" + process.getProcessID() + ") " + ": " + process.getCurrentTaskId() + " " + 
                                process.getTask(process.getCurrentTaskId()).getName() + " deadline: " + Worker.dateFormat.format(process.getDeadline()));
                    }
                    ...
                    worker.getLogger().log(Level.WARNING, "Worker: " + desc + " ("+ worker.getId() + ") " + ex.getMessage(), ex);
                }
            }
            ...
            try {
              ...
              worker.getLogger().log(Level.SEVERE, "Worker: " + desc + " ("+ worker.getId() + ") " + e.getMessage(), e);
            } 
