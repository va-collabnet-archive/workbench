 ...
 * Path management. 
 * 
 * Defines methods for obtaining and modifying paths. Paths are now stored/defined in 
 * reference sets (extension by reference).  
 * 
 * This implementation avoids the use of the redundant Path store and instead marshals
...
public class PathManager implements I_Manage<I_Path> {
    ...
    public boolean exists(int nid) throws TerminologyException {
        try {
            return refsetHelper.hasCurrentRefsetExtension(
                pathRefsetId, pathConceptId, 
                new BeanPropertyMap().with(ThinExtByRefPartProperty.CONCEPT_ONE, nid));
    ...
    public Set<I_Path> getAll() throws TerminologyException {
        try {
            ...
            for (I_ThinExtByRefPartConcept extPart : 
                    refsetHelper.<I_ThinExtByRefPartConcept>getAllCurrentRefsetExtensions(pathRefsetId, pathConceptId)) {
     ...
     * NOTE it will not automatically remove origins. This must be done explicitly with {@link #removeOrigin(I_Path, I_Position)}.
     */
    public void write(final I_Path path) throws TerminologyException {
        try {
            ...
            refsetHelper.newRefsetExtension(
                pathRefsetId, pathConceptId, I_ThinExtByRefPartConcept.class, 
    ...
    public void writeOrigin(final I_Path path, final I_Position origin) throws TerminologyException {
        ...
        try {
            if (refsetHelper.hasCurrentRefsetExtension(
                    pathOriginRefsetId, path.getConceptId(), 
                    new BeanPropertyMap()
                        .with(ThinExtByRefPartProperty.CONCEPT_ONE, origin.getPath().getConceptId())
                        .with(ThinExtByRefPartProperty.INTEGER_VALUE, origin.getVersion()))) {
                // Skip already exists
                return;
            }
            
            // Retire any positions that may exist that just have a different version (time) point
            refsetHelper.retireRefsetExtension(
                    pathOriginRefsetId, path.getConceptId(),
                    new BeanPropertyMap().with(ThinExtByRefPartProperty.CONCEPT_ONE, origin.getPath().getConceptId()));
            
            // Create the new origin/position
            refsetHelper.newRefsetExtension(
                    pathOriginRefsetId, path.getConceptId(), I_ThinExtByRefPartConceptInt.class, 
                    new BeanPropertyMap()
                            .with(ThinExtByRefPartProperty.CONCEPT_ONE, origin.getPath().getConceptId())
                            .with(ThinExtByRefPartProperty.INTEGER_VALUE, origin.getVersion()));
