...
public class RelWithPartCoreBdb implements I_StoreRelationships {
	...
	private class RelationshipIterator implements Iterator<I_RelVersioned> {
		...
		private void getNext() {
			try {
				hasNext = (relCursor.getNext(foundKey, foundData,
	...
	public static class C1KeyForRelCreator implements SecondaryKeyCreator {
		...
		EntryBinding intBinder = TupleBinding
		...
		public boolean createSecondaryKey(SecondaryDatabase secDb,
			...
			I_RelVersioned emptyRel = (I_RelVersioned) relBinding
		...
		public boolean createSecondaryKey(int relId, int c1id,
				DatabaseEntry resultEntry) throws DatabaseException {
	...
	public static class C2KeyForRelCreator implements SecondaryKeyCreator {
		...
		public boolean createSecondaryKey(int relId, int concId,
				DatabaseEntry resultEntry) throws DatabaseException {
	...
	public RelWithPartCoreBdb(Environment env, DatabaseConfig dbConfig)
	...
	private void openDatabases(DatabaseConfig dbConfig)
			throws DatabaseException {
	...
	public List<I_RelVersioned> getDestRels(int conceptId)
		...
		c2ToRelKeyCreator.createSecondaryKey(Integer.MIN_VALUE, conceptId,
				secondaryKey);
		...
		OperationStatus retVal = mySecCursor.getSearchKeyRange(secondaryKey,
				primaryKey, foundData, LockMode.DEFAULT);
		List<I_RelVersioned> matches = new ArrayList<I_RelVersioned>();
		while (retVal == OperationStatus.SUCCESS) {
			ThinRelVersioned relFromConceptId = (ThinRelVersioned) relWithPartCoreBinding
			...
			retVal = mySecCursor.getNext(secondaryKey, primaryKey, foundData,
					LockMode.DEFAULT);
		}
		mySecCursor.close();
		if (AceLog.getAppLog().isLoggable(Level.FINE)) {
			AceLog.getAppLog().fine(
					"dest rels fetched for: " + conceptId + " elapsed time: "
	...
	public List<I_RelVersioned> getSrcRels(int conceptId)
		...
		DatabaseEntry secondaryKey = new DatabaseEntry();
		c1ToRelKeyCreator.createSecondaryKey(Integer.MIN_VALUE, conceptId,
				secondaryKey);
		DatabaseEntry foundData = new DatabaseEntry();
		...
		if (AceLog.getAppLog().isLoggable(Level.FINE)) {
			AceLog.getAppLog().fine(
					"src rels fetched for: " + conceptId + " elapsed time: "
							+ timer.getElapsedTime() / 1000 + " secs");
		}
		return matches;
	}
	...
	public boolean hasDestRel(int conceptId, Set<Integer> destRelTypeIds)
		...
		while (retVal == OperationStatus.SUCCESS) {
			I_RelVersioned relFromConceptId = (I_RelVersioned) relWithPartCoreBinding
					.entryToObject(foundData);
			if (relFromConceptId.getC2Id() == conceptId) {
		...
		if (AceLog.getAppLog().isLoggable(Level.FINE)) {
			AceLog.getAppLog().fine(
					"hasDestRel for: " + conceptId + " elapsed time: "
							+ timer.getElapsedTime() / 1000 + " secs");
		}
		return false;
	}
	...
	public boolean hasDestRelTuple(int conceptId, I_IntSet allowedStatus,
		...
		while (retVal == OperationStatus.SUCCESS) {
			ThinRelVersioned relFromConceptId = (ThinRelVersioned) relWithPartCoreBinding
					.entryToObject(foundData);
			if (relFromConceptId.getC2Id() == conceptId) {
				relFromConceptId.setRelId((Integer) intBinder
						.entryToObject(primaryKey));
