...
import org.ihtsdo.mojo.mojo.compare.MonitorComponents;
...
public class VodbCopyBranches extends AbstractMojo implements I_ProcessConcepts, I_ProcessPaths {
    ...
    /**
     * Compare Operator used to test if the concepts should be copied to the new
     * path
     * 
     * @parameter
     * @required
    ...
    /**
     * If this parameter is set, then the system will only check one position at
     * a time, rather than comparing
     * paths for differences. This is useful if the only requirement is to check
     * if one path has a particular status
     * set for a concept
     * 
     * @parameter
     */
        ...
        if (compareOperator.compare(matches)) {
            ...
            for (I_DescriptionTuple tuple : matchDescriptionTuples) {
                ...
                if ((rejectedStatus == null || (rejectedStatus != null && !rejectedStatusIds.contains(tuple.getStatusId())))
                    ...
                    // System.out.println("rejected description: " +
                    // tuple.getText());
            ...
            for (I_RelTuple tuple : matchRelationshipTuples) {
                I_RelPart newPartOnNewPath = tuple.duplicate();
                relationshipCount++;
                if ((rejectedStatus == null || (rejectedStatus != null && !rejectedStatusIds.contains(newPartOnNewPath.getStatusId())))
                    && (acceptedStatus == null || (acceptedStatus != null && acceptedStatusIds.contains(newPartOnNewPath.getStatusId())))) {
                    changed = true;
        ...
        try {
            ...
            if (rejectedStatus != null) {
                for (ConceptDescriptor rejectedstatus : rejectedStatus) {
                    rejectedStatusIds.add(termFactory.getConcept(rejectedstatus.getVerifiedConcept().getUids())
                        .getConceptId());
                }
            }
            if (acceptedStatus != null) {
                for (ConceptDescriptor acceptedstatus : acceptedStatus) {
                    acceptedStatusIds.add(termFactory.getConcept(acceptedstatus.getVerifiedConcept().getUids())
                        .getConceptId());
                }
            }

            updatedStatusId = termFactory.getConcept(updatedStatus.getVerifiedConcept().getUids()).getConceptId();

            updatedStatusOnNewPathId = termFactory.getConcept(updatedStatusOnNewPath.getVerifiedConcept().getUids())
                .getConceptId();
            ...
            if (!checkSinglePath) {
                ...
                System.out.println("Copied " + (descriptionCount - removedDescriptionCount)
                    + " descriptions out of a total of " + descriptionCount + "");
                System.out.println("Copied " + (relationshipCount - removedRelationshipCount)
                    + " relationships out of a total of " + relationshipCount + "");
            ...
            if (conflicts > 0) {
                ...
                htmlWriter.append("Monitored " + conceptCount + " components.");
                htmlWriter.append("<br>");
                htmlWriter.append("Number of agreed changes: " + agreedChanges);
                htmlWriter.append("<br>");
                htmlWriter.append("Number of conflicts: " + conflicts);
