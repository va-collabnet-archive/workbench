...
import org.ihtsdo.mojo.mojo.ImportFromFile;

/**
 * Imports the contents of a file as a sequence of extensions and adds them to
 * an existing refset in the database.
 * 
 * @see https
...
public class ImportRefset extends ImportFromFile {
    ...
    /**
     * May be provided to overwrite the destination refset details from the file
     * being imported
     * 
     * @parameter
     */
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            if (MojoUtil.alreadyRun(getLog(), fileHandler.getSourceFile().toString(), this.getClass(), targetDirectory)) {
                return;
            }
            ...
            for (I_ThinExtByRefVersioned extension : fileHandler) {
                ...
                if (extensions.containsKey(memberId)) {
                    ...
                    // Keep only the latest version part, superseding older
                    // parts
            ...
            for (I_ThinExtByRefVersioned extension : extensions.values()) {
                ...
                if (destination != null) {
                    extPart.setPathId(destination.getPath().getVerifiedConcept().getIdentifier().getNid());
                    extension.setRefsetId(destination.getRefset().getVerifiedConcept().getIdentifier().getNid());
                }
