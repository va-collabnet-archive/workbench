...
public abstract class MemberRefsetHandler {
	 ...
	 * @throws IOException
	 */
	public String formatRefsetLineRF2(I_TermFactory tf, I_ExtendByRefPart part,
			Integer memberId, int refsetNid, int componentId, boolean sctId)
			throws TerminologyException, IOException {
		String formattedLine = getRefsetAndReferencePart(tf, part, memberId,
				refsetNid, componentId, sctId);
		...
		try {
			if (part instanceof I_ExtendByRefPartCid) {
				...
				throw new Exception(
						"No known refset format type for this extension "
	 ...
	 *             DB errors
	 */
	private String getRefsetAndReferencePart(I_TermFactory tf,
			I_ExtendByRefPart part, Integer memberId, int refsetNid,
			int componentId, boolean useSctId) throws TerminologyException,
			IOException {
	...
	public String getSubsetFormatHeaderLine() {
		return "SUBSETID" + FILE_DELIMITER + "MEMBERID" + FILE_DELIMITER
				+ "MEMBERSTATUS" + FILE_DELIMITER + "LINKEDID";
	}
	...
	public String toId(I_TermFactory tf, int componentId, boolean sctId)
			throws TerminologyException, IOException {
		if (sctId) {
			...
			if (snomedId != null) {
				...
				return Long.toString(getSctGenerator()
						.getWithGeneration(
								tf.getUids(componentId).iterator().next(),
								TYPE.CONCEPT));
			}
		} else { // uuid
	...
	public String getSnomedIntegerId(I_TermFactory tf, int componentId)
		...
		int snomedIntegerId = tf.getId(
				ArchitectonicAuxiliary.Concept.SNOMED_INT_ID.getUids()
						.iterator().next()).getNid();
