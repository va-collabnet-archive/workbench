 ...
 *
 * @goal sct-rf2-refset-cfloat-to-arf
 * @requiresDependencyResolution compile
 * @requiresProject false
 */
public class SctRf2RefsetCFloatToArfMojo extends AbstractMojo implements Serializable {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            ...
            BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(
                    outDir + "float_refset_rf2.refset"), "UTF-8"));
            getLog().info("::: FLOAT REFSET FILE: " + outDir + "float_refset_rf2.refset");
            filesIn = Rf2File.getFiles(wDir, targetSubDir, inputDir, inputFile, ".txt");
            for (Rf2File rf2File : filesIn) {
                Rf2_RefsetCFloatRecord[] members = Rf2_RefsetCFloatRecord.parseRefset(rf2File, exclusions);
                for (Rf2_RefsetCFloatRecord m : members) {
                	m.setPath(pathStr);
                    m.writeArf(bw);
                }
            }
            ...
            Logger.getLogger(SctRf2RefsetCFloatToArfMojo.class.getName()).log(Level.SEVERE, null, ex);
