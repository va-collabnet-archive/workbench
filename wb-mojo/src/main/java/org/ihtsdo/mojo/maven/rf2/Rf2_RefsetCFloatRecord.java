...
public class Rf2_RefsetCFloatRecord implements Comparable<Rf2_RefsetCFloatRecord> {
    ...
    final String valueFloat;
    final long concpetIdL;

    String pathUuidStr; // SNOMED Core default
    // String authorUuidStr; // saved as user
    String moduleUuidStr;

    public Rf2_RefsetCFloatRecord(String id, String dateStr, boolean active, String moduleUuidStr,
            long refsetIdL, long referencedComponentIdL, long conceptIdL, String valueFloat) throws ParseException {
        this.id = id;
    ...
    static Rf2_RefsetCFloatRecord[] parseRefset(Rf2File f, Long[] exclusions)
            throws IOException, ParseException {

        int count = Rf2File.countFileLines(f);
        int countExludedMembers = 0;
        int currentCount = 0;
        ArrayList<Rf2_RefsetCFloatRecord> a = new ArrayList<>();
        ...
        int CONCEPT_ID = 6; // should be "normal member"
        int VALUE_ID = 7; // For Language refset VALUE_ID is ACCEPTIBILITY_ID
        ...
        try {
			while (br.ready()) {
			    ...
			    a.add(new Rf2_RefsetCFloatRecord(line[ID],
			            ...
			            Long.parseLong(line[REFERENCED_COMPONENT_ID]),
                                    Long.parseLong(line[CONCEPT_ID]),
			            line[VALUE_ID]));
			}
        ...
        Logger.getLogger(Rf2_RefsetCFloatRecord.class.getName()).info(sb.toString());
        ...
        Rf2_RefsetCFloatRecord[] b = new Rf2_RefsetCFloatRecord[a.size()];
        int idx = 0;
        for (Rf2_RefsetCFloatRecord rec : a) {
            b[idx] = rec;
            idx++;
        }
    ...
    public void writeArf(BufferedWriter writer) throws IOException, TerminologyException {
        ...
        writer.append(Rf2x.convertSctIdToUuidStr(concpetIdL) + TAB_CHARACTER);
        
        // Float Extension Value UUID
        writer.append(valueFloat + TAB_CHARACTER);
    ...
    @Override
    public int compareTo(Rf2_RefsetCFloatRecord t) {
