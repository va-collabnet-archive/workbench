 ...
 *
 * @goal sct-rf2-refset-string-string-to-arf
 * @requiresDependencyResolution compile
 * @requiresProject false
 */
public class SctRf2RefsetStringStringToArfMojo extends AbstractMojo implements Serializable {
     ...
     * 
     * @parameter default-value="der2_ssRefset"
     */
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            ...
            BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(
                    outDir + "stringstring_refset_rf2.refset"), "UTF-8"));
            getLog().info("::: STRING STRING REFSET FILE: " + outDir + "stringstring_refset_rf2.refset");
            filesIn = Rf2File.getFiles(wDir, targetSubDir, inputDir, inputFile, ".txt");
            for (Rf2File rf2File : filesIn) {
                Rf2_RefsetStringStringRecord[] members = Rf2_RefsetStringStringRecord.parseRefset(rf2File, exclusions);
                for (Rf2_RefsetStringStringRecord m : members) {
                	m.setPath(pathStr);
                    m.writeArf(bw);
                }
            }
            ...
            Logger.getLogger(SctRf2RefsetStringStringToArfMojo.class.getName()).log(Level.SEVERE, null, ex);
