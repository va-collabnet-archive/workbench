 ...
 * <b>DESCRIPTION: </b><br>
 * 
 * Sct1ArfToEConceptMojo is a maven mojo which converts SNOMED concepts,
 * descriptions, stated relationships and inferred relationships (Distribution
 * Normal Form) RF1 release files to IHTSDO Workbench versioned import eConcepts
 * format. ARF formatted files can also be combined with the SCT 1 files.
 * <p>
 * <b>Relationship uuids are generated based on the algorithm below. Note that
 * changing the role group in terms of relationship members or an non-mutable
 * part any role group member will cause that role group to be retired and a new
 * role group to be created.</b>
 * 
 * <pre>
 ...
 * <b>REQUIRMENTS:</b><br>
 * 
 * 1. RELEASE DATE must be in either the SNOMED file name. The preferred date
 * format in <code>yyyyMMdd</code>. <br>
 * <br>
 ...
 * <br>
 * 3. STATED & INFERRED. Stated relationship files names must begin with
 * "sct1_relationships_stated". Inferred relationship file names must begin with
 * "sct1_relationships_inferred". Relationship file names without "_stated" or
 * "_inferred" are not supported.
 * <p>
 * <b>PROCESSING:</b><br>
 * Step #1. Versioning & Relationship Generated IDs. Merge time series of
 * releases into a versioned intermediate concept, description, and relationship
 * files. This step also adds an algorithmically computed relationship ids. Ids
 * are kept directly with each primary (concept, description & relationship)
 * component. <br>
 * <br>
 * Step #2. ARF files. Append arf files to sct binary records files.<br>
 * <br>
 * Step #3. Destination Rels. Build file for destination rels. Non-required
 * fields are dropped.<br>
 * <br>
 * Step #4. Match IDs. Associate ids with each specific component.<br>
 * <br>
 * Step #5. Refset. Refset preparation.<br>
 * <br>
 * Step #6. Sort. Sort into concept order for merging the prepared files to
 * create eConcepts in the next step.<br>
 * <br>
 * Step #7. Create EConcepts. Concurrently read pre-sorted concept, description,
 * source relationship and destination relationship files and creates eConcepts.
 * <br>
 * <p>
 * <b>NOTES:</b><br>
...
public class Sct1ArfToEConceptMojo extends AbstractMojo implements Serializable {
	...
	// workaround to set stated relationship characteristic as
	// STATED_RELATIONSHIP
	...
	private static UUID uuidCurrent = ArchitectonicAuxiliary.Concept.CURRENT
	...
	private class SCTFile {
		...
		Boolean mapSctIdInferredToStated; // :DEPRECIATED: Cross map inferred id
											// to stated.
		...
		public SCTFile(File f, String wDir, String subDir, String d,
				Sct1Dir sctDir) throws ParseException {
			this.file = f;
			...
			this.sourceUuid = ArchitectonicAuxiliary.Concept.SNOMED_INT_ID
					.getUids().iterator().next().toString();
			this.sourceUuidIdx = lookupZSourceUuidIdx(sourceUuid); // "ID SOURCE"
																	// i.e.
																	// SNOMED
																	// IDs
			...
			if (sctDir.getWbPathUuidInferred() != null) {
				this.pathInferredIdx = lookupZPathIdx(sctDir
						.getWbPathUuidInferred().toString());
			} else {
				this.pathInferredIdx = pathIdx; // DEFAULT TO CORE PATH
			}
			if (sctDir.getWbPathUuidStated() != null) {
				this.pathStatedIdx = lookupZPathIdx(sctDir
						.getWbPathUuidStated().toString());
			} else {
	...
	private int lookupZSourceUuidIdx(String sourceUuidStr) {
		...
		if (tmp == null) {
			...
			zSourceUuidMap.put(sourceUuidStr,
					Integer.valueOf(zSourceUuidIdxCounter));
	...
	private int lookupZStatusUuidIdx(String statusUuidStr) {
		...
		if (tmp == null) {
			...
			zStatusUuidMap.put(statusUuidStr,
					Integer.valueOf(zStatusUuidIdxCounter));
	...
	private int lookupZDesTypeUuidIdx(String desTypeUuidStr) {
		...
		if (tmp == null) {
			...
			zDesTypeUuidMap.put(desTypeUuidStr,
					Integer.valueOf(zDesTypeUuidIdxCounter));
	...
	private int lookupSrcSystemIdx(String uuidStr) {
		...
		for (int idx = 0; idx < last; idx++) {
			if (uuidStr
					.equalsIgnoreCase(zIdSrcSystemList.get(idx).srcSystemIdStr)) {
				return idx;
			}
		}
		...
		getLog().info(
				" ::: IMPORT DISCOVERED NOT-DECLARED ID SYSTEM = " + uuidStr);
		return last;
	}
	...
	private void setupLookupPartA() throws MojoFailureException {
		...
		try {
			...
			for (int idx = 0; idx < 12; idx++) {
				lookupZStatusUuidIdx(ArchitectonicAuxiliary
						.getStatusFromId(idx).getUids().iterator().next()
						.toString());
			}
			...
			for (int i = 0; i < 5; i++) {
				lookupZDesTypeUuidIdx(ArchitectonicAuxiliary
						.getSnomedDescriptionType(i).getUids().iterator()
						.next().toString());
			}
			lookupZDesTypeUuidIdx(ArchitectonicAuxiliary.Concept.TEXT_DEFINITION_TYPE
					.getUids().iterator().next().toString());

			// RELATIONSHIP CHARACTERISTIC
			zRelCharArray = new UUID[6];
			for (int i = 0; i < 5; i++) {
				zRelCharArray[i] = ArchitectonicAuxiliary
						.getSnomedCharacteristicType(i).getUids().iterator()
						.next();
			}
			zRelCharArray[STATED_CHAR_WORKAROUND] = ArchitectonicAuxiliary.Concept.STATED_RELATIONSHIP
					.getUids().iterator().next();
			...
			for (int i = 0; i < 3; i++) {
				zRelRefArray[i] = ArchitectonicAuxiliary
						.getSnomedRefinabilityType(i).getUids().iterator()
						.next();
			}
			// string lookup array
			...
			zIdSrcSystemList.add(new IdSrcSystemEntry(uuidSourceCtv3,
					IdDataType.STRING));
			zIdSrcSystemList.add(new IdSrcSystemEntry(uuidSourceSnomedRt,
					IdDataType.STRING));
			zIdSrcSystemList.add(new IdSrcSystemEntry(
					ArchitectonicAuxiliary.Concept.ICD_9.getUids().iterator()
							.next(), IdDataType.STRING));

		} catch (Exception e) {
			getLog().info(e);
			throw new MojoFailureException(
					"FAILED: Sct1ArfToEConcept -- setupLookupPartA()");
		}
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		if (sct1Dirs != null) {
			for (int i = 0; i < sct1Dirs.length; i++) {
				sct1Dirs[i].setDirectoryName(sct1Dirs[i].getDirectoryName()
						.replace('/', File.separatorChar));
				getLog().info(
						"POM SCT Input Directory (" + i + ") = "
								+ sct1Dirs[i].getDirectoryName());
				if (!sct1Dirs[i].getDirectoryName().startsWith(FILE_SEPARATOR)) {
					sct1Dirs[i].setDirectoryName(FILE_SEPARATOR
							+ sct1Dirs[i].getDirectoryName());
				}
			}
		}
		...
		for (int i = 0; i < arfInputDirs.length; i++) {
			...
			getLog().info(
					"POM ARF Input Directory (" + i + ") = " + arfInputDirs[i]);
		...
		executeMojo(targetDir, targetSubDir, arfInputDirs, sct1Dirs,
				outputDirectory, includeCTV3ID, includeSNOMEDRTID);
		getLog().info("::: END Sct1ArfToEConcept");
	}

	void executeMojo(String tDir, String tSubDir, String[] arfDirs,
			Sct1Dir[] sctDirs, String outDir, boolean ctv3idTF,
		...
		if (dateStartObj != null) {
			getLog().info(
					"::: Start date (inclusive) = " + sdf.format(dateStartObj));
		}
		if (dateStopObj != null) {
			getLog().info(
					":::  Stop date (inclusive) = " + sdf.format(dateStopObj));
		}
		if (sctDirs != null) {
			for (int i = 0; i < sctDirs.length; i++) {
				getLog().info(
						"::: SCT Input Directory (" + i + ") = "
								+ sctDirs[i].getDirectoryName());
				getLog().info(
						":::     UUID Core:     "
								+ sctDirs[i].getWbPathUuidCore() + " : "
								+ sctDirs[i].getWbPathUuidCoreFromName());
				getLog().info(
						":::     UUID Stated:   "
								+ sctDirs[i].getWbPathUuidStated() + " : "
								+ sctDirs[i].getWbPathUuidStatedFromName());
				getLog().info(
						":::     UUID Inferred: "
								+ sctDirs[i].getWbPathUuidInferred() + " : "
		...
		fNameStep1Con = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step1_concepts.ser";
		fNameStep1Rel = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step1_relationships.ser";
		fNameStep1Desc = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step1_descriptions.ser";
		fNameStep1Ids = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step1_ids.ser";

		fNameStep2Refset = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step2_refset.ser";

		fNameStep3RelDest = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step3_rel_dest.ser";

		fNameStep4Con = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step4_concepts.ser";
		fNameStep4Desc = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step4_descriptions.ser";
		fNameStep4Rel = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step4_relationships.ser";

		fNameStep5RsByCon = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step5_refset_by_con.ser";
		fNameStep5RsByRs = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step5_refet_by_refset.ser";

		fNameStep6Con = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step6_concepts.ser";
		fNameStep6Desc = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step6_descriptions.ser";
		fNameStep6Rel = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step6_relationships.ser";
		fNameStep6RelDest = tDir + scratchDirectory + FILE_SEPARATOR
				+ "step6_rel_dest.ser";
		...
		try {
			...
			if (sctDirs != null) {
				executeMojoStep1(tDir, tSubDir, sctDirs, ctv3idTF, snomedrtTF,
						oosCon, oosDes, oosRel, oosIds);
				System.gc();
			}
			...
			executeMojoStep2(tDir, tSubDir, arfDirs, oosCon, oosDes, oosRel,
					oosIds, oosRefSet);
	...
	private void executeMojoStep1(String wDir, String subDir, Sct1Dir[] inDirs,
			boolean ctv3idTF, boolean snomedrtTF, ObjectOutputStream oosCon,
			ObjectOutputStream oosDes, ObjectOutputStream oosRel,
			ObjectOutputStream oosIds) throws MojoFailureException {
		...
		try {
			...
			List<List<SCTFile>> listOfCDirs = getSctFiles(wDir, subDir, inDirs,
					"concept", ".txt");
		...
		try {
			...
			List<List<SCTFile>> listOfDDirs = getSctFiles(wDir, subDir, inDirs,
					"descriptions", ".txt");
		...
		try {
			...
			BufferedWriter erw;
			erw = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream(erFileName), "UTF-8"));
			getLog().info(
					"RELATIONSHIPS Exceptions Report OUTPUT: " + erFileName);

			processRelationshipsFiles(wDir, listOfRiDirs, oosRel, oosIds, erw,
					USER_SNOROCKET_IDX);
			processRelationshipsFiles(wDir, listOfRsDirs, oosRel, oosIds, erw,
					USER_DEFAULT_IDX);
		...
		getLog().info(
				"*** VERSIONING TIME: "
						...
						+ " seconds");
		getLog().info(
				"*** Sct1ArfToEConcept STEP #1  SCT1 PROCESSING COMPLETE ***\r\n");
	}

	private void executeMojoStep2(String wDir, String subDir, String[] arfDirs,
			ObjectOutputStream oosCon, ObjectOutputStream oosDes,
			ObjectOutputStream oosRel, ObjectOutputStream oosIds,
			ObjectOutputStream oosRefSet) {
		getLog().info(
				"*** Sct1ArfToEConcept STEP #2 BEGINNING - INGEST ARF ***");
		long start = System.currentTimeMillis();

		try {

			// PROCESS CONCEPT ARF FILES
			List<List<ARFFile>> listOfCDirs = getArfFiles(wDir, subDir,
					arfDirs, "concepts", ".txt");
			processArfConFiles(wDir, listOfCDirs, oosCon);
			listOfCDirs = null;
			System.gc();

			// PROCESS DESCRIPTION ARF FILES
			List<List<ARFFile>> listOfDDirs = getArfFiles(wDir, subDir,
					arfDirs, "descriptions", ".txt");
			processArfDesFiles(wDir, listOfDDirs, oosDes);
			listOfDDirs = null;
			System.gc();

			// PROCESS RELATIONSHIP ARF FILES
			List<List<ARFFile>> listOfRDirs = getArfFiles(wDir, subDir,
					arfDirs, "relationships", ".txt");
			processArfRelFiles(wDir, listOfRDirs, oosRel);
			listOfRDirs = null;
			System.gc();

			FileOutputStream fos = new FileOutputStream(fNameStep1Ids
					+ ".showme");

			// PROCESS IDS ARF FILES
			List<List<ARFFile>> listOfIDirs = getArfFiles(wDir, subDir,
					arfDirs, "ids", ".txt");
			...
			List<List<ARFFile>> listOfRsBoolDirs = getArfFiles(wDir, subDir,
					arfDirs, "boolean", ".refset");
			processArfRsBoolFiles(wDir, listOfRsBoolDirs, oosRefSet);
			listOfRsBoolDirs = null;
			System.gc();

			// PROCESS REFSET CONCEPT FILES
			List<List<ARFFile>> listOfRsConDirs = getArfFiles(wDir, subDir,
					arfDirs, "concept", ".refset");
			processArfRsConFiles(wDir, listOfRsConDirs, oosRefSet);
			listOfRsConDirs = null;
			System.gc();

			// PROCESS REFSET INTEGER FILES
			List<List<ARFFile>> listOfRsIntDirs = getArfFiles(wDir, subDir,
					arfDirs, "integer", ".refset");
			processArfRsIntFiles(wDir, listOfRsIntDirs, oosRefSet);
			listOfRsIntDirs = null;
			System.gc();

			// PROCESS REFSET STRING FILES
			List<List<ARFFile>> listOfRsStrDirs = getArfFiles(wDir, subDir,
					arfDirs, "string", ".refset");
			...
			getLog().info(
					"\r\nstatRsBoolFromArf= " + statRsBoolFromArf
							+ "\r\nstatRsIntFromArf= " + statRsIntFromArf
		...
		getLog().info(
				"*** ARF TO BINARY OBJECT TIME: "
						+ ((System.currentTimeMillis() - start) / 1000)
						+ " seconds");
		getLog().info(
				"*** Sct1ArfToEConcept STEP #2 COMPLETED - INGEST ARF ***\r\n");
	}

	private void processArfConFiles(String wDir,
			List<List<ARFFile>> listOfDirs, ObjectOutputStream oos)
	...
	private void parseArfConFile(File f, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		...
		while (br.ready()) {
			...
			int isPrimitive = 0;
			if (isPrimitiveStr.startsWith("1")
			...
			Sct1_ConRecord tmpConRec = new Sct1_ConRecord(uuidCon,
					conceptStatus, isPrimitive, revTime, pathIdx, authorIdx,
					moduleIdx);
	...
	private void processArfDesFiles(String wDir,
			List<List<ARFFile>> listOfDirs, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		for (List<ARFFile> laf : listOfDirs) {
			for (ARFFile f : laf) {
	...
	private void parseArfDesFile(File f, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		BufferedReader br = new BufferedReader(new InputStreamReader(
				new FileInputStream(f), "UTF-8"));
		...
		while (br.ready()) {
			...
			// int capitalization =
			// Integer.parseInt(line[CAPITALIZATION_STATUS_INT]);
			String capitalizationStr = line[CAPITALIZATION_STATUS_INT];
			int capitalization = 0;
			if (capitalizationStr.startsWith("1")
			...
			Sct1_DesRecord tmpDesRec = new Sct1_DesRecord(uuidDes, status,
					uuidCon, termStr, capitalization, descriptionType,
					langCodeStr, revTime, pathIdx, authorIdx, moduleIdx);
			...
			// getLog().info(":DEBUG: ... conUuidLsb = " +
			// tmpDesRec.conUuidLsb);
			// getLog().info(":DEBUG: ... conUuidMsb = " +
			// tmpDesRec.conUuidMsb);
	...
	private void processArfRelFiles(String wDir,
			List<List<ARFFile>> listOfDirs, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		for (List<ARFFile> laf : listOfDirs) {
			for (ARFFile f : laf) {
	...
	private void parseArfRelFile(File f, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		BufferedReader br = new BufferedReader(new InputStreamReader(
				new FileInputStream(f), "UTF-8"));
		...
		while (br.ready()) {
			...
			if (line.length > AUTHOR_UUID) {
				if (line[AUTHOR_UUID].equalsIgnoreCase(uuidUserSnorocket
						.toString())) {
			...
			Sct1_RelRecord tmpRelRec = new Sct1_RelRecord(uuidRelId, status,
					uuidC1, roleTypeIdx, uuidC2, characteristic, refinability,
					group, revTime, pathIdx, userIdx, moduleIdx);
	...
	private void processArfIdsFiles(String wDir,
			List<List<ARFFile>> listOfDirs, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		for (List<ARFFile> laf : listOfDirs) {
			for (ARFFile f : laf) {
	...
	private void parseArfIdsFile(File f, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		BufferedReader br = new BufferedReader(new InputStreamReader(
				new FileInputStream(f), "UTF-8"));
		...
		while (br.ready()) {
			...
			Sct1_IdRecord tmpIdRec = new Sct1_IdRecord(uuidPrimaryId,
					sourceSystemIdx, idFromSourceSystem, status, revTime,
					pathIdx, authorIdx, moduleIdx);
	...
	private void processArfRsBoolFiles(String wDir,
			List<List<ARFFile>> listOfDirs, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		for (List<ARFFile> laf : listOfDirs) {
			for (ARFFile f : laf) {
	...
	private void parseArfRsBoolFile(File f, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		BufferedReader br = new BufferedReader(new InputStreamReader(
				new FileInputStream(f), "UTF-8"));
		...
		while (br.ready()) {
			...
			UUID uuidComponent = UUID
					.fromString(line[REFERENCED_COMPONENT_UUID]);
			// EFFECTIVE_DATE = 4; // yyyy-MM-dd HH:mm:ss
			long revTime = convertDateStrToTime(line[EFFECTIVE_DATE]);
			// PATH_UUID = 5;
			int pathIdx = lookupZPathIdx(line[PATH_UUID]);
			// EXT_VALUE_UUID = 6;
			boolean vBool = false;
			if (line[EXT_VALUE_UUID].charAt(0) == 't'
					|| line[EXT_VALUE_UUID].charAt(0) == 'T') {
				vBool = true;
			}
			// AUTHOR_UUID = 7;
			int authorIdx = -1;
			...
			// :DEBUG:
			// if
			// (uuidComponent.equals(UUID.fromString("7c57f6b4-4a63-52ad-b762-73acc15f23de")))
			// getLog().info("FOUND IT");

			Sct1_RefSetRecord tmpRsRec = new Sct1_RefSetRecord(uuidRefset,
					uuidMember, uuidComponent, status, revTime, pathIdx,
					authorIdx, moduleIdx, vBool);
	...
	private void processArfRsConFiles(String wDir,
			List<List<ARFFile>> listOfDirs, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		for (List<ARFFile> laf : listOfDirs) {
			for (ARFFile f : laf) {
	...
	private void parseArfRsConFile(File f, ObjectOutputStream oos)
		...
		while (br.ready()) {
			...
			Sct1_RefSetRecord tmpRsRec = new Sct1_RefSetRecord(uuidRefset,
					uuidMember, uuidComponent, status, revTime, pathIdx,
					authorIdx, moduleIdx, uuidConExt);
	...
	private void processArfRsIntFiles(String wDir,
			List<List<ARFFile>> listOfDirs, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		for (List<ARFFile> laf : listOfDirs) {
			for (ARFFile f : laf) {
	...
	private void parseArfRsIntFile(File f, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		BufferedReader br = new BufferedReader(new InputStreamReader(
				new FileInputStream(f), "UTF-8"));
	...
	private void processArfRsStrFiles(String wDir,
			List<List<ARFFile>> listOfDirs, ObjectOutputStream oos)
			throws IOException, MojoFailureException {
		for (List<ARFFile> laf : listOfDirs) {
			for (ARFFile f : laf) {
	...
	private void executeMojoStep3() throws MojoFailureException {
		getLog().info(
				"*** Sct1ArfToEConcept STEP #3 BEGINNING -- GATHER DESTINATION RELs ***");
		long start = System.currentTimeMillis();

		try {
			// read in relationships, sort by C2-ROLETYPE
			ObjectInputStream ois = new ObjectInputStream(
					new BufferedInputStream(new FileInputStream(fNameStep1Rel)));
			...
			try {
				...
				getLog().info(e);
				throw new MojoFailureException(
						"ClassNotFoundException -- Step 2 reading file");
			}
			ois.close();
			getLog().info(" relationship count = " + count + "\r\n");
			...
			for (Sct1_RelRecord r : aRel) {
				if (r.relUuidMsb != lastRelMsb || r.relUuidLsb != lastRelLsb) {
					oos.writeUnshared(new Sct1_RelDestRecord(r.relUuidMsb,
							r.relUuidLsb, r.c2UuidMsb, r.c2UuidLsb,
							r.roleTypeIdx));
				}
		...
		getLog().info(
				"*** DESTINATION RELs: "
						...
						+ " seconds");
		getLog().info(
				"*** Sct1ArfToEConcept STEP #3 COMPLETED -- GATHER DESTINATION RELs ***\r\n");
	}

	private void executeMojoStep4_MatchIds() throws MojoFailureException {
		getLog().info(
				"*** Sct1ArfToEConcept STEP #4 BEGINNING -- MATCH IDs ***");
		long start = System.currentTimeMillis();
		int nWrite = 0; // counter for memory optimization for object files
						// writing
		...
		try {
			...
			ois = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep1Ids)));
			...
			while (theIdIdx < lastIdIdx && theConIdx < lastConIdx) {
				...
				if (match == 0) {
					...
					if (aId.get(theIdIdx).srcSystemIdx == 0 &&
							tmpCon.conSnoId == Long.MAX_VALUE) {						
			...
			// *** DESCRIPTIONS ***
			ois = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep1Desc)));
			...
			oos = new ObjectOutputStream(new BufferedOutputStream(
					new FileOutputStream(fNameStep4Desc)));
			...
			while (theIdIdx < lastIdIdx && theDesIdx < lastDesIdx) {
				...
				if (match == 0) { // MATCH
					if (aId.get(theIdIdx).srcSystemIdx == 0 &&
							tmpDes.desSnoId == Long.MAX_VALUE) {
			...
			oos = new ObjectOutputStream(new BufferedOutputStream(
					new FileOutputStream(fNameStep4Rel)));
			...
			while (theIdIdx < lastIdIdx && theRelIdx < lastRelIdx) {
				...
				if (match == 0) { // MATCH
					if (aId.get(theIdIdx).srcSystemIdx == 0 &&
							tmpRel.relSnoId == Long.MAX_VALUE) {
		...
		getLog().info(
				"*** ATTACH IDs TIME: "
						...
						+ " seconds");
		getLog().info(
				"*** Sct1ArfToEConcept STEP #4 COMPLETED - MATCH IDs ***\r\n");
	}
	...
	private TkIdentifier createEIdentifierString(Sct1_IdRecord id) {
		...
		// MODULE UUID
		if (id.moduleIdx == MODULE_DEFAULT_IDX) {
			eId.setModuleUuid(uuidModule);
		} else {
			eId.setModuleUuid(zModuleArray[id.moduleIdx]);
		}
	...
	private int checkIdConMatched(Sct1_IdRecord id, Sct1_ConRecord con) {
		...
		if (id.primaryUuidMsb > con.conUuidMsb) {
			return AFTER;
		} else if (id.primaryUuidMsb == con.conUuidMsb
				&& id.primaryUuidLsb > con.conUuidLsb) {
			return AFTER;
		}
	...
	private int checkIdDesMatched(Sct1_IdRecord id, Sct1_DesRecord des) {
		...
		if (id.primaryUuidMsb > des.desUuidMsb) {
			return AFTER;
		} else if (id.primaryUuidMsb == des.desUuidMsb
				&& id.primaryUuidLsb > des.desUuidLsb) {
			return AFTER;
		}
	...
	private int checkIdRelMatched(Sct1_IdRecord id, Sct1_RelRecord rel) {
		...
		if (id.primaryUuidMsb > rel.relUuidMsb) {
			return AFTER;
		} else if (id.primaryUuidMsb == rel.relUuidMsb
				&& id.primaryUuidLsb > rel.relUuidLsb) {
			return AFTER;
		}
	...
	private void executeMojoStep5() {
		getLog().info(
				"*** Sct1ArfToEConcept Step #5 BEGINNING -- REFSET ATTACHMENT ***");
		long start = System.currentTimeMillis();
		...
		try {
			...
			ObjectInputStream ois;
			ois = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep2Refset)));
			ArrayList<Sct1_RefSetRecord> aRs = new ArrayList<Sct1_RefSetRecord>(
					numObj);
			...
			try {
				...
				while (obj != null && idxRsA < aRsMax) {
					...
					int rsVin = compareMsbLsb(rsRec.referencedComponentUuidMsb,
							rsRec.referencedComponentUuidLsb,
			...
			// *** DESCRIPTIONS ***
			ois = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep4Desc)));
			try {
				count = 0;
				idxRsA = 0;
				obj = ois.readUnshared();
						...
						if (rsRec.conUuidMsb != Long.MAX_VALUE) {
							getLog().info(
									"ERROR: Refset Envelop UUID Concept/Description conflict"
											+ "\r\nExisting UUID:"
											+ new UUID(rsRec.conUuidMsb,
													rsRec.conUuidLsb)
											+ "\r\nDescription UUID:"
											+ new UUID(desRec.desUuidMsb,
													desRec.desUuidLsb));
						}
			...
			// *** RELATIONSHIPS ***
			ois = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep4Rel)));
			try {
				count = 0;
				idxRsA = 0;
				obj = ois.readUnshared();
			...
			ArrayList<Sct1_RefSetRecord> bRs = new ArrayList<Sct1_RefSetRecord>(
					aRs);
			...
			while (idxRsA < aRsMax && idxRsB < aRsMax) {
				...
				int rsVin = compareMsbLsb(rsRecA.referencedComponentUuidMsb,
						rsRecA.referencedComponentUuidLsb,
					...
					if (rsRecA.conUuidMsb != Long.MAX_VALUE) {
						getLog().info(
								"ERROR: Refset Envelop UUID Concept/Refset conflict"
										+ "\r\nExisting UUID:"
										+ new UUID(rsRecA.conUuidMsb,
												rsRecA.conUuidLsb)
										+ "\r\nRefset UUID:"
										+ new UUID(rsRecB.refsetUuidMsb,
			...
			oos = new ObjectOutputStream(new BufferedOutputStream(
					new FileOutputStream(fNameStep5RsByRs)));
			for (Sct1_RefSetRecord r : aRs) {
				oos.writeUnshared(r);
			}
			oos.flush();
		...
		getLog().info(
				"*** MASTER SORT TIME: "
						...
						+ " seconds");
		getLog().info(
				"*** Sct1ArfToEConcept Step #5 COMPLETED -- REFSET ATTACHMENT ***\r\n");
	}
	...
	private void executeMojoStep6() throws MojoFailureException {
		getLog().info(
				"*** Sct1ArfToEConcept Step #6 BEGINNING -- SORT BY CONCEPT ***");
		long start = System.currentTimeMillis();
		try {

			// *** CONCEPTS ***
			ObjectInputStream ois = new ObjectInputStream(
					new BufferedInputStream(new FileInputStream(fNameStep4Con)));
			ArrayList<Sct1_ConRecord> aCon = new ArrayList<Sct1_ConRecord>();
			...
			oos = new ObjectOutputStream(new BufferedOutputStream(
					new FileOutputStream(fNameStep6Desc)));
			...
			oos = new ObjectOutputStream(new BufferedOutputStream(
					new FileOutputStream(fNameStep6Rel)));
			for (Sct1_RelRecord r : aRel) {
				oos.writeUnshared(r);
			}
			oos.flush();
			...
			ois = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep3RelDest)));
			...
			try {
				while ((obj = ois.readObject()) != null) {
					if (obj instanceof Sct1_RelDestRecord) {
						...
						if (count % 100000 == 0) {
							getLog().info(
									" destination relationships count = "
											+ count);
						}
					}
				}
				...
				getLog().info(
						" destination relationships count = " + count + "\r\n");
			}
			ois.close();
			getLog().info(
					" destination relationships count = " + count + " @EOF\r\n");
			...
			oos = new ObjectOutputStream(new BufferedOutputStream(
					new FileOutputStream(fNameStep6RelDest)));
			...
			getLog().info(e);
			throw new MojoFailureException(
					"ClassNotFoundException -- Step 6 Sort");
		}
		getLog().info(
				"*** MASTER SORT TIME: "
						+ ((System.currentTimeMillis() - start) / 1000)
						+ " seconds");
		getLog().info(
				"*** Sct1ArfToEConcept Step #6 COMPLETED -- SORT BY CONCEPT ***\r\n");
	}
	...
	private void executeMojoStep7() throws MojoFailureException, IOException {
		...
		getLog().info(
				"*** Sct1ArfToEConcept STEP #7 BEGINNING -- CREATE eCONCEPTS ***");
		long start = System.currentTimeMillis();
		countEConWritten = 0;
		...
		try {
			oisCon = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep6Con)));
			oisDes = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep6Desc)));
			oisRel = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep6Rel)));
			oisRelDest = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep6RelDest)));
			oisRsByCon = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep5RsByCon)));
			oisRsByRs = new ObjectInputStream(new BufferedInputStream(
					new FileInputStream(fNameStep5RsByRs)));
		...
		while (notDone) {
			...
			while (theRel.compareTo(theCon) == IS_LESS) {
				...
				if (relNext == null && relList.isEmpty()) {
					...
					if (theRel.compareTo(theCon) == IS_LESS) {
						getLog().info(
								"ORPHAN RELATIONSHIP :: relid="
										+ relList.get(0).relSnoId + " c1=="
			...
			while (theRelDest.compareTo(theCon) == IS_LESS) {
				relDestNext = readNextRelDest(oisRelDest, relDestList,
						relDestNext);
				if (relDestNext == null && relDestList.isEmpty()) {
					...
					theRelDest = new UUID(tmpRelDest.c2UuidMsb,
							tmpRelDest.c2UuidLsb);
					countRelDest++;
			...
			while (theRsByCon.compareTo(theCon) == IS_LESS) {
				rsByConNext = readNextRsByCon(oisRsByCon, rsByConList,
						rsByConNext);
				...
				if (rsByConNext == null && rsByConList.isEmpty()) {
					...
					theRsByCon = new UUID(tmpRsByCon.conUuidMsb,
							tmpRsByCon.conUuidLsb);
					countRsByCon++;
					if (theRsByCon.compareTo(theCon) == IS_LESS) {
						getLog().info(
								"ORPHAN REFSET MEMBER RECORD_A :: "
										+ new UUID(
												rsByConList.get(0).refsetMemberUuidMsb,
			...
			while (theRsByRs.compareTo(theCon) == IS_LESS) {
				...
				// :DEBUG:
				// UUID debugUuidRsByCon111 =
				// UUID.fromString("ccbd4a65-9b1a-5df3-94d1-4a1085f3c758");
				...
				if (rsByRsNext == null && rsByRsList.isEmpty()) {
					...
					theRsByRs = new UUID(tmpRsByRs.refsetUuidMsb,
							tmpRsByRs.refsetUuidLsb);
					countRsByRs++;
					if (theRsByRs.compareTo(theCon) == IS_LESS) {
						getLog().info(
								"ORPHAN REFSET MEMBER RECORD_B :: "
										+ new UUID(
												rsByRsList.get(0).refsetMemberUuidMsb,
			...
			if (theCon.compareTo(theDes) != IS_EQUAL
				...
				if (reportRootConcepts) {
					...
					UUID uuid = new UUID(conList.get(0).conUuidMsb,
							conList.get(0).conUuidLsb);
					getLog().info(
							" -is- concept SNOMED UUID =" + uuid.toString());
					getLog().info(
							" -is- concept SNOMED id ="
									+ conList.get(0).conSnoId);
					getLog().info(" -is- concept counter #" + countCon);
					getLog().info(
							" -is- description \"" + desList.get(0).termText
									+ "\"\r\n");
					getLog().info(
							" ...prev... " + prevCon + " " + prevDes + " "
									+ prevRel + " " + prevRelDest);
					getLog().info(
							" ...-is-... " + theCon + " " + theDes + " "
									+ theRel + " " + theRelDest);
					...
					String rdnStr = "*null*";
					if (relDestNext != null) {
						uuid = new UUID(relDestNext.c2UuidMsb,
								relDestNext.c2UuidLsb);
			...
			if (theCon.compareTo(theDes) == IS_EQUAL
				...
				createEConcept(conList, desList, relList, relDestList,
						addRsByCon, addRsByRs, dos);
			} else if (theCon.compareTo(theDes) == IS_EQUAL
					&& theCon.compareTo(theRel) != IS_EQUAL
					&& theCon.compareTo(theRelDest) == IS_EQUAL) {
				// TOP CASE theCon ==theDes !=theRel ==theRelDest
				createEConcept(conList, desList, null, relDestList, addRsByCon,
						addRsByRs, dos);
			} else if (theCon.compareTo(theDes) == IS_EQUAL
					&& theCon.compareTo(theRel) == IS_EQUAL
					&& theCon.compareTo(theRelDest) != IS_EQUAL) {
				// BOTTOM CASE theCon ==theDes ==theRel !=theRelDest
				createEConcept(conList, desList, relList, null, addRsByCon,
						addRsByRs, dos);
			} else if (theCon.compareTo(theDes) == IS_EQUAL
					&& theCon.compareTo(theRel) != IS_EQUAL
					&& theCon.compareTo(theRelDest) != IS_EQUAL) {
				// UNCONNECTED CONCEPT theCon ==theDes !=theRel !=theRelDest
				createEConcept(conList, desList, null, null, addRsByCon,
						addRsByRs, dos);
			} else {
				getLog().info(
						"!!! Note: the following can occur if the placeholder concepts mojo is used. "
								+ "Placeholders are created for where the concept attach to existing data, potentially resulting in empty placeholders.");
				getLog().info(
						"!!! Case what case is this??? -- Step 4"
								+ " theCon=\t" + theCon + "\ttheDes=\t"
								+ theDes + "\ttheRel=\t" + theRel
								+ "\ttheRelDest\t" + theRelDest);
				getLog().info("!!! --- concept UUID id   =" + theCon);
				getLog().info(
						"!!! --- concept SNOMED id =" + conList.get(0).conSnoId);
				...
				getLog().info("!!! \r\n");
				// throw new
				// MojoFailureException("Case not implemented -- executeMojoStep7()");
			}
		...
		getLog().info(
				"RECORD COUNT = " + countCon + "(Con) " + countDes + "(Des) "
						+ countRel + "(Rel)");
		getLog().info(
				"COMPONENT COUNT = " + statCon + "(statCon) " + statDes
						+ "(statDes) " + statRel + "(statRel)");
		getLog().info(
				"INDEX COUNT = " + statRelDest + "(statRelDest) " + statRsByCon
						+ "(statRsByCon) " + statRsByRs + "(statRsByRs)");
		getLog().info(
				"REFSET COUNT = " + countRsByCon + "(countRsByCon) "
						+ countRsByRs + "(countRsByRs) ");
		...
		try {
			...
			getLog().info(e);
			throw new MojoFailureException(
					"IO Exception -- Step 4, closing files");
		}
		getLog().info(
				"*** ECONCEPT CREATION TIME: "
						+ ((System.currentTimeMillis() - start) / 1000)
						+ " seconds");
		getLog().info("*** ECONCEPTS WRITTEN TO FILE = " + countEConWritten);
		getLog().info(
				"*** Sct1ArfToEConcept STEP #7 COMPLETED -- CREATE eCONCEPTS ***\r\n");
	}

	// UUID OF INTEREST
	// ICD-0-3 == cff53f1a-1d11-5ae7-801e-d3301cfdbea0
	// private static final UUID debugUuid01 =
	// UUID.fromString("445d932f-f552-3b4b-a323-3a06f1d26d98");
	// private static final UUID debugUuid02 =
	// UUID.fromString("daa9598a-2ddb-5527-beda-ee4303a7656c");
	// private static final UUID debugUuid03 =
	// UUID.fromString("3ca0d065-06b8-596c-8ca0-e4d2a605701c");
	private void createEConcept(ArrayList<Sct1_ConRecord> conList,
			ArrayList<Sct1_DesRecord> desList,
			ArrayList<Sct1_RelRecord> relList,
			ArrayList<Sct1_RelDestRecord> relDestList,
			ArrayList<Sct1_RefSetRecord> rsByConList,
		...
		if (cRec0.status == -1) {
			...
			if (cRec0.moduleIdx == MODULE_DEFAULT_IDX) {
				...
				ca.setModuleUuid(zModuleArray[cRec0.moduleIdx]);
			}
			...
			if (cRec0.conSnoId < Long.MAX_VALUE) {
				...
				cid.setPathUuid(zPathArray[cRec0.pathIdx]);
				...
				if (cRec0.moduleIdx == MODULE_DEFAULT_IDX) {
					...
					cid.setModuleUuid(zModuleArray[cRec0.moduleIdx]);
				}
			...
			if (cRec0.ctv3id != null) {
				...
				cids.setPathUuid(zPathArray[cRec0.pathIdx]);
				...
				if (cRec0.moduleIdx == MODULE_DEFAULT_IDX) {
					...
					cids.setModuleUuid(zModuleArray[cRec0.moduleIdx]);
				}
			...
			ca.setPathUuid(zPathArray[cRec0.pathIdx]);
			...
			for (int i = 1; i < max; i++) {
				...
				rev.setPathUuid(zPathArray[cRec.pathIdx]);
				rev.setTime(cRec.revTime);
				rev.authorUuid = uuidUser;
				if (cRec.moduleIdx == MODULE_DEFAULT_IDX) {
					rev.setModuleUuid(uuidModule);
				} else {
					rev.setModuleUuid(zModuleArray[cRec.moduleIdx]);
				}
				caRevisions.add(rev);
			}
		...
		if (desList != null) {
			...
			for (Sct1_DesRecord dRec : desList) {
				if (dRec.desUuidMsb != theDesMsb
						|| dRec.desUuidLsb != theDesLsb) {
					...
					if (dRec.desSnoId < Long.MAX_VALUE) {
						...
						if (dRec.moduleIdx == MODULE_DEFAULT_IDX) {
							...
							did.setModuleUuid(zModuleArray[dRec.moduleIdx]);
						}
					...
					des.setPrimordialComponentUuid(new UUID(theDesMsb,
							theDesLsb));
					des.setConceptUuid(theConUUID);
					des.setText(dRec.termText);
					des.setInitialCaseSignificant(dRec.capStatus == 1 ? true
							: false);
					...
					if (dRec.moduleIdx == MODULE_DEFAULT_IDX) {
						...
						des.setModuleUuid(zModuleArray[dRec.moduleIdx]);
					}
					des.revisions = null;
				} else {
					...
					edv.setInitialCaseSignificant(dRec.capStatus == 1 ? true
							: false);
					...
					if (dRec.moduleIdx == MODULE_DEFAULT_IDX) {
						...
						edv.setModuleUuid(zModuleArray[dRec.moduleIdx]);
					}
					revisions.add(edv);
				}
			}
		...
		if (relList != null) {
			...
			for (Sct1_RelRecord rRec : relList) {
				if (rRec.relUuidMsb != theRelMsb
						|| rRec.relUuidLsb != theRelLsb) {
					// CLOSE OUT OLD RELATIONSHIP
					if (rel != null) {
						if (revisions.size() > 0) {
					...
					rel.setPrimordialComponentUuid(new UUID(theRelMsb,
							theRelLsb));
					...
					if (rRec.moduleIdx == MODULE_DEFAULT_IDX) {
						...
						rel.setModuleUuid(zModuleArray[rRec.moduleIdx]);
					}
					rel.revisions = null;
				} else {
					...
					if (rRec.moduleIdx == MODULE_DEFAULT_IDX) {
						...
						erv.setModuleUuid(zModuleArray[rRec.moduleIdx]);
					}
					revisions.add(erv);
				}
			}
		...
		if (rsByConList != null && rsByConList.size() > 0) {
			...
			for (int rIdx = 0; rIdx < length; rIdx++) {
				...
				if (r.componentType == Sct1_RefSetRecord.ComponentType.CONCEPT) {
					listRefsetUuidMemberUuidForCon.add(new UUID(
							r.refsetUuidMsb, r.refsetUuidLsb));
					listRefsetUuidMemberUuidForCon.add(new UUID(
							r.refsetMemberUuidMsb, r.refsetMemberUuidLsb));
				} else if (r.componentType == Sct1_RefSetRecord.ComponentType.DESCRIPTION) {
					listRefsetUuidMemberUuidForDes.add(new UUID(
							r.refsetUuidMsb, r.refsetUuidLsb));
					listRefsetUuidMemberUuidForDes.add(new UUID(
							r.refsetMemberUuidMsb, r.refsetMemberUuidLsb));
				} else if (r.componentType == Sct1_RefSetRecord.ComponentType.IMAGE) {
					listRefsetUuidMemberUuidForImage.add(new UUID(
							r.refsetUuidMsb, r.refsetUuidLsb));
					listRefsetUuidMemberUuidForImage.add(new UUID(
							r.refsetMemberUuidMsb, r.refsetMemberUuidLsb));
				} else if (r.componentType == Sct1_RefSetRecord.ComponentType.MEMBER) {
					listRefsetUuidMemberUuidForRefsetMember.add(new UUID(
							r.refsetUuidMsb, r.refsetUuidLsb));
					listRefsetUuidMemberUuidForRefsetMember.add(new UUID(
							r.refsetMemberUuidMsb, r.refsetMemberUuidLsb));
				} else if (r.componentType == Sct1_RefSetRecord.ComponentType.RELATIONSHIP) {
					listRefsetUuidMemberUuidForRel.add(new UUID(
							r.refsetUuidMsb, r.refsetUuidLsb));
					listRefsetUuidMemberUuidForRel.add(new UUID(
							r.refsetMemberUuidMsb, r.refsetMemberUuidLsb));
				} else {
		...
		if (rsByRsList != null && rsByRsList.size() > 0) {
			...
			if (rsByRsList.size() > 100000) {
				...
				getLog().info(
						"::: NOTE: concept with MANY refset members = "
								+ rsByRsList.size() + ", concept UUID = "
								+ tmpUUID.toString());
			}
			...
			while (hasMembersToProcess) {
				...
				if (r.valueType == Sct1_RefSetRecord.ValueType.BOOLEAN) {
					...
					tmp.setPrimordialComponentUuid(new UUID(
							r.refsetMemberUuidMsb, r.refsetMemberUuidLsb));
					...
					// :DEBUG:
					// UUID debugUuid = new UUID(r.refsetMemberUuidMsb,
	...
	private Sct1_ConRecord readNextCon(ObjectInputStream ois,
			ArrayList<Sct1_ConRecord> conList, Sct1_ConRecord conNext)
			throws MojoFailureException {
		conList.clear();
		if (conNext != null) {
			conList.add(conNext);
			...
			try { // CHECK FOR FIRST RECORD SITUATION
				...
				getLog().info(e);
				throw new MojoFailureException(
						"ClassNotFoundException - readNextCon()");
			}
		}
	...
	private Sct1_DesRecord readNextDes(ObjectInputStream ois,
			ArrayList<Sct1_DesRecord> desList, Sct1_DesRecord desNext)
			throws MojoFailureException {
		desList.clear();
		if (desNext != null) {
			desList.add(desNext);
			...
			try { // CHECK FOR FIRST RECORD SITUATION
				...
				getLog().info(e);
				throw new MojoFailureException(
						"ClassNotFoundException - readNextDes()");
			}
		}
	...
	private Sct1_RelRecord readNextRel(ObjectInputStream ois,
			ArrayList<Sct1_RelRecord> relList, Sct1_RelRecord relNext)
			throws MojoFailureException {
		relList.clear();
		if (relNext != null) {
			relList.add(relNext);
			...
			try { // CHECK FOR FIRST RECORD SITUATION
				...
				getLog().info(e);
				throw new MojoFailureException(
						"ClassNotFoundException - readNextRel()");
			}
		}
		...
		try {
			...
			while (notDone) {
				...
				if (obj instanceof Sct1_RelRecord) {
					...
					if (rec.c1UuidMsb == relNext.c1UuidMsb
							&& rec.c1UuidLsb == relNext.c1UuidLsb) {
						relList.add(rec);
					} else {
						relNext = rec;
						notDone = false;
	...
	private Sct1_RelDestRecord readNextRelDest(ObjectInputStream ois,
			ArrayList<Sct1_RelDestRecord> relDestList,
			Sct1_RelDestRecord relDestNext) throws MojoFailureException {
		relDestList.clear();
		if (relDestNext != null) {
			...
			try { // CHECK FOR FIRST RECORD SITUATION
				...
				getLog().info(e);
				throw new MojoFailureException(
						"IO Exception - readNextRelDest()");
			} catch (ClassNotFoundException e) {
				getLog().info(e);
				throw new MojoFailureException(
						"ClassNotFoundException - readNextRelDest()");
			}
		}
	...
	private Sct1_RefSetRecord readNextRsByCon(ObjectInputStream ois,
			ArrayList<Sct1_RefSetRecord> rsByConList,
			Sct1_RefSetRecord rsByConNext) throws MojoFailureException {
		rsByConList.clear();
		if (rsByConNext != null) {
			...
			try { // CHECK FOR FIRST RECORD SITUATION
				...
				getLog().info(e);
				throw new MojoFailureException(
						"IO Exception - readNextRsByCon()");
			} catch (ClassNotFoundException e) {
				getLog().info(e);
				throw new MojoFailureException(
						"ClassNotFoundException - readNextRsByCon()");
			}
		}
	...
	private Sct1_RefSetRecord readNextRsByRs(ObjectInputStream ois,
			ArrayList<Sct1_RefSetRecord> rsByRsList,
			Sct1_RefSetRecord rsByRsNext) throws MojoFailureException {
		rsByRsList.clear();
		if (rsByRsNext != null) {
			...
			try { // CHECK FOR FIRST RECORD SITUATION
				...
				getLog().info(e);
				throw new MojoFailureException(
						"IO Exception - readNextRsByRs()");
			} catch (ClassNotFoundException e) {
				getLog().info(e);
				throw new MojoFailureException(
						"ClassNotFoundException - readNextRsByRs()");
			}
		}
	...
	private void setupUuids() throws MojoFailureException {
		try {
			...
			uuidDescPrefTerm = UUID
					.fromString("d8e3b37d-7c11-33ef-b1d0-8769e2264d44");
			uuidDescFullSpec = UUID
					.fromString("5e1fe940-8faf-11db-b606-0800200c9a66");
			uuidRelCharStated = UUID
					.fromString("3fde38f6-e079-3cdc-a819-eda3ec74732d");
			uuidRelNotRefinable = UUID
					.fromString("e4cde443-8fb6-11db-b606-0800200c9a66");
			uuidWbAuxIsa = UUID
					.fromString("46bccdc4-8fb6-11db-b606-0800200c9a66");
			...
			uuidStatedDescFs = Type5UuidFactory.get(
					Type5UuidFactory.PATH_ID_FROM_FS_DESC, uuidPathWbAux
							+ uuidDescFullSpec.toString()
							+ "SNOMED Core Stated");

			uuidStatedDescPt = Type5UuidFactory.get(
					Type5UuidFactory.PATH_ID_FROM_FS_DESC, uuidWbAuxIsa
							+ uuidDescPrefTerm.toString()
							+ "SNOMED Core Stated");

			uuidStatedRel = Type5UuidFactory.get(
					Type5UuidFactory.PATH_ID_FROM_FS_DESC,
					uuidWbAuxIsa + uuidStatedDescFs.toString()
							+ uuidStatedDescPt.toString());

			uuidInferredDescFs = Type5UuidFactory.get(
					Type5UuidFactory.PATH_ID_FROM_FS_DESC, uuidPathWbAux
							+ uuidDescFullSpec.toString()
							+ "SNOMED Core Inferred");

			uuidInferredDescPt = Type5UuidFactory.get(
					Type5UuidFactory.PATH_ID_FROM_FS_DESC, uuidWbAuxIsa
							+ uuidDescPrefTerm.toString()
							+ "SNOMED Core Inferred");

			uuidInferredRel = Type5UuidFactory.get(
					Type5UuidFactory.PATH_ID_FROM_FS_DESC, uuidWbAuxIsa
							+ uuidInferredDescFs.toString()
							+ uuidInferredDescPt.toString());
			...
			uuidSourceCtv3 = ArchitectonicAuxiliary.Concept.CTV3_ID.getUids()
					.iterator().next();
			uuidSourceSnomedRt = ArchitectonicAuxiliary.Concept.SNOMED_RT_ID
					.getUids().iterator().next();
			uuidSourceSnomedLong = ArchitectonicAuxiliary.Concept.SNOMED_INT_ID
					.getUids().iterator().next();
			...
			getLog().info("SNOMED Core Inferred = " + uuidPathSnomedInferredStr);
			getLog().info(
					"  ... Inferred rel   = " + uuidInferredRel.toString());
	...
	private List<List<ARFFile>> getArfFiles(String wDir, String subDir,
			String[] arfDirs, String prefix, String postfix)
			throws MojoFailureException {
		...
		for (int ii = 0; ii < arfDirs.length; ii++) {
			...
			getLog().info(
					prefix.toUpperCase() + " (" + ii + ") " + wDir + subDir
							+ arfDirs[ii]);
			...
			for (File f2 : files) {
				...
				if (filter.accept(f2)) {
					...
					getLog().info(
							"    FILE : " + f2.getParent() + FILE_SEPARATOR
									+ f2.getName());
				}
	...
	private List<List<SCTFile>> getSctFiles(String wDir, String subDir,
			Sct1Dir[] inDirs, String prefix, String postfix)
			throws MojoFailureException {
		...
		for (Sct1Dir sctDir : inDirs) {
			...
			getLog().info(
					String.format("%1$s (%2$s%3$s%4$s) ", prefix.toUpperCase(),
							wDir, subDir, sctDir.getDirectoryName()));

			File f1 = new File(new File(wDir, subDir),
				...
				if (filter.accept(f2)) {
					...
					try {
						if (inDateRange(revDate)) {
							...
							SCTFile fo = new SCTFile(f2, wDir, subDir, revDate,
									sctDir);
							listOfFiles.add(fo);
							getLog().info(
									"::: FILE : " + f2.getName() + " "
											+ revDate + " hasSnomedId="
											+ fo.hasStatedSctRelId
											+ " doCrossMap="
						...
						getLog().info(e);
						getLog().info(
								"::: Date format missing or not supported : "
										+ f2.getName() + " " + revDate);
					}
				}
	...
	public void setDateStart(String sStart) throws MojoFailureException {
		...
		try {
			...
			getLog().info(e);
			throw new MojoFailureException(
					"SimpleDateFormat yyyy.MM.dd dateStart parse error: "
							+ sStart);
		}
	...
	public void setDateStop(String sStop) throws MojoFailureException {
		...
		try {
			...
			getLog().info(e);
			throw new MojoFailureException(
					"SimpleDateFormat yyyy.MM.dd dateStop parse error: "
							+ sStop);
		}
	 ...
	 */
	private void processConceptsFiles(String wDir, List<List<SCTFile>> sctv,
			boolean ctv3idTF, boolean snomedrtTF, ObjectOutputStream oos)
			throws Exception {
	...
	private void processDescriptionsFiles(String wDir,
			List<List<SCTFile>> sctv, ObjectOutputStream oos) throws Exception {
		int count1, count2; // records in arrays 1 & 2
		String fName1, fName2; // file pathIdx name
		int pathID;
		long revTime;
	...
	private void processRelationshipsFiles(String wDir,
			List<List<SCTFile>> sctI, ObjectOutputStream oos,
			ObjectOutputStream oosIds, BufferedWriter er, int user)
			throws Exception {
		int count1, count2; // records in arrays 1 & 2
		String fName1, fName2; // file pathIdx name
		long revTime;
		...
		while (dit.hasNext()) {
			...
			getLog().info(
					"            " + a1.length + " after non-defining filter");
			a1 = removeDuplRels(a1);
			getLog().info(
					"            " + a1.length + " after duplicate removal");
			count1 = a1.length;
			...
			while (fit.hasNext()) {
				...
				getLog().info(
						"            " + a2.length
								+ " after non-defining filter");
				a2 = removeDuplRels(a2);
				getLog().info(
						"            " + a2.length + " after duplicate removal");
				count2 = a2.length;

				int r1 = 0, r2 = 0, r3 = 0; // reset record indices
				int nSame = 0, nMod = 0, nAdd = 0, nDrop = 0; // counters
				int nModSidChange = 0, nSidOnlyChange = 0; // counters related
															// to SNOMED_ID
															// change
				int nWrite = 0; // counter for memory optimization for object
								// files writing
				...
				while ((r1 < count1) && (r2 < count2)) {
					...
					// :DEBUG:
					// if (debug)
					// if ((a1[r1].relSnoId == 2455349029L || a2[r2].relSnoId ==
					// 2455349029L)
					// || (a1[r1].relSnoId == 2671123026L || a2[r2].relSnoId ==
					// 2671123026L)) {
					...
					case 5: // SAME LOGICAL RELATIONSHIP, CHANGED SNOMED_ID
						...
						if (a1[r1].relSnoId < Long.MAX_VALUE) {
							idOnlyChange = new Sct1_IdRecord(a1[r1].relUuidMsb,
									a1[r1].relUuidLsb, uuidSourceSnomedIdx,
									a1[r1].relSnoId, 1, revTime,
									a1[r1].pathIdx, user);
						...
						if (a2[r2].relSnoId < Long.MAX_VALUE) {
							idOnlyChange = new Sct1_IdRecord(a2[r2].relUuidMsb,
									a2[r2].relUuidLsb, uuidSourceSnomedIdx,
									a2[r2].relSnoId, 0, revTime,
									a2[r2].pathIdx, user);
							oosIds.writeUnshared(idOnlyChange);
						}
					...
					case 2: // SAME LOGICAL RELATIONSHIP, SAME SNOMED_ID,
							// MODIFIED OTHER
						// Write history
						a2[r2].revTime = revTime;
						oos.writeUnshared(a2[r2]);
					...
					case 7: // SAME LOGICAL RELATIONSHIP, SAME SNOMED_ID,
							// MODIFIED USER
					...
					case 6: // SAME LOGICAL RELATIONSHIP, CHANGED SNOMED_ID,
							// MODIFIED OTHER
						// Write history
						a2[r2].revTime = revTime;
						oos.writeUnshared(a2[r2]);
						...
						if (a1[r1].relSnoId < Long.MAX_VALUE) {
							idMod = new Sct1_IdRecord(a1[r1].relUuidMsb,
									a1[r1].relUuidLsb, uuidSourceSnomedIdx,
									a1[r1].relSnoId, 1, revTime,
									a1[r1].pathIdx, user);
						...
						// WRITE CURRENT SNOMED_ID
						if (a2[r2].relSnoId < Long.MAX_VALUE) {
							idMod = new Sct1_IdRecord(a2[r2].relUuidMsb,
									a2[r2].relUuidLsb, uuidSourceSnomedIdx,
									a2[r2].relSnoId, 0, revTime,
									a2[r2].pathIdx, user);
							oosIds.writeUnshared(idMod);
					...
					case 8: // MODIFIED LOGICAL RELATIONSHIP, CHANGED SNOMED_ID,
							// CHANGED USER
						// RETIRE PREVIOUS USER
						a1[r1].status = 1; // RETIRE OLD USER
						a1[r1].revTime = revTime;
						...
						if (a1[r1].relSnoId < Long.MAX_VALUE) {
							idMod2 = new Sct1_IdRecord(a1[r1].relUuidMsb,
									a1[r1].relUuidLsb, uuidSourceSnomedIdx,
									a1[r1].relSnoId, 1, revTime,
									a1[r1].pathIdx, user);
						...
						// WRITE CURRENT SNOMED_ID
						if (a2[r2].relSnoId < Long.MAX_VALUE) {
							idMod2 = new Sct1_IdRecord(a2[r2].relUuidMsb,
									a2[r2].relUuidLsb, uuidSourceSnomedIdx,
									a2[r2].relSnoId, 0, revTime,
									a2[r2].pathIdx, user);
							oosIds.writeUnshared(idMod2);
					...
					case 3: // ADDED LOGICAL RELATIONSHIP
						...
						// WRITE CURRENT SNOMED_ID
						if (a2[r2].relSnoId < Long.MAX_VALUE) {
							Sct1_IdRecord idAdded = new Sct1_IdRecord(
									a2[r2].relUuidMsb, a2[r2].relUuidLsb,
									uuidSourceSnomedIdx, a2[r2].relSnoId,
									a2[r2].status, revTime, a2[r2].pathIdx,
									user);
					...
					case 4: // DROPPED LOGICAL RELATIONSHIP
						...
						if (a1[r1].status != 1) { // if not RETIRED
							...
							// RETIRE EXISTING SNOMED_ID
							if (a1[r1].relSnoId < Long.MAX_VALUE) {
								Sct1_IdRecord idDropped = new Sct1_IdRecord(
										a1[r1].relUuidMsb, a1[r1].relUuidLsb,
										uuidSourceSnomedIdx, a1[r1].relSnoId,
										a1[r1].status, revTime, a1[r1].pathIdx,
										user);
	...
	private int compareDescription(Sct1_DesRecord d1, Sct1_DesRecord d2) {
		...
		if (d1.desUuidMsb == d2.desUuidMsb && d1.desUuidLsb == d2.desUuidLsb) {
			if ((d1.status == d2.status) && (d1.conSnoId == d2.conSnoId)
					&& d1.termText.contentEquals(d2.termText)
	...
	private int compareRelationship(Sct1_RelRecord c1, Sct1_RelRecord c2) {
		if (c1.relUuidMsb == c2.relUuidMsb && c1.relUuidLsb == c2.relUuidLsb) {
			...
			if ((c1.status == c2.status)
					...
					&& (c1.group == c2.group) && (c1.moduleIdx == c2.moduleIdx)) {
				...
				if (isUserChanged(c1.characteristic, c2.characteristic) == false) {
					return 2; // SAME LOGICAL REL, SAME SNOMED_ID, MODIFIED
								// OTHER
				} else {
	...
	private void parseConcepts(String fName, Sct1_ConRecord[] a, int count,
		...
		while (br.ready()) {
			...
			a[concepts] = new Sct1_ConRecord(conceptKey, conceptStatus,
	...
	private void parseDescriptions(String fName, Sct1_DesRecord[] a, int count)
			throws Exception {
		...
		while (r.ready()) {
			...
			// Save to sortable array
			a[descriptions] = new Sct1_DesRecord(descriptionId, status,
					conSnoId, text, capStatus, typeInt, lang);
			descriptions++;
	...
	private Sct1_RelRecord[] parseRelationships(String fName,
			Sct1_RelRecord[] a, int count, SCTFile f) throws Exception {
		...
		while (br.ready()) {
			...
			if ((f.isStated == true && f.hasStatedSctRelId)
					|| f.isStated == false) {
			...
			if (characteristic == 0 && f.isStated) {
				...
				characteristic = STATED_CHAR_WORKAROUND; // :NOTE: transient use
															// for
															// STATED_RELATIONSHIP
			...
			// 0=Defining, 1=Qualifier, 2=Historical, 3=Additional,
			// 5=STATED_CHAR_WORKAROUND
			if (characteristic == 0 || (characteristic == 1 && f.keepQualifier)
				...
				a[relationships] = new Sct1_RelRecord(relID, status,
						conceptOneID, roleTypeSnoId, roleTypeIdx, conceptTwoID,
						characteristic, refinability, group, pathIdx, userIdx);
				relationships++;
			} else {
				// :NYI: count "not kept"
			}
			// if (conceptOneID == 391181005 && roleTypeSnoId == 116680003 &&
			// conceptTwoID == 6254007) {
		...
		if (useSctRelId) {
			...
			// :DEPRECIATED:
			computeRelationshipUuids_Old(a, f.hasStatedSctRelId,
					f.mapSctIdInferredToStated);
		}
		Arrays.sort(a);
	...
	private void computeRelationshipUuids(Sct1_RelRecord[] a, boolean isStated,
			boolean isStatedSctRelIdPresent) throws NoSuchAlgorithmException,
		...
		for (int i = 0; i < max; i++) {
			...
			if (isStated == true && isStatedSctRelIdPresent == false) {
				UUID uuid = Type5UuidFactory.get(REL_ID_NAMESPACE_UUID_TYPE1
						+ a[i].c1SnoId + a[i].roleTypeSnoId + a[i].c2SnoId
						+ GroupListStr);
	...
	private void computeRelationshipUuids_Old(Sct1_RelRecord[] a,
			boolean hasSnomedId, boolean doCrossMap)
		...
		for (int i = 0; i < max; i++) {
			...
			// :DEBUG
			// if
			// (uuid.toString().compareToIgnoreCase("8003f34d-e069-57a5-b7db-919fec994ced")
			// == 0)
			...
			// ")");
			// if (a[i].c1SnoId == 391181005 && a[i].roleTypeSnoId == 116680003
			// && a[i].c2SnoId == 6254007) {
			...
			// UPDATE SNOMED ID
			if (doCrossMap) {
				if (hasSnomedId) // relUuidMap.put(a[i].relUuidMsb,
									// a[i].relUuidLsb, a[i].relSnoId);
				{
					relUuidMap.put(uuid, new Long(a[i].relSnoId));
				} else {
					// a[i].relSnoId = relUuidMap.get(a[i].relUuidMsb,
					// a[i].relUuidLsb);
	...
	private String getGroupListString(Sct1_RelRecord[] a, int startIdx) {
		...
		if (a[startIdx].group > 0) {
			...
			int i = startIdx;
			while ((i < max - 1) && (a[i].c1SnoId == keepC1)
					&& (a[i].group == keepGroup)) {
	...
	private void writeConcepts(ObjectOutputStream oos, Sct1_ConRecord[] a,
			int count, long releaseDateTime, int pathIdx) throws Exception {
	...
	private void writeDescriptions(ObjectOutputStream oos, Sct1_DesRecord[] a,
			int count, long releaseDateTime, int pathIdx) throws Exception {
	...
	private void writeRelationships(ObjectOutputStream oos,
			ObjectOutputStream oosIds, Sct1_RelRecord[] a, int count,
			long releaseDateTime, int user) throws Exception {
		...
		for (int i = 0; i < count; i++) {
			...
			if (a[i].relSnoId < Long.MAX_VALUE) {
				Sct1_IdRecord id = new Sct1_IdRecord(a[i].relUuidMsb,
						a[i].relUuidLsb, uuidSourceSnomedIdx, a[i].relSnoId,
						a[i].status, a[i].revTime, a[i].pathIdx, user);
	...
	private void stateSave(String wDir) {
		try {
			String fNameState = wDir + scratchDirectory + FILE_SEPARATOR
					+ "state.ser";
			ObjectOutputStream oos = new ObjectOutputStream(
					new BufferedOutputStream(new FileOutputStream(fNameState)));
	...
	private void stateRestore(String wDir) {
		try {
			String fNameState = wDir + scratchDirectory + FILE_SEPARATOR
					+ "state.ser";
			ObjectInputStream ois = new ObjectInputStream(
					new BufferedInputStream(new FileInputStream(fNameState)));
	...
	private long convertDateStrToTime(String date) throws MojoFailureException {
		try {
			...
			Logger.getLogger(Sct1ArfToEConceptMojo.class.getName()).log(
	...
	private void countCheck(int count1, int count2, int same, int modified,
			int added, int dropped, int idmod, int idonly) {

		// CHECK COUNTS TO MASTER FILE1 RECORD COUNT
		if ((same + modified + dropped + idmod + idonly) == count1) {
			getLog().info(
					"PASSED1:: SAME+MODIFIED+DROPPED+MODIFIED_IDCHANGE+IDCHANGEONLY = "
							+ same + "+" + modified + "+" + dropped + "+"
							+ idmod + "+" + idonly + " = "
							+ (same + modified + dropped + idmod + idonly)
							+ " == " + count1);
		} else {
			getLog().info(
					"FAILED1:: SAME+MODIFIED+DROPPED+MODIFIED_IDCHANGE+IDCHANGEONLY = "
							+ same + "+" + modified + "+" + dropped + "+"
							+ idmod + "+" + idonly + " = "
							+ (same + modified + dropped + idmod + idonly)
							+ " != " + count1);
		...
		if ((same + modified + added + idmod + idonly) == count2) {
			getLog().info(
					"PASSED2:: SAME+MODIFIED+ADDED+MODIFIED_IDCHANGE+IDCHANGEONLY   = "
							+ same + "+" + modified + "+" + added + "+" + idmod
							+ "+" + idonly + " = "
							+ (same + modified + added + idmod + idonly)
							+ " == " + count2);
		} else {
			getLog().info(
					"FAILED2:: SAME+MODIFIED+ADDED+MODIFIED_IDCHANGE+IDCHANGEONLY   = "
							+ same + "+" + modified + "+" + added + "+" + idmod
							+ "+" + idonly + " = "
							+ (same + modified + added + idmod + idonly)
							+ " != " + count2);
	...
	private int countFileObjects(String fName) throws FileNotFoundException,
		...
		try {
			...
			getLog().info(
					" object count = " + objCount + " @EOF " + fName + "\r\n");
		}
	...
	private String getFileRevDate(File f) throws MojoFailureException {
		...
		s1 = s1.substring(0, 4) + "-" + s1.substring(4, 6) + "-"
				+ s1.substring(6);
