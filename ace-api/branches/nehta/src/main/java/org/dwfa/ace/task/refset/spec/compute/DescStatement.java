...
public class DescStatement extends RefsetSpecStatement {
    ...
    public boolean getStatementResult(I_AmTermComponent component)
        ...
        if (queryToken.equals(termFactory
            .getConcept(RefsetAuxiliary.Concept.DESC_IS_MEMBER_OF.getUids()))) {
            return descriptionIsMemberOf(descriptionTuple);
        } else if (queryToken.equals(termFactory
            .getConcept(RefsetAuxiliary.Concept.DESC_STATUS_IS.getUids()))) {
            return componentStatusIs(descriptionTuple);
        } else if (queryToken.equals(termFactory
            .getConcept(RefsetAuxiliary.Concept.DESC_STATUS_IS_KIND_OF
                .getUids()))) {
            return componentStatusIsKindOf(descriptionTuple);
        } else if (queryToken.equals(termFactory
            .getConcept(RefsetAuxiliary.Concept.DESC_TYPE_IS.getUids()))) {
            return descriptionTypeIs(descriptionTuple);
        } else if (queryToken
            .equals(termFactory
                .getConcept(RefsetAuxiliary.Concept.DESC_TYPE_IS_KIND_OF
                    .getUids()))) {
            return descriptionTypeIsKindOf(descriptionTuple);
        } else if (queryToken.equals(termFactory
            .getConcept(RefsetAuxiliary.Concept.DESC_LUCENE_MATCH.getUids()))) {
            return descriptionRegexMatch(descriptionTuple);
        } else if (queryToken.equals(termFactory
            .getConcept(RefsetAuxiliary.Concept.DESC_REGEX_MATCH.getUids()))) {
