...
public class MigrateAllSpecRefsets extends AbstractTask {
    ...
    public void init() throws Exception {
        ...
        concepts.put("DEFINING_CHARACTERISTIC", termFactory.getConcept(ArchitectonicAuxiliary.Concept.DEFINING_CHARACTERISTIC.localize().getNid()));
        concepts.put("OPTIONAL_REFINABILITY", termFactory.getConcept(ArchitectonicAuxiliary.Concept.OPTIONAL_REFINABILITY.getUids()));
    ...
    private void copySpecRefsetExtensions(I_GetConceptData specRefsetConcept, I_GetConceptData memberRefsetConcept)
        ...
        if (exclusionExtRefset != refsetHelper.ID_NOT_FOUND) {
            termFactory.newRelationship(
                    UUID.randomUUID(), memberRefsetConcept, concepts.get("EXCLUDE_MEMBERS_REL_TYPE"),
                    termFactory.getConcept(exclusionExtRefset), concepts.get("DEFINING_CHARACTERISTIC"),
                    concepts.get("OPTIONAL_REFINABILITY"), concepts.get("CURRENT"), 0, config);
        }
    }

    /**
     * The 'member' refset need a relationship to the new 'marked parent member' refset
     */
    private void linkMemberRefsetToParentRefset(I_GetConceptData memberRefsetConcept, I_GetConceptData parentMemberRefset, I_GetConceptData parentRefsetRel)
            throws Exception {

        termFactory.newRelationship(
                UUID.randomUUID(), memberRefsetConcept, parentRefsetRel, parentMemberRefset,
                concepts.get("DEFINING_CHARACTERISTIC"), concepts.get("OPTIONAL_REFINABILITY"), concepts.get("CURRENT"), 0, config);

    }

    /**
     * Creates the new type of relationship to link a member refset with another refset containing its marked parent members
     */
    private I_GetConceptData createRefsetRel() throws Exception {
        ...
        termFactory.newRelationship(
                UUID.randomUUID(), newRefsetRel, concepts.get("SNOMED_IS_A"), concepts.get("REFSET_RELATIONSHIP"),
                concepts.get("DEFINING_CHARACTERISTIC"), concepts.get("OPTIONAL_REFINABILITY"), concepts.get("CURRENT"), 0, config);
    ...
    /**
     * Creates the new purpose which can be given the marked parent member refset concepts
     */
	private I_GetConceptData createParentRefsetPurpose() throws Exception {
        ...
        termFactory.newRelationship(
                UUID.randomUUID(), newRefsetPurpose, concepts.get("SNOMED_IS_A"), concepts.get("REFSET_PURPOSE"),
                concepts.get("DEFINING_CHARACTERISTIC"), concepts.get("OPTIONAL_REFINABILITY"), concepts.get("CURRENT"), 0, config);
	...
	private void addMemberRefsetPurpose(I_GetConceptData memberRefsetConcept, I_GetConceptData refsetPurpose) 
		...
		termFactory.newRelationship(
				UUID.randomUUID(), memberRefsetConcept, concepts.get("REFSET_PURPOSE"), refsetPurpose, 
				concepts.get("DEFINING_CHARACTERISTIC"), concepts.get("OPTIONAL_REFINABILITY"), concepts.get("CURRENT"), 0, config);			
	}
     ...
     */
    private I_GetConceptData createParentMemberRefset(I_GetConceptData parentHierarchy, I_GetConceptData memberRefsetConcept, I_GetConceptData refsetPurpose)
            throws Exception {
        ...
        termFactory.newDescription(
                UUID.randomUUID(), newParentMemberConcept, "en", refsetName,
                ArchitectonicAuxiliary.Concept.FULLY_SPECIFIED_DESCRIPTION_TYPE.localize(), config);
        ...
        termFactory.newRelationship(
                UUID.randomUUID(), newParentMemberConcept, concepts.get("SNOMED_IS_A"), parentHierarchy,
                concepts.get("DEFINING_CHARACTERISTIC"), concepts.get("OPTIONAL_REFINABILITY"), concepts.get("CURRENT"), 0, config);

        termFactory.newRelationship(
                UUID.randomUUID(), newParentMemberConcept, concepts.get("REFSET_TYPE_REL"), concepts.get("CONCEPT_EXTENSION"),
                concepts.get("DEFINING_CHARACTERISTIC"), concepts.get("OPTIONAL_REFINABILITY"), concepts.get("CURRENT"), 0, config);

        termFactory.newRelationship(
                UUID.randomUUID(), newParentMemberConcept, concepts.get("REFSET_PURPOSE"), refsetPurpose,
                concepts.get("DEFINING_CHARACTERISTIC"), concepts.get("OPTIONAL_REFINABILITY"), concepts.get("CURRENT"), 0, config);
    ...
    private I_GetConceptData createParentMemberHierarchy() throws Exception {
        ...
        termFactory.newDescription(
                UUID.randomUUID(), newHierarchyConcept, "en", PARENT_MEMBER_HIERARCHY_NAME,
                ArchitectonicAuxiliary.Concept.FULLY_SPECIFIED_DESCRIPTION_TYPE.localize(), config);
        ...
        termFactory.newRelationship(
                UUID.randomUUID(), newHierarchyConcept, concepts.get("SNOMED_IS_A"), concepts.get("REFSET_IDENTITY"),
                concepts.get("DEFINING_CHARACTERISTIC"), concepts.get("OPTIONAL_REFINABILITY"), concepts.get("CURRENT"), 0, config);
	...
	public Condition evaluate(I_EncodeBusinessProcess process, I_Work worker) throws TaskFailedException {
        try {
            ...
            for (Integer specRefsetId : specificationRefsets) {
                ...
                I_GetConceptData parentMemberRefset = 
                	createParentMemberRefset(parentRefsetHierarchy, memberRefsetConcept, parentRefsetPurpose);
