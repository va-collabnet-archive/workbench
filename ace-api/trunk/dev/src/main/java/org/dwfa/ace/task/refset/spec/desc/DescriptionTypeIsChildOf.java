...
/**
 * Task for adding a "description type is child of" structural query to a refset spec.
 * 
 * @author Chrissy Hill
 * 
 */
@BeanList(specs = { @Spec(directory = "tasks/refset/spec/desc", type = BeanType.TASK_BEAN) })
public class DescriptionTypeIsChildOf extends AddStructuralQueryToRefsetSpec {
    /**
	 * 
	 */
    ...
    protected int getStructuralQueryTokenId() throws IOException, TerminologyException {
        return RefsetAuxiliary.Concept.DESC_TYPE_IS_CHILD_OF.localize().getNid();
    }
