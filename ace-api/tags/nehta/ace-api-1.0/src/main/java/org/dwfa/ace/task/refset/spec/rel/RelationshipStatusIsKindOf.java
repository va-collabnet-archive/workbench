...
/**
 * Task for adding a "relationship status is kind of" structural query to a refset spec.
 * @author Chrissy Hill
 *
 */
@BeanList(specs = { @Spec(directory = "tasks/refset/spec", type = BeanType.TASK_BEAN) })
public class RelationshipStatusIsKindOf extends AddStructuralQueryToRefsetSpec {
	/**
	 * 
	 */
	...
	protected int getStructuralQueryTokenId() throws IOException, TerminologyException {
		return RefsetAuxiliary.Concept.REL_STATUS_IS_KIND_OF.localize().getNid();
	}
