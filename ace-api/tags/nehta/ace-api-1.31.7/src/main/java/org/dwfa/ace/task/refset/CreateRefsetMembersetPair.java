...
//	private String newConceptName = "newConcpt";
	...
	throws TaskFailedException {
		...
		try{
			...
			if(isValidParent()){
				...
				JOptionPane.showMessageDialog(frame, "Unable to create a refset under the selected concept in the heirachy. Please select a refset concept and try again.","Invalid Selection",JOptionPane.WARNING_MESSAGE);
	...
	private boolean isValidParent() throws TaskFailedException{
		try{
			 ...
			 */
			I_GetConceptData parentConcept = selectedConcept.getSourceRelTargets(null, null, null, false).iterator().next(); 
	...
	private void createMembersetRels(I_GetConceptData concept) throws TaskFailedException{
		try{	
			...
			termFactory.newRelationship(UUID.randomUUID()
					, concept
					, termFactory.getConcept(ConceptConstants.REFSET_TYPE_REL.localize().getNid()) //rel type
					, termFactory.getConcept(RefsetAuxiliary.Concept.CONCEPT_EXTENSION.localize().getNid()) //dest concept
					, relChar
					, refinability
					, relStat
					, 0
	...
	private void createRefsetRels(I_GetConceptData concept) throws TaskFailedException{
		try{	
			...
			I_GetConceptData relChar = termFactory.getConcept(ArchitectonicAuxiliary.Concept.STATED_RELATIONSHIP.localize().getNid());
			...
			termFactory.newRelationship(UUID.randomUUID()
					, concept
					, termFactory.getConcept(RefsetAuxiliary.Concept.REFSET_RELATIONSHIP.REFSET_TYPE_REL.localize().getNid()) //rel type
					, termFactory.getConcept(RefsetAuxiliary.Concept.CONCEPT_EXTENSION.localize().getNid()) //dest concept
					, relChar
					, refinability
					, relStat
			...
			termFactory.newRelationship(UUID.randomUUID()
					, concept
					, termFactory.getConcept(ConceptConstants.REFSET_PURPOSE_REL.localize().getNid()) //rel type
					, termFactory.getConcept(RefsetAuxiliary.Concept.INCLUSION_SPECIFICATION.localize().getNid()) //dest concept
					, relChar
					, refinability
					, relStat
					, 0
			...
			termFactory.newRelationship(UUID.randomUUID()
					, concept
					, termFactory.getConcept(ConceptConstants.GENERATES_REL.localize().getNid()) //rel type
