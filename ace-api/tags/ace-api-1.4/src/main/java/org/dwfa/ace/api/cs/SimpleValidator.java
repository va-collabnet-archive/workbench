...
public class SimpleValidator implements I_ValidateChangeSetChanges {
    ...
    public boolean validateChange(I_AmChangeSetObject csObj, I_TermFactory tf)
        ...
        } else if (UniversalAceExtByRefBean.class.isAssignableFrom(csObj
            .getClass())) {
            if (AceLog.getEditLog().isLoggable(Level.FINE)) {
                AceLog.getEditLog().fine(
                    "Read UniversalAceExtByRefBean... " + csObj);
            }
            return validateAceExtByRefBean((UniversalAceExtByRefBean) csObj, tf);
        } else {
            throw new IOException("Can't handle class: "
                + csObj.getClass().getName());
        }
    }

    protected boolean validateAceExtByRefBean(UniversalAceExtByRefBean bean,
            I_TermFactory tf) throws IOException, TerminologyException {
        return false;
    }

    protected boolean validateAcePath(UniversalAcePath path, I_TermFactory tf)
            throws IOException, TerminologyException {
        return false;
    }

    protected boolean validateAceIdList(UniversalIdList bean, I_TermFactory tf)
            throws IOException, TerminologyException {
        return true;
    }
