...
public class ConceptConceptConceptExtTupleFileUtil {
    ...
    public static String exportTuple(I_ThinExtByRefTuple tuple)
        ...
        try {
            ...
            UUID tupleUuid =
                    ArchitectonicAuxiliary.Concept.EXT_CONCEPT_CONCEPT_CONCEPT_TUPLE
            ...
            UUID componentUuid =
                    termFactory.getUids(tuple.getComponentId()).iterator()
                        .next();
            UUID typeUuid =
                    termFactory.getUids(tuple.getTypeId()).iterator().next(); // this
            ...
            I_ThinExtByRefPartConceptConceptConcept part =
                    (I_ThinExtByRefPartConceptConceptConcept) tuple.getPart();
            ...
            return tupleUuid + "\t" + memberUuid + "\t" + refsetUuid + "\t"
                ...
                + c2Uuid + "\t" + c3Uuid + "\t" + pathUuid + "\t" + statusUuid
                + "\t" + effectiveDate + "\n";
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static boolean importTuple(String inputLine,
        ...
        try {
            ...
            if (!termFactory.hasId(c1Uuid)) {
                String errorMessage =
                        "c1Uuid UUID has no identifier - importing with temporary assigned ID.";
                outputFileWriter.write("Error on line " + lineCount + " : ");
                outputFileWriter.write(errorMessage);
                outputFileWriter.newLine();
            ...
            if (!termFactory.hasId(c2Uuid)) {
                String errorMessage =
                        "c2Uuid has no identifier - importing with temporary assigned ID.";
                outputFileWriter.write("Error on line " + lineCount + " : ");
                outputFileWriter.write(errorMessage);
                outputFileWriter.newLine();
            ...
            if (!termFactory.hasId(c3Uuid)) {
                String errorMessage =
                        "c3Uuid has no identifier - importing with temporary assigned ID.";
                outputFileWriter.write("Error on line " + lineCount + " : ");
                outputFileWriter.write(errorMessage);
                outputFileWriter.newLine();
            ...
            try {
                refsetHelper.newConceptConceptConceptRefsetExtension(
                    termFactory.getId(refsetUuid).getNativeId(), termFactory
                        .getId(componentUuid).getNativeId(), termFactory.getId(
                        c1Uuid).getNativeId(), termFactory.getId(c2Uuid)
                        .getNativeId(),
            ...
            e.printStackTrace();
            String errorMessage =
                    "Exception of unknown cause thrown while importing concept-concept-concept ext tuple";
            try {
                outputFileWriter.write("Error on line " + lineCount + " : ");
                outputFileWriter.write(errorMessage);
                outputFileWriter.newLine();
