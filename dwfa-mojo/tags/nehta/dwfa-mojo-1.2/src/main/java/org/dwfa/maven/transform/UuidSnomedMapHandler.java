...
public class UuidSnomedMapHandler {
    ...
    public UuidSnomedMapHandler(File idGeneratedDir, File sourceDirectory, boolean useDbSctIdMap) throws IOException, ClassNotFoundException {
    ...
    private void init(File idGeneratedDir, File sourceDirectory, final boolean useDbSctIdMap) throws IOException, ClassNotFoundException {
        if (mapMap == null) {
			...
			for (final TYPE type : TYPE.values()) {
				...
				mapMap.put(type, DerbyBackedUuidSctidMap.read(fileMap.get(type),
						namespace, project));
			}
			for (File fixedMapFile : idSourceDir.listFiles(new SctMapFilter())) {
                Map<UUID, Long> fixedMap;
                if (useDbSctIdMap) {
                    fixedMap = DerbyBackedUuidSctidFixedMap
                    .read(fixedMapFile);
                } else {
                    fixedMap = UuidSnomedFixedMap
                        .read(fixedMapFile);
                }
