...
public class UuidToSctRelIdWithGenerationTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        uuidToSctRelIdWithGeneration.setupImpl(null);
    }
    ...
    public void testConceptIdGeneration() throws Exception {
        ...
        String sctId = uuidToSctRelIdWithGeneration.transform(uuid.toString(), namespace, project);

        assertTrue("Must return the same sctId for the same UUID", sctId.equals(uuidToSctRelIdWithGeneration.transform(
            uuid.toString(), namespace, project)));
    }

    public void testLoad() throws Exception {
        for (int i = 0; i < loadTestSize; i++) {
            String sctId = uuidToSctRelIdWithGeneration.transform(UUID.randomUUID().toString(), getRandomNamespace(), getRandomProject());
            assertNotNull(sctId);
        }
    }
}
