...
public class QueueViewerPanel extends JPanel {
    ...
    private class RefreshListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent arg0) {
            QueueAdaptor qAdaptor =  (QueueAdaptor) tableOfQueues.getModel().getValueAt(tableOfQueues.getSelectedRow(), 0);
 
            if (qAdaptor == null) {
                // nothing to do...
            } else {
                try {
                    tableOfQueueEntriesModel = new QueueTableModel(
                            qAdaptor.queue);
                    tableOfQueueEntriesSorter = new TableSorter(
                            tableOfQueueEntriesModel);
                    tableOfQueueEntries = new JTable(tableOfQueueEntriesSorter);
                    tableOfQueueEntriesSorter.setTableHeader(tableOfQueueEntries
                            .getTableHeader());

                    // Set up tool tips for column headers.
                    tableOfQueueEntriesSorter
                            .getTableHeader()
                            .setToolTipText(
                                    "Click to specify sorting; Control-Click to specify secondary sorting");
                    tableOfQueueEntries.getSelectionModel()
                            .addListSelectionListener(
                                    new ProcessSelectionActionListener(tableOfQueueEntriesSorter));
                    int dividerLocation = queueContentsSplitPane
                            .getDividerLocation();
            ...
            if (lsm.isSelectionEmpty()) {
                ...
                try {
                    ...
                    int dividerLoc = queueContentsSplitPane
                            .getDividerLocation();
                    queueContentsSplitPane.setBottomComponent(new JScrollPane(
                            processPanel));
                    queueContentsSplitPane.setDividerLocation(dividerLoc);
    ...
    public QueueViewerPanel(Configuration jiniConfig, I_Work worker)
        ...
        this.execute.addActionListener(new ExecuteProcessActionListener(
    ...
    private class GetQueuesAsWorker implements PrivilegedExceptionAction {
        ...
        public Object run() throws Exception {
            ...
            for (int i = 0; i < services.length; i++) {
                logger.info("Service[" + i + "]: " + services[i].service
                        + " Attributes: "
