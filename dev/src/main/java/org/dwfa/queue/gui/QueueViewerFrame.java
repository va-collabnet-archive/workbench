...
public class QueueViewerFrame extends ComponentFrame {
	...
	protected static Logger logger = Logger
			.getLogger("org.dwfa.queue.gui.QueueViewer");
	...
	public class AddRemoteQueue implements ActionListener, Runnable {
		...
		private class GetSelectedQueueAsWorker implements
			...
			public Object run() throws Exception {
				...
				authorityServices = worker.lookup(template, 1, 20, null,
						1000 * 60);
				if ((authorityServices == null)
					...
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							JOptionPane
									.showMessageDialog(QueueViewerFrame.this,
											"No queues found...");
						}
					});
					return null;
				...
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						try {
							ServiceItem item = (ServiceItem) worker
							...
							if (item != null) {
								...
								queue = (I_QueueProcesses) worker.prepareProxy(
	...
	public void addAppMenus(JMenuBar mainMenuBar) {
		...
		queueMenu.add(moveToDiskMI = new JMenuItem(
