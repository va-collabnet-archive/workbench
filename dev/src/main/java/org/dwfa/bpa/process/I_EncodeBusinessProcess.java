...
/**
 * Business Processes are a series of tasks encoded as JavaBeans. <p>  
 * 
 * No concurrent threads of execution, but tasks can 
 * launch processes, which can execute concurrently. <p>
 * 
 * Business Processes implement the <code>I_DefineTask</code> interface, so that
 * processes themselves can encapsulate a series of tasks inside of other processes. 
 * 
 * 
 * @author kec
 *
...
public interface I_EncodeBusinessProcess extends I_DefineTask, I_ManageProperties, I_DescribeBusinessProcess  {
    ...
    /**
     * <code>NAME</code> is the bound property name for a descriptive name of this business process. 
     */
    public static final String NAME = "name";
    /**
     * <code>PROCESS_ID</code> is the bound property name for the identifier of the business process. 
     */
    public static final String PROCESS_ID = "processID";
    
    /**
     * Prerequisites allow the business process to define conditions that must
     * be met before this process can be executed. Examples might include a specific
     ...
     * on this task to corespond with the processes internal representation of tasks.
     * @param task A new task that has not yet been added to another business process. 
    ...
    /**
     * Remove a task from this business process. This method will use the identifier
     ...
     * will be executed (or has been executed of the process has already intiated).
     */
    ...
    /**
     * @param taskId Identifier for the task for with the execution records are requested.
     ...
     * @param worker The worker that provides the resources and thread of execution
     * for this business process. Note that this thread of execution is typicall not the 
     * event dispatch thread, so tasks must ensure that steps that must execute on the event
     * dispatch thread (such as Swing operations) execute on the proper thread. 
    ...
    /**
     * @param address An address where outbox workers deliver this process for execution.
     */
    ...
    /**
     * Task bounds allow layout of a business process with in a GUI builder or viewer. 
    ...
    /**
     * Data containers are a means of representing data accessable to all the tasks
     * within a process. 
    ...
    /**
     * Provides a uniform logging strategy for logging messages by business processes. 
    ...
    /**
     * Takes the attachment from the business processes, and removes the key from
     * the attachment list. 
     * @param key Key for the desired attachment. 
     * @return The attachment associated with the key. 
     */
    ...
    /**
     * Set the name of this business process. The name is used by builder applications to tell the 
    ...
    /**
     * This method provides tasks with a uniform means of resolving URLs that may
     ...
     * @return An <code>InputStream</code> derived from the provided <code>URL</code>.
     */
