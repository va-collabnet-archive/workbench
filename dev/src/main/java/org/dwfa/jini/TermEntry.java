...
public class TermEntry implements Entry, Comparable {
	...
	public int compareTo(Object obj) {
		TermEntry another = (TermEntry) obj;
		try {
			return getLocalDesc().getText().compareTo(
					another.getLocalDesc().getText());
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public I_ConceptualizeLocally getLocalConcept() throws Exception {
		I_StoreLocalFixedTerminology termServer = LocalFixedTerminology
				.getStore();
	...
	public I_DescribeConceptLocally getLocalDesc() throws Exception {
		return getLocalConcept().getDescription(ArchitectonicAuxiliary.getLocalToStringDescPrefList(LocalFixedTerminology.getStore()));
	}

	public I_DescribeConceptUniversally getUniversalDesc(
			I_StoreUniversalFixedTerminology termServer) throws Exception {
