...
public class MojoGraph {
	...
	private void createLegend(Graphics2D g2d){
	  ...
	  private void createGraphAxis(Graphics2D g2d){
		   ...
		   */
//		  g2d.setColor(Color.WHITE);
//		  g2d.fillRoundRect(0,0, graphDisplayWidth, graphDisplayHeight, 10, 10);
		  g2d.setColor(Color.BLACK);
		  ...
		  for(int i=1;i<xAxisMaxIncrements;i++){
			  ...
			  float labelXPosition =  (incrementLabel.length() <= 3) ?  xOffset * 0.75f : (xOffset * 0.75f) - ((font.getSize2D()/2) * (incrementLabel.length() - 3));
		  ...
		  for(int i=1;i<yAxisMaxIncrements;i++){
			  ...
			  case VALUEOVERTIME:
				  ...
				  labelYPosition = (xOffset + (yAxisIncrementSize * i)) -  ((font.getSize2D()/2) * (incrementLabel.length()/2));
			  ...
			  default:
				  incrementLabel = new Integer(yAxisIncrementValue * i).toString();
			  	  ...
			  	  labelYPosition = (xOffset + (yAxisIncrementSize * i)) -  ((font.getSize2D()/2) * incrementLabel.length());
				  g2d.drawString(incrementLabel, labelYPosition, (yOffset + xAxisLength + 15));
			  }
	  ...
	  private void labelAxis(Graphics2D g2d){
		  ...
		  g2d.translate(((Double)(xOffset * 0.25)).intValue(),  (xAxisLength/2) - ((font.getSize2D()/2) * (yAxisLabel.length()/2)) + (yOffset) );
		  ...
		  if (dataSource != null){
			  ...
			  while(it.hasNext()){
				  ...
				  for(int i=0;i < pointData.length; i ++){
					  ...
					  coords.add(new float[]{ (yAxisIncrementSize * (yPointData/yAxisIncrementValue)) + xOffset,
							  				  ((xAxisLength) -  (xAxisIncrementSize * (xPointData/xAxisIncrementValue)))+ yOffset
	  ...
	  private void calcAxisIncrements(float maxXAxisValue, float maxYAxisValue){
		  ...
		  xAxisIncrementValue = ( ((xAxisPlus10PC)/(xAxisLength))* 10  <= 0.0f ) ? 1 :  Math.round( ((xAxisPlus10PC)/(xAxisLength))* 10 );
		  ...
		  yAxisIncrementValue = ( (yAxisPlus10PC/(yAxisLength))* 10 <= 0 ) ? 1 : Math.round( (yAxisPlus10PC/(yAxisLength))* 10 );
