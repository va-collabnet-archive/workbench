...
public class OpcsToAce extends AbstractMojo {
   ...
   private void processRow(Writer concepts, Writer descriptions, Writer relationships, Writer ids, UUID pathUUID, String line) throws IOException, NoSuchAlgorithmException, UnsupportedEncodingException {
      
      String id = line.substring(0, 8).trim();
      String desc = line.substring(8).trim();
      
      ids.append(Type5UuidFactory.get(Type5UuidFactory.OPCS_CONCEPT_ID, id).toString()); // concept id
      ids.append("\t");
      ...
      concepts.append(Type5UuidFactory.get(Type5UuidFactory.OPCS_CONCEPT_ID, id).toString()); // concept id
      concepts.append("\t");
      ...
      descriptions.append("\t");
      descriptions.append(ArchitectonicAuxiliary.Concept.FULLY_SPECIFIED_DESCRIPTION_TYPE.getUids().iterator().next().toString()); //status
      descriptions.append("\t");
   ...
   private void writeRoot(Writer concepts, Writer descriptions, Writer relationships, Writer ids, UUID pathUUID) throws IOException, NoSuchAlgorithmException, UnsupportedEncodingException {
      String id = "opcs";
