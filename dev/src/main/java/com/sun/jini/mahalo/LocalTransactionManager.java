...
/*
 * This file class is based on sun's TxnManagerImpl, but is modified to operate in a local only mode.
 * 
 * 
...
class LocalTransactionManager implements TxnManager,
	...
	static final Logger initLogger = Logger.getLogger(TxnManager.MAHALO
			+ ".init");

	/** Logger for service destruction related messages */
	static final Logger destroyLogger = Logger.getLogger(TxnManager.MAHALO
			+ ".destroy");

	/** Logger for service operation messages */
	static final Logger operationsLogger = Logger.getLogger(TxnManager.MAHALO
			+ ".operations");
	 ...
	 * transition, etc.)
	 */
	static final Logger transactionsLogger = Logger.getLogger(TxnManager.MAHALO
			+ ".transactions");

	/** Logger for transaction participant related messages */
	static final Logger participantLogger = Logger.getLogger(TxnManager.MAHALO
			+ ".participant");

	/** Logger for transaction persistence related messages */
	static final Logger persistenceLogger = Logger.getLogger(TxnManager.MAHALO
			+ ".persistence");
	...
	private void init(String[] configArgs) throws Exception {
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(this.getClass().getName(), "init",
					(Object[]) configArgs);
		}
		...
		if (JiniManager.isLocalOnly()) {
			...
			Entry[] entries = (Entry[]) config.getEntry(TxnManager.MAHALO, 
					"entries", Entry[].class, new Entry[] {});
			entryList.addAll(Arrays.asList(entries));
			ServiceItem serviceItem = new ServiceItem(new ServiceID(topUuid
					.getMostSignificantBits(), topUuid
					.getLeastSignificantBits()), this, (Entry[]) entryList
	...
	private void doInit(Configuration config) throws Exception {
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(this.getClass().getName(), "doInit",
					config);
		}
		...
		txnLeasePeriodPolicy = (LeasePeriodPolicy) Config.getNonNullEntry(
				...
				LeasePeriodPolicy.class, new FixedLeasePeriodPolicy(3 * HOURS,
						1 * HOURS));
		...
		txns = Collections
				.synchronizedMap(new HashMap<Long, LocalTxnManagerTransaction>());

		// Used by log recovery logic
		settlerWakeupMgr = new WakeupManager(new WakeupManager.ThreadDesc(null,
				true));
		taskWakeupMgr = new WakeupManager(new WakeupManager.ThreadDesc(null,
				true));

		settlerpool = (TaskManager) Config.getNonNullEntry(config,
				TxnManager.MAHALO, "settlerPool", TaskManager.class,
				new TaskManager(settlerthreads, settlertimeout, settlerload));
		taskpool = (TaskManager) Config.getNonNullEntry(config,
				TxnManager.MAHALO, "taskPool", TaskManager.class,
	...
	public TransactionManager.Created create(long lease)
		...
		if (transactionsLogger.isLoggable(Level.FINEST)) {
			transactionsLogger.log(Level.FINEST, "Transaction ID is: {0}",
					new Long(tid));
		}

		txntr = new LocalTxnManagerTransaction(serverStub, logmgr, tid, taskpool,
		...
		if (transactionsLogger.isLoggable(Level.FINEST)) {
			transactionsLogger.log(Level.FINEST,
					"Created new LocalTxnManagerTransaction ID is: {0}", new Long(
							tid));
		}
		...
		try {
			...
			if (transactionsLogger.isLoggable(Level.FINEST)) {
				transactionsLogger.log(Level.FINEST,
						"recorded new LocalTxnManagerTransaction", txntr);
			}
			...
			RuntimeException wrap = new RuntimeException(
					"Unable to create transaction", e);
			transactionsLogger.throwing(this.getClass().getName(), "create",
					wrap);
			throw wrap;
		}

		TransactionManager.Created tmp = new TransactionManager.Created(str.id,
				txnmgrlease);
		if (operationsLogger.isLoggable(Level.FINER)) {
	...
	public void join(long id, TransactionParticipant part, long crashCount)
		...
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(this.getClass().getName(), "join",
					new Object[] { new Long(id), part, new Long(crashCount) });
		}
		readyState.check();

		LocalTxnManagerTransaction txntr = (LocalTxnManagerTransaction) txns
				.get(new Long(id));
	...
	public int getState(long id) throws UnknownTransactionException {
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(this.getClass().getName(), "getState",
		...
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.exiting(this.getClass().getName(), "getState",
					new Integer(state));
		}
		return state;
	}
	...
	public void commit(long id) throws UnknownTransactionException,
		...
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(this.getClass().getName(), "commit",
					new Long(id));
		}
		readyState.check();
	...
	public void commit(long id, long waitFor)
		...
		if (transactionsLogger.isLoggable(Level.FINEST)) {
			transactionsLogger.log(Level.FINEST,
					"Retrieved LocalTxnManagerTransaction: {0}", txntr);
		}
	...
	public void abort(long id) throws UnknownTransactionException,
		...
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(this.getClass().getName(), "abort",
					new Object[] { new Long(id) });
		}
		readyState.check();
		try {
	...
	public void abort(long id, long waitFor)
		...
		if (transactionsLogger.isLoggable(Level.FINEST)) {
			transactionsLogger.log(Level.FINEST, "aborted transaction id {0}",
					new Long(id));
		}
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.exiting(this.getClass().getName(), "abort");
	...
	public void recover(long cookie, LogRecord rec) throws LogException {
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(this.getClass().getName(), "recover",
	...
	private synchronized void settleTxns() throws InterruptedException {
		...
		if (transactionsLogger.isLoggable(Level.FINEST)) {
			transactionsLogger.log(Level.FINEST, "Settling {0} transactions.",
		...
		while (true) {
			...
			SettlerTask task = new SettlerTask(settlerpool, settlerWakeupMgr,
					this, tid);
	 ...
	 */
	public long renew(Uuid uuid, long extension) throws UnknownLeaseException,
			LeaseDeniedException {

		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(this.getClass().getName(), "renew",
		...
		synchronized (txntr) {
			...
			if (operationsLogger.isLoggable(Level.FINER)) {
				operationsLogger.exiting(this.getClass().getName(), "renew",
	...
	public void cancel(Uuid uuid) throws UnknownLeaseException {
		...
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(this.getClass().getName(), "cancel",
					new Object[] { uuid });
		}
		readyState.check();
		...
		if (state == ACTIVE) {
			...
			try {
				...
				throw new UnknownLeaseException("When canceling abort threw:"
						+ e.getClass().getName() + ":"
	...
	public Landlord.RenewResults renewAll(Uuid[] cookies, long[] extensions) {
		...
		Landlord.RenewResults results = LandlordUtil.renewAll(this, cookies,
				extensions);
		if (operationsLogger.isLoggable(Level.FINER)) {
	...
	static long nextID() {
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(LocalTransactionManager.class.getName(),
					"nextID");
		}
		long id;
		synchronized (idGen) {
		...
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.exiting(LocalTransactionManager.class.getClass()
					.getName(), "nextID", new Long(id));
		}
		return id;
	}
	...
	private LocalTxnManagerTransaction enterTMT(long cookie) {
		...
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger.entering(this.getClass().getName(), "enterTMT",
					key);
		}
		...
		if (tmt == null) {
			...
			tmt = new LocalTxnManagerTransaction(serverStub, logmgr, cookie,
					taskpool, taskWakeupMgr, this, uuid);
		...
		if (operationsLogger.isLoggable(Level.FINER)) {
			operationsLogger
					.exiting(this.getClass().getName(), "enterTMT", tmt);
		}
		return tmt;
	}
	...
	public void modifyLookupAttributes(Entry[] arg0, Entry[] arg1)
	...
	public void removeLookupLocators(LookupLocator[] arg0)
			throws RemoteException {
		// TODO Auto-generated method stub
