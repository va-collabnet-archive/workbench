...
/**
 * Processes a tab delimited file containing refset specification extensions in
 * the export format.
 * 
 * <p>
 * File columns are:
...
public class ExportedRefsetHandler extends IterableFileReader<I_ThinExtByRefVersioned> {
    ...
    private final DateFormat EXPORTED_DATE_FORMAT = AceDateFormat.getDataInputDateFormat();
    ...
    /**
     * Get the refset type to be used. This may be defined in the mojo
     * configuration otherwise
     * it is determined from the name of the source file being handled.
     * 
     * @throws Exception
     */
    public RefsetType getRefsetType() throws Exception {
        if (refsetType == null && sourceFile != null) {
            return RefsetType.findByFile(sourceFile);
        } else {
            return refsetType;
        }
    }
     ...
     * 
     * @param extensionType Must be a literal name from the enumeration
     *            {@link org.dwfa.ace.refset.RefsetExtensionType}
     */
    ...
    protected I_ThinExtByRefVersioned processLine(String line) {
        try {
            ...
            int typeId = termFactory.uuidToNative(getRefsetType().getAuxiliaryConcept().getUids().iterator().next());
