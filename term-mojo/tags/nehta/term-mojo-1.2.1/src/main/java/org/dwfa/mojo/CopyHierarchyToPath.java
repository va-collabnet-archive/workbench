...
public class CopyHierarchyToPath extends AbstractMojo implements I_ProcessConcepts {
	...
	/**
	 * Allows specification of paths for source data that should be excluded from
	 * the copy
	 * @parameter
	 */
	...
	private void processExtensions(int id)
	...
	private <T extends I_AmTuple> boolean processEntity(I_AmTermComponent termComponent, List<T> allTuples) throws Exception {
		...
		if (latestStateOnly) {
	        ...
	        //then copy the part
	        if (latestPart != null && latestPart.getPathId() != toPathId
