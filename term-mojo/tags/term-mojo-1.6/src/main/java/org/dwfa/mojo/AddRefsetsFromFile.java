...
public class AddRefsetsFromFile extends AbstractMojo {
    ...
    private String findExtensionType(String fileName) {
        ...
        if (typeIndex > -1) {
            if (tmpType[typeIndex].equalsIgnoreCase("boolean"))
                return "BOOLEAN_EXTENSION";
            if (tmpType[typeIndex].equalsIgnoreCase("concept"))
                return "CONCEPT_EXTENSION";
            if (tmpType[typeIndex].equalsIgnoreCase("conint"))
                return "CONCEPT_INT_EXTENSION";
            if (tmpType[typeIndex].equalsIgnoreCase("string"))
                return "STRING_EXTENSION";
            if (tmpType[typeIndex].equalsIgnoreCase("integer"))
                return "INT_EXTENSION";
            if (tmpType[typeIndex].equalsIgnoreCase("measurement"))
                return "MEASUREMENT_EXTENSION";
            if (tmpType[typeIndex].equalsIgnoreCase("language"))
    ...
    private void processFile(File refsetFile) throws MojoExecutionException {
        ...
        try {
            ...
            while (line != null) {
                ...
                int referenceSetId =
                        termFactory.uuidToNative(UUID.fromString(tokens[0]));

                int memberId =
                        termFactory.uuidToNativeWithGeneration(UUID
                            .randomUUID(),
                ...
                int statusId =
                        termFactory.uuidToNative(UUID.fromString(tokens[2]));
                int componentId =
                        termFactory.uuidToNative(UUID.fromString(tokens[3]));
                int pathId =
                        termFactory.uuidToNative(UUID.fromString(tokens[5]));

                I_GetConceptData concept = termFactory.getConcept(componentId);

                int typeId =
                        termFactory
                            .uuidToNative(RefsetAuxiliary.Concept.valueOf(
                                extensionType).getUids().iterator().next());
                ...
                if (termFactory
                    .uuidToNative(RefsetAuxiliary.Concept.BOOLEAN_EXTENSION
                        .getUids()) == typeId) {
                    extPart = termFactory.newBooleanExtensionPart();
                    ((I_ThinExtByRefPartBoolean) extPart).setValue(new Boolean(
                        tokens[6]).booleanValue());
                } else if (termFactory
                    .uuidToNative(RefsetAuxiliary.Concept.CONCEPT_EXTENSION
                        .getUids()) == typeId) {
                    extPart = termFactory.newConceptExtensionPart();
                    ((I_ThinExtByRefPartConcept) extPart)
                        .setConceptId(componentId);
                } else if (termFactory
                    .uuidToNative(RefsetAuxiliary.Concept.CONCEPT_INT_EXTENSION
                        .getUids()) == typeId) {
                    extPart = termFactory.newConceptIntExtensionPart();
                    ((I_ThinExtByRefPartConceptInt) extPart)
                        .setConceptId(componentId);
                    ((I_ThinExtByRefPartConceptInt) extPart)
                        .setIntValue(new Integer(tokens[6]).intValue());
                } else if (termFactory
                    .uuidToNative(RefsetAuxiliary.Concept.STRING_EXTENSION
                        .getUids()) == typeId) {
                    extPart = termFactory.newStringExtensionPart();
                    ((I_ThinExtByRefPartString) extPart)
                        .setStringValue(tokens[6]);
                } else if (termFactory
                    .uuidToNative(RefsetAuxiliary.Concept.INT_EXTENSION
                        .getUids()) == typeId) {
                    extPart = termFactory.newIntegerExtensionPart();
                    ((I_ThinExtByRefPartInteger) extPart).setValue(new Integer(
                        tokens[6]).intValue());
                } else if (termFactory
                    .uuidToNative(RefsetAuxiliary.Concept.MEASUREMENT_EXTENSION
                        .getUids()) == typeId) {
                    extPart = termFactory.newMeasurementExtensionPart();
                    ((I_ThinExtByRefPartMeasurement) extPart)
                        .setMeasurementValue(new Double(tokens[6])
                            .doubleValue());
                    ((I_ThinExtByRefPartMeasurement) extPart)
                        .setUnitsOfMeasureId(new Integer(tokens[7]).intValue());
                } else if (termFactory
                    .uuidToNative(RefsetAuxiliary.Concept.LANGUAGE_EXTENSION
                        .getUids()) == typeId) {
                    extPart = termFactory.newLanguageExtensionPart();
                    ((I_ThinExtByRefPartLanguage) extPart)
                        .setAcceptabilityId(new Integer(tokens[6]).intValue());
                    ((I_ThinExtByRefPartLanguage) extPart)
                        .setCorrectnessId(new Integer(tokens[7]).intValue());
                    ((I_ThinExtByRefPartLanguage) extPart)
                        .setDegreeOfSynonymyId(new Integer(tokens[8])
                            .intValue());
                }
