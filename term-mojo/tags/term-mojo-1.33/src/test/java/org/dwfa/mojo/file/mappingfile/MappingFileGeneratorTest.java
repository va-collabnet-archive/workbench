...
public class MappingFileGeneratorTest {
	...
	private File conceptsInputFile = new File(BASE_RESOURCE_DIR + "Uuid_sct_concepts_au.gov.nehta.amt.standalone_1.6.txt");
	private File descriptionsInputFile = new File(BASE_RESOURCE_DIR + "Uuid_sct_descriptions_au.gov.nehta.amt.standalone_1.6.txt");
	private File relationshipsInputFile = new File(BASE_RESOURCE_DIR + "Uuid_sct_relationships_au.gov.nehta.amt.standalone_1.6.txt");
	...
	/**
	 * asserts that every file in the test file is in the result file and vice versa
	 * @throws IOException 
	 */
	private void validateFilesEquivalent(File testFile, File expectedResult, Collection<String> expectedExtraTestFileLines, Collection<String> expectedExtraResultSetLines) throws IOException {
		...
		assertEquals("result set set should be the same as the number of read lines", expectedlineCount, resultSet.size());
		...
		assertEquals("test file contains " + extraLines.size() + " extra lines not in expected result", extraLines, expectedExtraTestFileLines);

		assertEquals("expected result file contains " + resultSet.size() + " extra lines not in test file", resultSet, expectedExtraResultSetLines);
